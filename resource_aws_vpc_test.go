...
func TestAccAWSVpc_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccVpcConfigUpdated,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpcExists(resourceName, &vpc),
					testAccCheckVpcCidr(&vpc, "10.2.0.0/16"),
					testAccMatchResourceAttrRegionalARN(resourceName, "arn", "ec2", regexp.MustCompile(`vpc/vpc-.+`)),
					resource.TestCheckResourceAttr(resourceName, "tags.%", "0"),
					resource.TestCheckResourceAttr(resourceName, "assign_generated_ipv6_cidr_block", "false"),
					resource.TestMatchResourceAttr(resourceName, "default_route_table_id", regexp.MustCompile(`^rtb-.+`)),
					resource.TestCheckResourceAttr(resourceName, "cidr_block", "10.2.0.0/16"),
					resource.TestCheckResourceAttr(resourceName, "enable_dns_support", "true"),
					resource.TestCheckResourceAttr(resourceName, "instance_tenancy", "default"),
					resource.TestCheckResourceAttr(resourceName, "ipv6_association_id", ""),
					resource.TestCheckResourceAttr(resourceName, "ipv6_cidr_block", ""),
...
func TestAccAWSVpcIpam_basic(t *testing.T) {
	var vpc ec2.Vpc
	resourceName := "aws_vpc.test"
...
func TestAccAWSVpcIpamIpv6_basic(t *testing.T) {
	var vpc ec2.Vpc
	resourceName := "aws_vpc.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpcIpamIpv6NetmaskHardcoded,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpcExists(resourceName, &vpc),
					// testAccCheckVpcCidrPrefix(&vpc, "56"),
					// testAccMatchResourceAttrRegionalARN(resourceName, "arn", "ec2", regexp.MustCompile(`vpc/vpc-.+`)),
				),
			},
			{
			...
			// Associate does not currently work
			// {
			// 	Config: testAccVpcIpamIpv6NetmaskHardcoded,
			// 	Check: resource.ComposeTestCheckFunc(
			// 		testAccCheckVpcExists(resourceName, &vpc),
			...
			// 	ImportState:             true,
			// 	ImportStateVerify:       true,
			// 	ImportStateVerifyIgnore: []string{"ipv6_netmask_length"},
			// },
		},
	})
}

func TestAccAWSVpcIpamIpv6_basicExplicit(t *testing.T) {
	var vpc ec2.Vpc
	resourceName := "aws_vpc.test"
...
func TestAccAWSVpcIpamExplicitCidr_basic(t *testing.T) {
	var vpc ec2.Vpc
	resourceName := "aws_vpc.test"
	cidr := "172.2.0.0/28"
...
// func testAccVpcIpamIpv6NetmaskHardcoded(pool string) string {
// 	return fmt.Sprintf(`
const testAccVpcIpamIpv6NetmaskHardcoded = `
resource "aws_vpc" "test" {
...
const testAccVpcIpamIpv6ExplicitCidrHardcoded = `
resource "aws_vpc" "test" {
	ipv6_ipam_pool_id   = "ipam-pool-02952e4ca7df60087"
	ipv6_cidr_block     =
