...
sane: prereq-go ## Run sane check
	...
	@TF_ACC=1 $(GO_VER) test \
		...
		./internal/function/... \
		-v -count $(TEST_COUNT) -parallel $(ACCTEST_PARALLELISM) -timeout $(ACCTEST_TIMEOUT) -vet=off -buildvcs=false \
		-run='^$(subst $(eval) ,$$|^,$(strip $(SMOKE_TESTS_STAGE_3)))$$|^TestAccSTSCallerIdentityDataSource_basic$$|^TestAccSecretsManagerSecret_basic$$|^TestAccSSMParameterEphemeral_basic$$|^TestARNParseFunction_known$$'
...
sanity: prereq-go ## Run sanity check (failures allowed)
	...
	lambda=`TF_ACC=1 $(GO_VER) test \
		...
		./internal/function/... \
		-v -count $(TEST_COUNT) -parallel $(ACCTEST_PARALLELISM) -timeout $(ACCTEST_TIMEOUT) -vet=off -buildvcs=false \
		-run='^$(subst $(eval) ,$$|^,$(strip $(SMOKE_TESTS_STAGE_3)))$$|^TestAccSTSCallerIdentityDataSource_basic$$|^TestAccSecretsManagerSecret_basic$$|^TestAccSSMParameterEphemeral_basic$$|^TestARNParseFunction_known$$' || true` ; \
