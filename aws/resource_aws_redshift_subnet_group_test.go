...
func testSweepRedshiftSubnetGroups(region string) error {
	...
	if err != nil {
		errors = multierror.Append(errors, fmt.Errorf("error describing Redshift Subnet Groups: %w", err))
	}

	if len(sweepResources) > 0 {
		// any errors didn't prevent gathering of some work, so do it
		if err := testSweepResourceOrchestrator(sweepResources); err != nil {
			errors = multierror.Append(errors, fmt.Errorf("error sweeping Redshift Subnet Groups for %s: %w", region, err))
		}
	}

	if testSweepSkipSweepError(errors.ErrorOrNil()) {
		log.Printf("[WARN] Skipping Redshift Subnet Group sweep for %s: %s", region, errors)
		return nil
	}
