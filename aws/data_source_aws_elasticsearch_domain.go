...
func dataSourceAwsElasticSearchDomainRead(d *schema.ResourceData, meta interface{}) error {
	ElasticSearchConn := meta.(*awsprovider.AWSClient).ElasticSearchConn
	...
	resp, err := ElasticSearchConn.DescribeElasticsearchDomain(req)
	if err != nil {
	...
	tags, err := keyvaluetags.ElasticsearchserviceListTags(ElasticSearchConn, d.Id())
