...
func TestAccAWSCognitoUserPool_withSchemaAttributes(t *testing.T) {
	name := acctest.RandString(5)
	var userpool CognitoUserPoolInfo
...
type CognitoUserPoolInfo struct {
	UserPoolID string
}

func testAccCheckAWSCognitoUserPoolExists(name string, existing *CognitoUserPoolInfo) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if existing != nil {
			existing.UserPoolID = *poolInfo.UserPool.Id
		}
...
func testAccCheckAWSCognitoUserPoolSame(name string, existing *CognitoUserPoolInfo) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[name]
		if !ok {
			return fmt.Errorf("Not found: %s", name)
		...
		if rs.Primary.ID != existing.UserPoolID {
			return fmt.Errorf("Expected Cognito Pool ID %s, but got %s", existing.UserPoolID, rs.Primary.ID)
		}
