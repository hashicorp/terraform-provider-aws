...
func ValidateArn(v interface{}, k string) (ws []string, errors []error) {
	...
	} else if !awsprovider.PartitionRegexp.MatchString(parsedARN.Partition) {
		errors = append(errors, fmt.Errorf("%q (%s) is an invalid ARN: invalid partition value (expecting to match regular expression: %s)", k, value, awsprovider.PartitionRegexpPattern))
	}

	if parsedARN.Region != "" && !awsprovider.RegionRegexp.MatchString(parsedARN.Region) {
		errors = append(errors, fmt.Errorf("%q (%s) is an invalid ARN: invalid region value (expecting to match regular expression: %s)", k, value, awsprovider.RegionRegexpPattern))
	}

	if parsedARN.AccountID != "" && !awsprovider.AccountIDRegexp.MatchString(parsedARN.AccountID) {
		errors = append(errors, fmt.Errorf("%q (%s) is an invalid ARN: invalid account ID value (expecting to match regular expression: %s)", k, value, awsprovider.AccountIDRegexpPattern))
	}
...
func validateVersionString(v interface{}, k string) (ws []string, errors []error) {
	...
	if !awsprovider.VersionStringRegexp.MatchString(value) {
