...
func TestAccAWSCodeDeployDeploymentGroup_onPremiseTagSet(t *testing.T) {
	var group codedeploy.DeploymentGroupInfo
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSCodeDeployDeploymentGroupOnPremiseTagSet(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSCodeDeployDeploymentGroupExists("aws_codedeploy_deployment_group.foo", &group),
					resource.TestCheckResourceAttr(
						"aws_codedeploy_deployment_group.foo", "app_name", "foo_app_"+rName),
...
func testAccAWSCodeDeployDeploymentGroupOnPremiseTagSet(rName string) string {
	return fmt.Sprintf(`
resource "aws_codedeploy_app" "foo_app" {
  name = "foo_app_%s"
}
