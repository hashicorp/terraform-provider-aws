		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("aws_mq_broker.test", "engine_version", "5.15.9"),
					resource.TestCheckResourceAttr("aws_mq_broker.test", "storage_type", "ebs"),
					resource.TestCheckResourceAttr("aws_mq_broker.test", "host_instance_type", "mq.m5.large"),
...
func TestAccAWSMqBroker_rabbitmq(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMqBrokerConfigRabbit(sgName, brokerName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAwsMqBrokerExists(resourceName, &broker),
...
func testAccMqBrokerConfigRabbit(sgName, brokerName string) string {
	return fmt.Sprintf(`
resource "aws_security_group" "test" {
  name = "%s"
}
