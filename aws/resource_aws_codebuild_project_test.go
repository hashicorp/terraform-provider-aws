...
func init() {
	atest.RegisterServiceErrorCheckFunc(codebuild.EndpointsID, testAccErrorCheckSkipCodebuild)
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					atest.CheckAttrRegionalARN(resourceName, "arn", "codebuild", fmt.Sprintf("project/%s", rName)),
					...
					atest.CheckAttrRegionalARN(resourceName, "encryption_key", "kms", "alias/aws/s3"),
...
func TestAccAWSCodeBuildProject_FileSystemLocations(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:     func() { atest.PreCheck(t); testAccPreCheckAWSCodeBuild(t) },
		ErrorCheck:   atest.ErrorCheck(t, codebuild.EndpointsID, "efs"), //using efs.EndpointsID will import efs and make linters sad
		Providers:    atest.Providers,
		CheckDestroy: testAccCheckAWSCodeBuildProjectDestroy,
		Steps: []resource.TestStep{
			{
...
func TestAccAWSCodeBuildProject_SecondaryArtifacts_Name(t *testing.T) {
	atest.Skip(t, "Currently no solution to allow updates on name attribute")
...
func testAccAWSCodeBuildProjectConfig_basic(rName string) string {
	return atest.ComposeConfig(testAccAWSCodeBuildProjectConfig_Base_ServiceRole(rName), fmt.Sprintf(`
