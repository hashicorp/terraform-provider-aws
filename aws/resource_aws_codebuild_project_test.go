...
func TestAccAWSCodeBuildProject_Source_BuildStatusConfig(t *testing.T) {
	var project codebuild.Project
	rName := acctest.RandomWithPrefix("tf-acc-test")
	resourceName := "aws_codebuild_project.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSCodeBuildProjectConfig_Source_BuildStatusConfig(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSCodeBuildProjectExists(resourceName, &project),
					resource.TestCheckResourceAttr(resourceName, "source.0.build_status_config.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "source.0.build_status_config.0.context", "codebuild"),
					resource.TestCheckResourceAttr(resourceName, "source.0.build_status_config.0.target_url", "https://local/buildstatusconfig"),
				),
			},
			{
				ResourceName:      resourceName,
			...
			{
				Config: testAccAWSCodeBuildProjectConfig_Source_BuildStatusConfig_2(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSCodeBuildProjectExists(resourceName, &project),
					resource.TestCheckResourceAttr(resourceName, "source.0.build_status_config.#", "0"),
				),
			},
		},
	})
...
resource "aws_codebuild_project" "test" {
  ...
  source {
    ...
    build_status_config {
        context    = "codebuild"
        target_url = "https://local/buildstatusconfig"
    }
  }
}
`, rName)
...
func testAccAWSCodeBuildProjectConfig_Source_BuildStatusConfig_2(rName string) string {
	return testAccAWSCodeBuildProjectConfig_Base_ServiceRole(rName) + fmt.Sprintf(`
resource "aws_codebuild_project" "test" {
  name         = "%s"
  service_role = aws_iam_role.test.arn
