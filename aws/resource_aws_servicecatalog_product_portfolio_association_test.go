		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAwsServiceCatalogProductPortfolioAssociationExists(resourceName),
					atest.CheckDisappears(atest.Provider, resourceAwsServiceCatalogProductPortfolioAssociation(), resourceName),
				),
...
func testAccAWSServiceCatalogProductPortfolioAssociationConfig_basic(rName string) string {
	return atest.ComposeConfig(testAccAWSServiceCatalogProductPortfolioAssociationConfig_base(rName), `
