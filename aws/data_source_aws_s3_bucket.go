...
func dataSourceAwsS3BucketRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("arn", arn)
	d.Set("bucket_domain_name", meta.(*awsprovider.AWSClient).PartitionHostname(fmt.Sprintf("%s.s3", bucket)))

	err = bucketLocation(meta.(*awsprovider.AWSClient), d, bucket)
	if err != nil {
...
func bucketLocation(client *awsprovider.AWSClient, d *schema.ResourceData, bucket string) error {
