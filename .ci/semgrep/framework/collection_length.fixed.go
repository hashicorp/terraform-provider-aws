...
func testNotNull2(x basetypes.SetValue) {
	// ruleid: collection-length-not-null
	if x.Length(fwtypes.CollectionLengthUnhandledAsZero) > 0 {
		_ = x
	}
}
...
func testLength3(x basetypes.MapValue) {
	logAttrs := map[string]any{
		// ruleid: collection-length
		"size": x.Length(fwtypes.CollectionLengthUnhandledAsZero),
	}
	_ = logAttrs
}
...
func testLength4(x basetypes.ListValue) []string {
	// ruleid: collection-length
	out := make([]string, x.Length(fwtypes.CollectionLengthUnhandledAsZero))
	return out
}

func testLength5(x basetypes.ListValue) int32 {
	// ruleid: collection-length
	return int32(x.Length(fwtypes.CollectionLengthUnhandledAsZero))
}
