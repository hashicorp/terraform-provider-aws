...
The most common case is parameter `rName` with a value generated by `acctest.RandomWithPrefix(t, acctest.ResourcePrefix)`, so this is the default.
...
Some acceptance tests require an ACM certificate associated with a specific root domain.
The root domain is read from the environment variable `ACM_CERTIFICATE_ROOT_DOMAIN`.
This can be included by setting the annotation `@Testing(acmRootDomainTfVar)` or `@Testing(acmRootDomainTfVar="<variable name>")`.
The default Terraform variable name is `rootDomain`.
The test will be skipped if the environment variable is not set.

A random subdomain of the root domain can be generated by setting the annotation `@Testing(acmSubdomainTfVar="<parent variable>;<variable name>")`.
The default parent variable name is `rootDomain` and the default Terraform variable name is `domainName`.
For example, `@Testing(acmSubdomainTfVar="rootDomain;domainName")` generates a random subdomain of the `rootDomain` variable.
