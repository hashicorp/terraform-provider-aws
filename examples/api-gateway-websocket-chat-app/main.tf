...
resource "aws_api_gateway_v2_api" "SimpleChatWebSocket" {
  name                       = "SimpleChatWebSocket"
  protocol_type              = "WEBSOCKET"
  route_selection_expression = "$request.body.message"
}

resource "aws_api_gateway_v2_deployment" "Deployment" {
  api_id = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"

  depends_on = [
    "aws_api_gateway_v2_route.ConnectRoute",
    "aws_api_gateway_v2_route.DisconnectRoute",
    "aws_api_gateway_v2_route.SendRoute",
  ]
}

resource "aws_api_gateway_v2_stage" "Stage" {
  api_id        = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
  name          = "Prod"
  description   = "Prod Stage"
  deployment_id = "${aws_api_gateway_v2_deployment.Deployment.id}"
}

###########
# OnConnect
###########
resource "aws_api_gateway_v2_integration" "ConnectIntegrat" {
  api_id             = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
...
resource "aws_api_gateway_v2_route" "ConnectRoute" {
  api_id         = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
  route_key      = "$connect"
  operation_name = "ConnectRoute"
  target         = "integrations/${aws_api_gateway_v2_integration.ConnectIntegrat.id}"
}

##############
# OnDisconnect
##############
resource "aws_api_gateway_v2_integration" "DisconnectInteg" {
  api_id             = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
  integration_type   = "AWS_PROXY"
...
resource "aws_api_gateway_v2_route" "DisconnectRoute" {
  api_id         = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
  route_key      = "$disconnect"
  operation_name = "DisconnectRoute"
  target         = "integrations/${aws_api_gateway_v2_integration.DisconnectInteg.id}"
}

#############
# SendMessage
#############
resource "aws_api_gateway_v2_integration" "SendInteg" {
  api_id             = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
  integration_type   = "AWS_PROXY"
...
resource "aws_api_gateway_v2_route" "SendRoute" {
  api_id         = "${aws_api_gateway_v2_api.SimpleChatWebSocket.id}"
  route_key      = "sendmessage"
  operation_name = "SendRoute"
  target         = "integrations/${aws_api_gateway_v2_integration.SendInteg.id}"
}
  ...
  policy = <<EOT
{
  ...
  "Statement": [{
    ...
    "Resource": "${aws_api_gateway_v2_api.SimpleChatWebSocket.execution_arn}/*"
  }]
}
EOT
}
...
output "WebSocketURI" {
  value = "${aws_api_gateway_v2_stage.Stage.invoke_url}"
}
