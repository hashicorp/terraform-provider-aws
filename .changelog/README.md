...
- **Imperative voice.** Write "Add", "Fix", "Support", "Remove" — not "Adds",
  "Added", or "Fixes". (The `new-*` types are the exception: just the name.)
- **Brief and specific.** One line describing the user-visible change. Name the
  concrete error, argument, or behavior rather than "various fixes".
- **Backtick identifiers.** Wrap resource names, arguments, attributes, and
  literal values in backticks.
- **Don't start descriptions with `The`, `A`, or `An`** (except deprecation
  notes, which read naturally as "The `x` attribute has been deprecated.").
- **One file per PR**, named `<PR_NUMBER>.txt`; combine multiple related notes
  as separate blocks in that single file.
