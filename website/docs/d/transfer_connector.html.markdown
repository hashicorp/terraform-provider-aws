...
* `as2_config` - Structure containing the parameters for an AS2 connector object. Contains the following attributes:
    ...
    * `mdn_response` - Whether outbound requests use an asynchronous response. Will be either SYNC or NONE.
    ...
    * `singing_algorithm` - Algorithm used for signing AS2 messages sent with the connector.
