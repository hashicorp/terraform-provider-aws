...
* `dns_support` (Optional) - Indicates whether DNS resolution is enabled for the core network. The value can be either `true` or `false`. When set to `true`, DNS resolution is enabled for VPCs attached to the core network, allowing resources in different VPCs to resolve each other's domain names. Omitted from the rendered document when not set, in which case AWS applies its own default of `true`.
* `security_group_referencing_support` — (Optional) Indicates whether security group referencing is enabled for the core network. The value can be either `true` or `false`. When set to `true`, security groups in one VPC can reference security groups in another VPC attached to the core network, enabling more flexible security configurations across your network. Omitted from the rendered document when not set, in which case AWS applies its own default of `false`. Core networks created before this setting existed can reject a policy that contains the key at all, so leave it unset unless you need it.
