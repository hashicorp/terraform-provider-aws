...
* `identity_provider_type` - Mode of authentication enabled for this service.
...
* `security_policy_name` - Name of the security policy that is attached to the server.
* `structured_log_destinations` - Set of ARNs of destinations that will receive structured logs from the transfer server such as CloudWatch Log Group ARNs.
