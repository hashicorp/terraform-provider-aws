...
* `filter` - (Optional) One or more name/value pairs to filter off of. For a full reference, check out [describe-images in the AWS CLI reference][1]. See [`filter` Block](#filter-block) below.
