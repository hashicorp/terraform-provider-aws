...
* `neuron_devices` - Describes the Neuron accelerator settings for the instance type.
    ...
    * `neuron_devices.#.version` - A number representing the version of the neuron accelerator.
...
* `srd_supported` - `true` if the instance type supports [ENA Express](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ena-express.html).
