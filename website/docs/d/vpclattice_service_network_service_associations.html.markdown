...
* `items` - List of objects containing Service Network Service Associations. Each object has the following attributes:
    ...
    * `created_at` - The date and time the association was created, in RFC 3339 format.
    ...
    * `status` - The status of the association. One of `CREATE_IN_PROGRESS`, `ACTIVE`, `DELETE_IN_PROGRESS`, `CREATE_FAILED`, or `DELETE_FAILED`.
