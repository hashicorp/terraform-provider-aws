...
* `use_fips_endpoint` - (Optional) Force the provider to resolve endpoints with FIPS capability for all services. Can also be set with the `AWS_USE_FIPS_ENDPOINT` environment variable or in a shared config file (`use_fips_endpoint`). This setting is ignored for any service with a custom endpoint specified. See also `endpoints`.
