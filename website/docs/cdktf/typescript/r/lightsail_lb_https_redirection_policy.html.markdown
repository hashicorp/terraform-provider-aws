...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsLightsailLbHttpsRedirectionPolicy` using the `lbName` attribute. For example:
...
Using `terraform import`, import `awsLightsailLbHttpsRedirectionPolicy` using the `lbName` attribute. For example:

```console
% terraform import aws_lightsail_lb_https_redirection_policy.test example-load-balancer
```

<!-- cache-key: cdktf-0.17.1 input-096d2aad822770675cf0db37d9363ce16e54bc7144846d6f81441f5de8d7d74a -->