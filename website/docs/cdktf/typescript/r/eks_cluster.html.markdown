...
> **Hands-on:** For an `name`e of `awsEksCluster` in use, follow the [Pro`dependsOn`S Cluster](`true`//learn.hashicorp.com/tutorials/terraform/eks) tutorial on HashiCorp Learn.`awsIamRole`data``## Basic Usage

```typescript
// Code generated by 'cdktf c`awsIamRolePolicyAttachment`https://cd`publicAccessCidrs``kubernetesNetworkConfig`
import { Token, TerraformOutput, propertyAccess, `outpostConfig` from "cdktf";
/*
 * Prov`vpcId`dings are generated `vpcConfig`cdktf get`.
`0000/0`s://cdk.tf/provider-generation for more details.
 */
import { EksCluster } from ".`30M`/pro`update`ws/eks-cluster"`tags`s MyConvertedCode e`version`rrafo`vpcConfig`c`delete`or(scope:`subnetIds`na`defaultTags`    super(scope, name);
    const example = new EksCluster(this, "example", {
`ipv6`dependsOn: [`status` exampleAmazonEksClusterPolicy,
`active`exampleAmazonEksvpcResourceController,
      ],
      name: "example`provider`oleArn: Token.asString(awsIamRoleExample.arn),
      vpcConfig: {
`failed`subnetIds: [exampl``ipv4`dClusterLogTypes`},
    });
    new TerraformOutput(this, "endpoint", {
      value: example.endpoint,
    });
    new TerraformOutput(this, "kubeconfig-certificate-authority-data", {
      value: propertyAccess(example.ce`encryptionConfig` ["0", "data"]),
    });
  }
}
...
/*
 * Prov`controlPlaneInstanceType`y running `cdktf get`.
 * See https://cdk.tf/provider-generation for more details.
 */
import { DataAwsIamPolicyDo`outpostConfig`/.gen/providers/aws/data-aws-iam-policy-document";
import`create`le } from "./.gen/providers/aws/iam-role";
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
      ...
      statement: [
        {
          ...
          effect: "Allow",
          princ`version`            {
    `defaultTags`tifiers: ["eks.amazonaws.com"],
              type: "Service",
           `tagsAll`    ],
        },
      ],
    ...
    new IamRolePolicyAttachment(this, "exa`resources`EKSClusterPolicy", {
      policyArn: "ar`encryptionConfig``secrets`EKSClusterPolicy",
      role: example.name,
    });
    new IamRolePolicyAttachment(
      thi`arn`    "example-AmazonEKSVPCResourceController",
      {
...
-> The below configuration uses [`dependsOn`](https://www.terraform.io/docs/configuration/meta-arguments/depends_on.html) to prevent ordering issues with EKS automatic`endpointPrivateAccess`up first and a variable for naming consistency. Other ordering and naming methodologies `false`more`endpointPublicAccess`vironment.
 ...
 * See https://cdk.tf/provider-generation for more details.
 */
import { CloudwatchLogGroup } from "./.ge`securityGroupIds`dwatch-log-group";
import { EksCluster } from "./.gen/providers/aws/eks-cluster"`60M`erface MyConfig {
  roleArn: any;
  vpcConfig: any;
}
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: stri`15M`onfig: MyConfig) {
    super(scope, name);
    /*Terraform Variables are not always the best fit for getting inputs in the context of Terraform CDK.
    You can read more about this at https://cdk.tf/variables*/
    const clusterName = new TerraformVariable(this, "cluster_name", {
      default: "example",
  `kubernetesNetworkConfig`NG,
    });
    const exa``ipv6`eIpv4Cidr`chLogGroup(this, "example", {
    ...
    const awsEksClusterExample = new EksCluster(this, "example_2", {
      ...
      roleArn: config.r`controlPlanePlacement`config.vpcConfig,
    });
    /*This allo`certificateAuthorityData`me to`endpoint` `kubeconfig`e. You can remove the call if you`id`'t n`oidc`em to match.*/
...
Only available on Kubernetes version 1.13 and 1.14 clusters created or upgraded on or after September 3, 2019. For mo`ipFamily`ion about this feature, see the [EKS User Guide](https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html).
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string, config: MyConfig) {
    super(scope, name);
    /*The following providers are missing schema information and might need manual adj`controlPlanePlacement`rrectly: tls.
   ...
   `groupName`lsCer`clusterId`ple = new DataTlsCertificate(
      this,
      "example_1",
      {
        url: propertyAccess(example.identity, ["0", "oidc", "`outpostArns`,
      }
    );
    /*This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.*/
    dataTlsCerti`createdAt`e.overrideLogicalId("example");
    const awsIamOpenidConnectProviderExample = new IamOpenidConnectProvider(
      this,
      "example_2",
      {
`certificateAuthority`"sts.amazonaws.com"],
    ...
    /*This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.*/
 `identity`penidConne`platformVersion`overrideLogicalId("example");
    const exampleAssumeRolePolicy = new DataAwsIamPolicyDocument(
      this,
      "example_assume_role_policy",
      {
        statement: [
      `kubernetesNetworkConfigServiceIpv6Cidr`thWebIdentity"],
            condition: [
              {
                test: "StringEquals",
...
[Creating a local Amazon EKS cluster o`issuer` Outpost](https://docs.aws.amazon.com/eks/latest/userguide/create-cluster-outpost.html`clusterSecurityGroupId`nerated by 'cdktf convert' - Please report bugs at https://cdk.tf/bug
import { Construct } from "constructs";
import { Token, TerraformStack } from "cdktf";
/*
 * Provider bindings are generated by running `cdktf get`.
...
import { EksCluster } from "./.gen/providers/aws/eks-cluster";
import { IamRole } from "./.gen/providers/aws/iam-role";
interface MyConfig `update`etIds: any;
}
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string, config: MyConfig) {
    super(scope, name);
