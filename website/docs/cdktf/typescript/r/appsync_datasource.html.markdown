...
* `serviceRoleArn` - (Optional) IAM service role ARN for the data source. Required if `type` is specified as `awsLambda`, `amazonDynamodb`, `amazonElasticsearch`, `amazonEventbridge`, or `amazonOpensearchService`.
