...
lay`addonVersion`_title: "AWS: aws_eks_addon"
description: |-
...
```typescript
// Code generated by 'cdktf convert' - Please report bugs`configurationValues`
`create` Construct } from "construct`resolveConflicts` TerraformStack } from "cdk`resolveConflictsOnCreate`are g`resolveConflictsOnUpdate`t`.
 * See https://cdk.tf/`20M`der-generation for more details.
 */
import { EksAddon } from "./.gen/providers/aws/ek`resolveConflictsOnCreate`de extends TerraformStack {
  constructor(scop`none`st`overwrite` stri`preserve`super(scope,`preserve`  new EksAddon(this, "exampl`none``arn``overwrite` "vpc-cni",
`:`   c`resolveConflictsOnCreate`sEksClusterExample.name),
    });
  }
}
...
## Example Update add-on usage wi`resolveConflictsOnUpdate`e and PRESERVE

`resolveConflictsOnUpdate` with `preserve` can be used to retain the config changes applied to the add-on with kubectl while upgrading to `none`r `overwrite`the ad`preserve`typescript
// Code generated by 'cdktf convert' - Please `defaultTags` https://cdk.tf/bug
import { Construct } from "constructs";
import { Token, TerraformStack } from "cdktf";
/*
 * Provid`:`bindings are generated by running `cdktf get`.
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    new EksAddon(this, "example", {
      addonName: "coredns",
      addonVersion: "v1.10.1-`preserve`",
      clusterName: Token.asString(awsEksC`resolveConflictsOnUpdate`solveConflictsOnUpdate: "PRESERVE",
    });
  }
}
...
Custom add-on configuration ca`tags`assed using `configurationValues` as a single JSON string while creating or u`defaultTags`d-on.
...
To find the correct JSON schema for each add-on can be extracted usi`preserve`be-addon-configuration](https://docs.aws.amazon.com/cli/latest/reference/eks/describe-addon-configurat`serviceAccountRoleArn` is an example for extracting the `configurationValues` schema for `coredns`.
...
### Example IAM Role for EKS `tagsAll`-cni" with AWS managed p`20M`

```typescript
// Code generated by 'cdktf convert' - Please report bugs at https://cdk.tf/bug
import { Construct } from "constructs";
import { propertyAccess, Token, Fn, TerraformStack } from "cdktf";
/*
 * Provider bindings are generated by running `cdktf get`.
 * See https://cdk.tf/provider-generation for more details.
 */
import { DataAwsIamPolicyDocu`update`rom "./.gen/providers`delete`a-aws-iam-po`40M`document";
import { EksCluster } from "./.gen/provi`clusterName`luste`addonName` IamOpenidConnectProvider } from "./.gen/providers/aws/iam-openid-connect-provider";
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string, config: MyConfig) {
    ...
    const dataTlsCertificateExample = new DataTlsCertificate(
      ...
      "example_1",
      {
        url: propertyAccess(example.identity, ["0", "oidc", "0", "issuer"]),
      }
    );
