...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import CloudWatch Log Metric Filter using the `logGroupName:name`. For example:
...
Using `terraform import`, import CloudWatch Log Metric Filter using the `logGroupName:name`. For example:

```console
% terraform import aws_cloudwatch_log_metric_filter.test /aws/lambda/function:test
```

<!-- cache-key: cdktf-0.17.1 input-c3539b00aea6703f03f30c699d9e6682c77afa8c2025da9bd2bce0e294523841 -->