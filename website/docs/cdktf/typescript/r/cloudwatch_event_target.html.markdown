...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    const graphqlApi = new AppsyncGraphqlApi(this, "graphql-api", {
      authenticationType: "AWS_IAM",
      name: "api",
      schema:
        "    schema {\n      mutation: Mutation\n      query: Query\n    }\n\n    type Query {\n      testQuery: String\n    }\n\n    type Mutation {\n      testMutation(input: MutationInput!): TestMutationResult\n    }\n\n    type TestMutationResult {\n      test: String\n    }\n\n    input MutationInput {\n      testInput: String\n    }\n\n",
    });
    const invokeAppsyncMutation = new CloudwatchEventRule(
      this,
    ...
    const appsyncMutationRolePolicyDocument = new DataAwsIamPolicyDocument(
      this,
    ...
    const appsyncMutationRoleTrust = new DataAwsIamPolicyDocument(
      this,
    ...
    const appsyncMutationRolePolicy = new IamPolicy(
      this,
      "appsync_mutation_role_policy",
      {
        name: "appsync-mutation-role-policy",
        policy: Token.asString(appsyncMutationRolePolicyDocument.json),
      }
    );
    const appsyncMutationRole = new IamRole(this, "appsync_mutation_role", {
      assumeRolePolicy: Token.asString(appsyncMutationRoleTrust.json),
      name: "appsync-mutation-role",
    });
    new IamRolePolicyAttachment(this, "appsync_mutation_role_attachment", {
      policyArn: appsyncMutationRolePolicy.arn,
      role: appsyncMutationRole.name,
    });
    const awsCloudwatchEventTargetInvokeAppsyncMutation =
      new CloudwatchEventTarget(this, "invoke_appsync_mutation_7", {
        appsyncTarget: {
          graphqlOperation:
            "mutation TestMutation($input:MutationInput!){testMutation(input: $input) {test}}",
        },
        arn: Token.asString(
          Fn.replace(graphqlApi.arn, "apis", "endpoints/graphql-api")
        ),
        inputTransformer: {
          inputPaths: {
            input: "$.detail.input",
    ...
    /*This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.*/
    awsCloudwatchEventTargetInvokeAppsyncMutation.overrideLogicalId(
      "invoke_appsync_mutation"
    );
  }
}
...
* `graphqlOperation` - (Optional) Contains the GraphQL mutation to be parsed and executed.
