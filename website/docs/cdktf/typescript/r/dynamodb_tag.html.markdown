...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsDynamodbTag` using the DynamoDB resource identifier and key, separated by a comma (`,`). For example:
...
Using `terraform import`, import `awsDynamodbTag` using the DynamoDB resource identifier and key, separated by a comma (`,`). For example:
