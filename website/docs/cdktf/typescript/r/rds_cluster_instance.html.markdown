...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string, config: MyConfig) {
    ...
    new RdsClusterInstance(this, "cluster_instances", {
      clusterIdentifier: defaultVar.id,
      engine: defaultVar.engine,
      engineVersion: defaultVar.engineVersion,
      identifier: "aurora-cluster-demo-${" + clusterInstancesCount.index + "}",
...
* `forceDestroy` - (Optional) Forces an instance to be destroyed when a part of a read replica cluster. **Note:** will promote the read replica to a standalone cluster before instance deletion.
