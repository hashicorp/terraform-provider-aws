...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsDetectiveInvitationAccepter` using the graph ARN. For example:
...
Using `terraform import`, import `awsDetectiveInvitationAccepter` using the graph ARN. For example:
