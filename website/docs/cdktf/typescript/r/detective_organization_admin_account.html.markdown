...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsDetectiveOrganizationAdminAccount` using `accountId`. For example:
...
Using `terraform import`, import `awsDetectiveOrganizationAdminAccount` using `accountId`. For example:
