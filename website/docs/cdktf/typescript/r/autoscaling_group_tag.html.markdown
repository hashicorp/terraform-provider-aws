...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsAutoscalingGroupTag` using the ASG name and key, separated by a comma (`,`). For example:
...
Using `terraform import`, import `awsAutoscalingGroupTag` using the ASG name and key, separated by a comma (`,`). For example:
