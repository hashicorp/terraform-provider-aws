...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import DataZone Project using a colon-delimited string combining `domainId` and `id`. For example:
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    DatazoneProject.generateConfigForImport(
      this,
      "example",
      "domain-1234:project-1234"
    );
  }
}
...
Using `terraform import`, import DataZone Project using a colon-delimited string combining `domainId` and `id`. For example:

```console
% terraform import aws_datazone_project.example domain-1234:project-1234
```

<!-- cache-key: cdktf-0.20.8 input-f4930f33d20f0d4998990d008e7126f3b8b3a0f12964d87688eec35a9b0fcb0c -->