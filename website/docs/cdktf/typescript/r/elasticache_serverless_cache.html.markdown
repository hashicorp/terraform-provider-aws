...
The `cacheUsageLimits` configuration block supports the following arguments:
...
* `ecpuPerSecond` - The configuration for the number of ElastiCache Processing Units (ECPU) the cache can consume per second. See [`ecpuPerSecond` Block](#ecpu_per_second-block) for details.

### `dataStorage` Block

The `dataStorage` configuration block supports the following arguments:
...
The `ecpuPerSecond` configuration block supports the following arguments:
...
The `readerEndpoint` configuration block exports the following attributes:
