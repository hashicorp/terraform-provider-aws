...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsServicecatalogProvisioningArtifact` using the provisioning artifact ID and product ID separated by a colon. For example:
...
Using `terraform import`, import `awsServicecatalogProvisioningArtifact` using the provisioning artifact ID and product ID separated by a colon. For example:
