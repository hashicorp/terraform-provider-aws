...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    new DefaultRouteTable(this, "example", {
      defaultRouteTableId: Token.asString(awsVpcExample.defaultRouteTableId),
      route: [
        {
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Default VPC route tables using the `vpcId`. For example:
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    DefaultRouteTable.generateConfigForImport(this, "example", "vpc-33cc44dd");
  }
}

```

Using `terraform import`, import Default VPC route tables using the `vpcId`. For example:
...
<!-- cache-key: cdktf-0.20.8 input-52890b2ce73c129b096972d97be8827bd1630df722976c50bc3863ebadce1093 -->