...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsAppsyncResolver` using the `apiId`, a hyphen, `type`, a hypen and `field`. For example:
...
Using `terraform import`, import `awsAppsyncResolver` using the `apiId`, a hyphen, `type`, a hypen and `field`. For example:

```console
% terraform import aws_appsync_resolver.example abcdef123456-exampleType-exampleField
```

<!-- cache-key: cdktf-0.17.1 input-c52e7e65e799b6104f6c311a148eb1f52433b7035e764e94e6ade1153f6ef6e4 -->