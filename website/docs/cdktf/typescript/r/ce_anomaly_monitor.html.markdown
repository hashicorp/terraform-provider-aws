...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsCeAnomalyMonitor` using the `id`. For example:
...
Using `terraform import`, import `awsCeAnomalyMonitor` using the `id`. For example:
