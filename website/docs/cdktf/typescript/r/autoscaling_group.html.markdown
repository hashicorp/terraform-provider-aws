  ...
  constructor(scope: Construct, name: string) {
    ...
    new AutoscalingGroup(this, "test", {
      ...
      tag: testDynamicIterator0.dynamic({
        key: propertyAccess(testDynamicIterator0.value, ["key"]),
        propagate_at_launch: propertyAccess(testDynamicIterator0.value, [
          "propagate_at_launch",
        ]),
        value: propertyAccess(testDynamicIterator0.value, ["value"]),
      }),
      vpcZoneIdentifier: [example1.id, example2.id],
    });
  }
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    you need to keep this like it is.*/
    const testDynamicIterator0 = TerraformIterator.fromList(
      Token.asAny(propertyAccess(awsVpclatticeTargetGroupTest, ["*"]))
    );
    new AutoscalingGroup(this, "test", {
      forceDelete: true,
      maxSize: 1,
      ...
      trafficSource: testDynamicIterator0.dynamic({
        identifier: propertyAccess(testDynamicIterator0.value, ["arn"]),
        type: "vpc-lattice",
      }),
    });
  }
