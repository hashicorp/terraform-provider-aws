...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    const glueJobRole = new IamRole(this, "glue_job_role", {
    ...
    new S3Object(this, "glue_etl_script", {
      bucket: glueScripts.id,
      key: "jobs/etl_job.py",
      source: "jobs/etl_job.py",
    });
    new GlueJob(this, "etl_job", {
      command: {
        name: "glueetl",
        pythonVersion: "3",
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    new S3Object(this, "python_shell_script", {
      bucket: glueScripts.id,
      key: "jobs/shell_job.py",
      source: "jobs/shell_job.py",
    });
    new GlueJob(this, "python_shell_job", {
      command: {
        name: "pythonshell",
        pythonVersion: "3.9",
      ...
      maxCapacity: Token.asNumber("0.0625"),
      maxRetries: 0,
...
* `workerType` - (Optional) The type of predefined worker that is allocated when a job runs. Valid values: `Standard`, `G.1X`, `G.2X`, `G.025X`, `G.4X`, `G.8X`, `G.12X`, `G.16X`, `R.1X`, `R.2X`, `R.4X`, `R.8X`, `Z.2X` (Ray jobs). See the [AWS documentation](https://docs.aws.amazon.com/glue/latest/dg/worker-types.html) for details.
...
* `authStrategy` - (Optional) The type of authentication, which can be an authentication token stored in Amazon Web Services Secrets Manager, or a personal access token. Valid values are: `PERSONAL_ACCESS_TOKEN` and `AWS_SECRETS_MANAGER`.
* `authToken` - (Optional) The value of an authorization token.
* `branch` - (Optional) A branch in the remote repository.
* `folder` - (Optional) A folder in the remote repository.
* `lastCommitId` - (Optional) The last commit ID for a commit in the remote repository.
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    GlueJob.generateConfigForImport(this, "myJob", "MyJob");
  }
}
