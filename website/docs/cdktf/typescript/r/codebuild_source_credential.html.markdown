...
* `token` - (Required) For a GitHub and GitHub Enterprise, this is the personal access token. For Bitbucket, this is the
  ...
  Connection ARN.
* `userName` - (Optional) The Bitbucket username when the authType is `BASIC_AUTH`. This parameter is not valid for
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    CodebuildSourceCredential.generateConfigForImport(
      this,
      "example",
      "arn:aws:codebuild:us-west-2:123456789:token:github"
    );
  }
}
