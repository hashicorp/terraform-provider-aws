...
* `bucketRegion` - (Optional) The region of the S3 buck used by the file share. Required when specifying a `vpc_endpoint_dns_name`.
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsStoragegatewaySmbFileShare` using the SMB File Share Amazon Resource Name (ARN). For example:
...
Using `terraform import`, import `awsStoragegatewaySmbFileShare` using the SMB File Share Amazon Resource Name (ARN). For example:
