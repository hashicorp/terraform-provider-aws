...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsApiGatewayMethodSettings` using `restApiId/stageName/methodPath`. For example:
...
Using `terraform import`, import `awsApiGatewayMethodSettings` using `restApiId/stageName/methodPath`. For example:
