 ...
 */
import { ConnectHoursOfOperation } from "./.gen/providers/aws/connect-hours-of-operation";
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    new ConnectHoursOfOperation(this, "test", {
      config: [
        {
          day: "MONDAY",
          endTime: {
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Amazon Connect Hours of Operations using the `instanceId` and `hoursOfOperationId` separated by a colon (`:`). For example:
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    ConnectHoursOfOperation.generateConfigForImport(
      this,
      "example",
...
Using `terraform import`, import Amazon Connect Hours of Operations using the `instanceId` and `hoursOfOperationId` separated by a colon (`:`). For example:
