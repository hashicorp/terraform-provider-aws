...
* `endpointType` - (Required) - The type of Verified Access endpoint to create. Currently `loadBalancer` or `networkInterface` are supported.
...
* `loadBalancerOptions` - (Optional) The load balancer details. This parameter is required if the endpoint type is `loadBalancer`.
* `networkInterfaceOptions` - (Optional) The network interface details. This parameter is required if the endpoint type is `networkInterface`.
