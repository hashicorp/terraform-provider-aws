...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Lambda function URLs using the `functionName` or `functionName/qualifier`. For example:
...
Using `terraform import`, import Lambda function URLs using the `functionName` or `functionName/qualifier`. For example:
