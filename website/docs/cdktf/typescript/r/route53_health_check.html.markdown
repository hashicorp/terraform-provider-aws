...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    const example = new CloudwatchMetricAlarm(this, "example", {
      alarmDescription: "This metric monitors ec2 cpu utilization",
      alarmName: "example",
      comparisonOperator: "GreaterThanOrEqualToThreshold",
      evaluationPeriods: Token.asNumber("2"),
    ...
    const awsRoute53HealthCheckExample = new Route53HealthCheck(
      ...
      "example_1",
      {
        cloudwatchAlarmName: example.alarmName,
        cloudwatchAlarmRegion: "us-west-2",
        insufficientDataHealthStatus: "Healthy",
        triggers: {
          threshold: Token.asString(example.threshold),
        },
        type: "CLOUDWATCH_METRIC",
      }
    );
...
~> **Note:** At least one of either `fqdn` or `ipAddress` must be specified for endpoint checks.
...
* `cloudwatchAlarmRegion` - (Optional) The region that the CloudWatch alarm was created in.
...
<!-- cache-key: cdktf-0.20.8 input-b44790de97bcbe061efc53803e76504f7f4d90230870eab41f7118633fed89e3 -->