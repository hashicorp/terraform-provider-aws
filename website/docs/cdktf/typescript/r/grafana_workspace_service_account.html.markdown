 ...
 */
import { GrafanaWorkspaceServiceAccount } from "./.gen/providers/aws/";
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Managed Grafana Workspace Service Account using the `workspaceId` and `service_account_id` separated by a comma (`,`). For example:
...
Using `terraform import`, import Managed Grafana Workspace Service Account using the `workspaceId` and `service_account_id` separated by a comma (`,`). For example:
