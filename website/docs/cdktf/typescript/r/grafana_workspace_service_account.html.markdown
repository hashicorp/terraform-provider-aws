 ...
 */
import { GrafanaWorkspaceServiceAccount } from "./.gen/providers/aws/grafana-workspace-service-account";
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    new GrafanaWorkspaceServiceAccount(this, "example", {
      grafanaRole: "ADMIN",
      name: "example-admin",
      workspaceId: Token.asString(awsGrafanaWorkspaceExample.id),
    });
  }
}
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import Managed Grafana Workspace Service Account using the `workspaceId` and `serviceAccountId` separated by a comma (`,`). For example:
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    GrafanaWorkspaceServiceAccount.generateConfigForImport(
      this,
      "example",
      "g-abc12345,1"
    );
...
Using `terraform import`, import Managed Grafana Workspace Service Account using the `workspaceId` and `serviceAccountId` separated by a comma (`,`). For example:
