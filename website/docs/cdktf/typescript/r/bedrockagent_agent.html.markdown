...
* `agentName` - (Required) Name for the agent.
* `agentResourceRoleArn` - (Required) ARN of the Role for the agent.
* `foundationModel` - (Required) Foundation model for the agent to use.

The following arguments are optional:

* `customerEncryptionKeyArn` - (Optional) ARN of customer manager key to use for encryption.
...
* `prepareAgent` (Optional) Whether or not to prepare the agent after creation or modification. Defaults to `true`.
* `promptOverrideConfiguration` (Optional) Prompt override configuration.
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    BedrockagentAgent.generateConfigForImport(
      this,
      "example",
      "agent-abcd1234"
    );
...
<!-- cache-key: cdktf-0.20.1 input-891f4f0bea0fd3d0c331fb6de12463e66a008ddf763086062623a115ac7f0ba2 -->