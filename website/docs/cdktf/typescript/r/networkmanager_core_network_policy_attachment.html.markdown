...
- Option 1: Use the `basePolicyDocument` argument in the [`aws_networkmanager_core_network` resource](/docs/providers/aws/r/networkmanager_core_network.html) that allows the most customizations to a base policy. Use this to customize the `edgeLocations` `asn`. In the example below, `us-west-2` and ASN `65500` are used in the base policy.
...
- Option 2: Pass a list of regions to the [`aws_networkmanager_core_network` resource](/docs/providers/aws/r/networkmanager_core_network.html) `basePolicyRegions` argument. In the example below, `us-west-2` and `us-east-1` are specified in the base policy.
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    NetworkmanagerCoreNetworkPolicyAttachment.generateConfigForImport(
      this,
      "example",
...
<!-- cache-key: cdktf-0.20.1 input-34a808698d2a9df593aff61b1eaca0c1844cef8687a30e103f1a946e027e2540 -->