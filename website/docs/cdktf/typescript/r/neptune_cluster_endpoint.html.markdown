...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import `awsNeptuneClusterEndpoint` using the `clusterIdentifier:endpointIdentfier`. For example:
...
Using `terraform import`, import `awsNeptuneClusterEndpoint` using the `clusterIdentifier:endpointIdentfier`. For example:
