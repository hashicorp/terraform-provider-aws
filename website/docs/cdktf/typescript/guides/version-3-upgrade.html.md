...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    new TerraformOutput(this, "lambda_result", {
      value: propertyAccess(example.resultMap, ['"key1"']),
    });
  }
}
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    super(scope, name);
    new TerraformOutput(this, "lambda_result", {
      value: propertyAccess(Fn.jsondecode(Token.asString(example.result)), [
        '"key1"',
      ]),
    });
  }
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    const awsRoute53RecordExisting = new Route53Record(this, "existing_2", {
      ...
      name: Token.asString(
        propertyAccess(
          propertyAccess(existing.domainValidationOptions, [
            existingCount.index,
          ]),
    ...
    const awsAcmCertificateValidationExisting = new AcmCertificateValidation(
      ...
      "existing_3",
      {
        ...
        validationRecordFqdns: Token.asList(
          propertyAccess(awsRoute53RecordExisting, ["*", "fqdn"])
        ),
      }
    );
    /*This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.*/
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    const existing = new Route53Record(this, "existing", {
      allowOverwrite: true,
      name: Token.asString(
        propertyAccess(existingForEachIterator.value, ["name"])
      ),
      records: [
        Token.asString(
          propertyAccess(existingForEachIterator.value, ["record"])
        ),
      ],
      ttl: 60,
      type: Token.asString(
        propertyAccess(existingForEachIterator.value, ["type"])
      ),
...
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name: string) {
    ...
    const existing1 = new Route53Record(this, "existing_1", {
      ...
      name: Token.asString(
        propertyAccess(
          propertyAccess(existingDomainValidationOptions, [
            "existing1.${" + publicRootDomain.value + "}",
          ]),
          ["name"]
        )
