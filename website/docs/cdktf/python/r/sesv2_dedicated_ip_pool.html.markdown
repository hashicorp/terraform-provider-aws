...
#
from imports.aws.sesv2_dedicated_ip_pool import Sesv2DedicatedIpPool
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        Sesv2DedicatedIpPool(self, "example",
            pool_name="my-pool"
        )
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        Sesv2DedicatedIpPool.generate_config_for_import(self, "example", "my-pool")
```
