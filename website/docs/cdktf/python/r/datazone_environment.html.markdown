...
#
from imports.aws.datazone_environment import DatazoneEnvironment
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        DatazoneEnvironment(self, "example",
            account_identifier=Token.as_string(test.account_id),
            account_region=Token.as_string(data_aws_region_test.name),
            blueprint_identifier=Token.as_string(aws_datazone_environment_blueprint_configuration_test.environment_blueprint_id),
            domain_identifier=Token.as_string(aws_datazone_domain_test.id),
            name="example",
            profile_identifier=Token.as_string(aws_datazone_environment_profile_test.id),
            project_identifier=Token.as_string(aws_datazone_project_test.id),
            user_parameters=[DatazoneEnvironmentUserParameters(
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        DatazoneEnvironment.generate_config_for_import(self, "example", "dzd_d2i7tzk3tnjjf4,5vpywijpwryec0")
```
