...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        aws_s3_object_example = S3Object(self, "example_5",
        ...
        aws_iam_policy_example = IamPolicy(self, "example_6",
            ...
            policy=Token.as_string(
                Fn.jsonencode({
                    "Statement": [{
                        "Action": ["s3:GetObject"],
                        "Effect": "Allow",
                        "Resource": "${" + example.arn + "}/${" + aws_s3_object_example.key + "}"
                    }, {
        ...
        aws_iam_role_policy_attachment_example = IamRolePolicyAttachment(self, "example_7",
        ...
        aws_quicksight_data_source_example = QuicksightDataSource(self, "example_8",
            data_source_id="example-id",
            name="manifest in S3",
            parameters=QuicksightDataSourceParameters(
                s3=QuicksightDataSourceParametersS3(
        ...
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_quicksight_data_source_example.override_logical_id("example")
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        QuicksightDataSource.generate_config_for_import(self, "example", "123456789123/my-data-source-id")
```
