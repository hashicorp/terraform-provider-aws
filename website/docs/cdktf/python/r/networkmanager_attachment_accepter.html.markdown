...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        NetworkmanagerAttachmentAccepter(self, "example",
            attachment_id=Token.as_string(aws_networkmanager_vpc_attachment_example.id),
            attachment_type=Token.as_string(aws_networkmanager_vpc_attachment_example.attachment_type)
        )
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        NetworkmanagerAttachmentAccepter(self, "example",
            attachment_id=Token.as_string(aws_networkmanager_site_to_site_vpn_attachment_example.id),
            attachment_type=Token.as_string(aws_networkmanager_site_to_site_vpn_attachment_example.attachment_type)
        )
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        NetworkmanagerAttachmentAccepter(self, "example",
            attachment_id=Token.as_string(aws_networkmanager_transit_gateway_route_table_attachment_example.id),
            attachment_type=Token.as_string(aws_networkmanager_transit_gateway_route_table_attachment_example.attachment_type)
        )
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        NetworkmanagerAttachmentAccepter(self, "example",
            attachment_id=Token.as_string(aws_networkmanager_dx_gateway_attachment_example.id),
            attachment_type=Token.as_string(aws_networkmanager_dx_gateway_attachment_example.attachment_type)
        )
```
