...
# See https://cdk.tf/provider-generation for more details.
#
from imports.aws.data_aws_odb_db_servers import DataAwsOdbDbServers
from imports.aws.odb_cloud_vm_cluster import OdbCloudVmCluster
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        example = DataAwsOdbDbServers(self, "example",
            cloud_exadata_infrastructure_id="exa_gjrmtxl4qk"
        )
        aws_odb_cloud_vm_cluster_example = OdbCloudVmCluster(self, "example_1",
            cloud_exadata_infrastructure_id=example.cloud_exadata_infrastructure_id,
            cpu_core_count=6,
            data_collection_options=[OdbCloudVmClusterDataCollectionOptions(
                is_diagnostics_events_enabled=False,
                is_health_monitoring_enabled=False,
            ...
            data_storage_size_in_tbs=20,
            db_node_storage_size_in_gbs=120,
            # Use the id attribute from each db_server to get the DB Server IDs (format: dbs_*)
            db_servers=[db_server.id for db_server in example.db_servers],
            display_name="my-exa-infra",
            gi_version="23.0.0.0",
            hostname_prefix="apollo12",
            is_local_backup_enabled=True,
        ...
        aws_odb_cloud_vm_cluster_example.override_logical_id("example")
```
