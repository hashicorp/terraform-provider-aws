...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        S3Object.generate_config_for_import(self, "example", "some-bucket-name/some/key.txt")
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        S3Object.generate_config_for_import(self, "example", "s3://some-bucket-name/some/key.txt")
```
...
<!-- cache-key: cdktf-0.20.8 input-f446cf85a73cf08f0000f5b47dece2c66fc39124030c70d4be8470c298a7acf9 -->