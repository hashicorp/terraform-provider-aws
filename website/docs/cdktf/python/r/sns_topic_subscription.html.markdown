...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        SnsTopicSubscription(self, "user_updates_sqs_target",
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        user_updates = SnsTopic(self, "user_updates",
            name="user-updates-topic"
        )
        user_updates_queue = SqsQueue(self, "user_updates_queue",
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        sns = TerraformVariable(self, "sns",
            default=[{
        ...
        sqs = TerraformVariable(self, "sqs",
            default=[{
        ...
        sns_topic_policy = DataAwsIamPolicyDocument(self, "sns-topic-policy",
            ...
            statement=[DataAwsIamPolicyDocumentStatement(
                ...
                condition=[DataAwsIamPolicyDocumentStatementCondition(
                    test="StringEquals",
                    values=[
                        Token.as_string(Fn.lookup_nested(sns.value, ["\"account-id\""]))
                    ],
                ...
                resources=["arn:aws:sns:" +
                    Token.as_string(Fn.lookup_nested(sns.value, ["\"region\""])) + ":" +
                    Token.as_string(Fn.lookup_nested(sns.value, ["\"account-id\""])) + ":" +
                    Token.as_string(Fn.lookup_nested(sns.value, ["\"name\""]))
                ],
                ...
                condition=[DataAwsIamPolicyDocumentStatementCondition(
                    test="StringLike",
                    values=["arn:aws:sqs:" +
                        Token.as_string(Fn.lookup_nested(sqs.value, ["\"region\""])) + ":" +
                        Token.as_string(
                            Fn.lookup_nested(sqs.value, ["\"account-id\""])) + ":" +
                        Token.as_string(Fn.lookup_nested(sqs.value, ["\"name\""]))
                    ],
        ...
        sqs_queue_policy = DataAwsIamPolicyDocument(self, "sqs-queue-policy",
            policy_id="arn:aws:sqs:" +
            Token.as_string(Fn.lookup_nested(sqs.value, ["\"region\""])) + ":" +
            Token.as_string(Fn.lookup_nested(sqs.value, ["\"account-id\""])) + ":" +
            Token.as_string(Fn.lookup_nested(sqs.value, ["\"name\""])) + "/SQSDefaultPolicy",
            statement=[DataAwsIamPolicyDocumentStatement(
                ...
                condition=[DataAwsIamPolicyDocumentStatementCondition(
                    ...
                    values=["arn:aws:sns:" +
                        Token.as_string(Fn.lookup_nested(sns.value, ["\"region\""])) + ":" +
                        Token.as_string(
                            Fn.lookup_nested(sns.value, ["\"account-id\""])) + ":" +
                        Token.as_string(Fn.lookup_nested(sns.value, ["\"name\""]))
                    ],
        ...
        aws_sns = AwsProvider(self, "aws",
            alias="sns",
            assume_role=[AwsProviderAssumeRole(
                role_arn="arn:aws:iam::" +
                Token.as_string(Fn.lookup_nested(sns.value, ["\"account-id\""])) + ":role/" +
                Token.as_string(Fn.lookup_nested(sns.value, ["\"role-name\""])),
                session_name="sns-" + Token.as_string(Fn.lookup_nested(sns.value, ["\"region\""]))
            )
            ],
            region=Token.as_string(Fn.lookup_nested(sns.value, ["\"region\""]))
        )
        aws_sqs = AwsProvider(self, "aws_5",
            alias="sqs",
            assume_role=[AwsProviderAssumeRole(
                role_arn="arn:aws:iam::" +
                Token.as_string(Fn.lookup_nested(sqs.value, ["\"account-id\""])) + ":role/" +
                Token.as_string(Fn.lookup_nested(sqs.value, ["\"role-name\""])),
                session_name="sqs-" + Token.as_string(Fn.lookup_nested(sqs.value, ["\"region\""]))
            )
            ],
            region=Token.as_string(Fn.lookup_nested(sqs.value, ["\"region\""]))
        )
        sns2_sqs = AwsProvider(self, "aws_6",
            alias="sns2sqs",
            assume_role=[AwsProviderAssumeRole(
                role_arn="arn:aws:iam::" +
                Token.as_string(Fn.lookup_nested(sqs.value, ["\"account-id\""])) + ":role/" +
        ...
        sns_topic = SnsTopic(self, "sns-topic",
            display_name=Token.as_string(
                Fn.lookup_nested(sns.value, ["\"display_name\""])),
            name=Token.as_string(Fn.lookup_nested(sns.value, ["\"name\""])),
            policy=Token.as_string(sns_topic_policy.json),
            provider="${aws.sns}"
        )
        sqs_queue = SqsQueue(self, "sqs-queue",
            name=Token.as_string(Fn.lookup_nested(sqs.value, ["\"name\""])),
            policy=Token.as_string(sqs_queue_policy.json),
            provider="${aws.sqs}"
        )
        aws_sns_topic_subscription_sns_topic = SnsTopicSubscription(self, "sns-topic_9",
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        SnsTopicSubscription(self, "example_with_delivery_policy",
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        SnsTopicSubscription.generate_config_for_import(self, "userUpdatesSqsTarget", "arn:aws:sns:us-west-2:123456789012:my-topic:8a21d249-4329-4871-acc6-7be709c6ea7f")
```
