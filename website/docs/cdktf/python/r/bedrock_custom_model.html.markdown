...
#
from imports.aws.bedrock_custom_model import BedrockCustomModel
from imports.aws.data_aws_bedrock_foundation_model import DataAwsBedrockFoundationModel
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        example = DataAwsBedrockFoundationModel(self, "example",
            model_id="amazon.titan-text-express-v1"
        )
        aws_bedrock_custom_model_example = BedrockCustomModel(self, "example_1",
            base_model_identifier=Token.as_string(example.model_arn),
            ...
            output_data_config=[BedrockCustomModelOutputDataConfig(
                s3_uri="s3://${" + output.id + "}/data/"
            )
            ],
            role_arn=Token.as_string(aws_iam_role_example.arn),
            training_data_config=[BedrockCustomModelTrainingDataConfig(
                s3_uri="s3://${" + training.id + "}/data/train.jsonl"
            )
            ]
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_bedrock_custom_model_example.override_logical_id("example")
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        BedrockCustomModel.generate_config_for_import(self, "example", "arn:aws:bedrock:us-west-2:123456789012:model-customization-job/amazon.titan-text-express-v1:0:8k/1y5n57gh5y2e")
```
