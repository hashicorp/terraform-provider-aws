...
#
from imports.aws.codebuild_webhook import CodebuildWebhook
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        CodebuildWebhook(self, "example",
            build_type="BUILD",
            filter_group=[CodebuildWebhookFilterGroup(
                filter=[CodebuildWebhookFilterGroupFilter(
                    pattern="PUSH",
                    type="EVENT"
            ...
            project_name=Token.as_string(aws_codebuild_project_example.name)
        )
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        example = CodebuildWebhook(self, "example",
            project_name=Token.as_string(aws_codebuild_project_example.name)
        )
        github_repository_webhook_example = RepositoryWebhook(self, "example_1",
            active=True,
            configuration=[{
                "content_type": "json",
                "insecure_ssl": False,
        ...
        github_repository_webhook_example.override_logical_id("example")
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        CodebuildWebhook.generate_config_for_import(self, "example", "MyProjectName")
```
