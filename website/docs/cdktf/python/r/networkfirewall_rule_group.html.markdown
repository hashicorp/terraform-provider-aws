...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        suricata_rules = DataAwsS3Object(self, "suricata_rules",
            bucket=Token.as_string(aws_s3_bucket_suricata_rules.id),
            key="rules/custom.rules"
        )
        NetworkfirewallRuleGroup(self, "s3_rules_example",
            capacity=1000,
            ...
            rule_group=NetworkfirewallRuleGroupRuleGroup(
                ...
                rules_source=NetworkfirewallRuleGroupRuleGroupRulesSource(
                    rules_string=Token.as_string(suricata_rules.body)
                )
            ),
            tags={
                "ManagedBy": "terraform"
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        NetworkfirewallRuleGroup.generate_config_for_import(self, "example", "arn:aws:network-firewall:us-west-1:123456789012:stateful-rulegroup/example")
```
