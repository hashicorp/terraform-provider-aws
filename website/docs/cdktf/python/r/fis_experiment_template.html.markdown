...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        report_access = DataAwsIamPolicyDocument(self, "report_access",
        ...
        aws_iam_policy_report_access = IamPolicy(self, "report_access_2",
            name="report_access",
            policy=Token.as_string(report_access.json)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_iam_policy_report_access.override_logical_id("report_access")
            ...
            assume_role_policy=Token.as_string(
                Fn.jsonencode({
                    "Statement": [{
                        ...
                        "Effect": "Allow",
                        "Principal": {
                            "Service": ["fis.${" + current.dns_suffix + "}"]
                        }
                    }
                    ],
                    "Version": "2012-10-17"
        ...
        aws_iam_role_policy_attachment_report_access = IamRolePolicyAttachment(self, "report_access_4",
            policy_arn=Token.as_string(aws_iam_policy_report_access.arn),
            role=test.name
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_iam_role_policy_attachment_report_access.override_logical_id("report_access")
        aws_fis_experiment_template_example = FisExperimentTemplate(self, "example_5",
            action=[FisExperimentTemplateAction(
                action_id="aws:ec2:terminate-instances",
                name="example-action",
                target=FisExperimentTemplateActionTarget(
            ...
            experiment_report_configuration=FisExperimentTemplateExperimentReportConfiguration(
                data_sources=FisExperimentTemplateExperimentReportConfigurationDataSources(
                    cloudwatch_dashboard=[FisExperimentTemplateExperimentReportConfigurationDataSourcesCloudwatchDashboard(
                        dashboard_arn=Token.as_string(aws_cloudwatch_dashboard_example.dashboard_arn)
                    )
                    ]
                ),
        ...
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_fis_experiment_template_example.override_logical_id("example")
```
...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        FisExperimentTemplate.generate_config_for_import(self, "template", "EXT123AbCdEfGhIjK")
```
