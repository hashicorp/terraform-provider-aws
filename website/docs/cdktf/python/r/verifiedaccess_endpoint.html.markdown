    ...
    def __init__(self, scope, name):
        ...
        VerifiedaccessEndpoint(self, "example",
            ...
            cidr_options=VerifiedaccessEndpointCidrOptions(
                cidr=Token.as_string(Fn.lookup_nested(test, ["0", "cidr_block"])),
                port_range=[VerifiedaccessEndpointCidrOptionsPortRange(
                    from_port=443,
                    to_port=443
                )
                ],
                protocol="tcp",
                subnet_ids=Token.as_list("${[ for subnet in ${" + test.fqn + "} : subnet.id]}")
            ),
            description="example",
            endpoint_type="cidr",
            security_group_ids=[Token.as_string(aws_security_group_test.id)],
            verified_access_group_id=Token.as_string(aws_verifiedaccess_group_test.id)
        )
```
