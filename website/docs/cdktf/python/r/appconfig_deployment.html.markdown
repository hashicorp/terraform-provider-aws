...
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        ...
        AppconfigDeployment(self, "example",
            ...
            kms_key_identifier=Token.as_string(aws_kms_key.example.arn),
            tags={
...
* `kms_key_identifier` - (Optional, Forces new resource) The KMS key identifier (key ID, key alias, or key ARN). Appconfig uses this ID to encrypt the configuration data using a customer managed key.
