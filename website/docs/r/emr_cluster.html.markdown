...
* `session_enabled` - (Optional, Forces new resource) Whether Spark Connect sessions are enabled on the cluster. Default value is `false`. Requires `release_label` 8.0.0 or greater. See the [Spark Connect documentation](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-spark-connect.html) for more details.
