...
* `customer_managed_key_arn` - (Optional) ARN of the AWS Key Management Service (AWS KMS) key to use to encrypt the application data. If not specified, an AWS owned key is used for encryption.
