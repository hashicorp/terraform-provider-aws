...
* `enable_prefix_ipv6_source_nat` - (Optional) Indicates whether to use an IPv6 prefix from each subnet for source NAT. The IP address type must be `dualstack`. The default value is `false`. Changing the value from `true` to `false` will cause resource recreation.
