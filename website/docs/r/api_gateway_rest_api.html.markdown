...
* `fail_on_warnings` - (Optional) Failure mode of the PutRestApi operation when importing an OpenAPI specification via the body argument (create or update operation). Indicates whether to rollback the API update (true) or not (false) when a warning is encountered. Defaults to `false`.
