...
* `security_policy` - (Optional) TLS version + cipher suite for the REST API's default execute-api endpoint. Must be configured for drift detection. Refer to the [Amazon API Gateway API Reference](https://docs.aws.amazon.com/apigateway/latest/api/API_CreateRestApi.html) for a list of valid security policies.
