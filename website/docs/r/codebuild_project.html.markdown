...
* `build_status_config` - (Optional) Information that defines how the build project reports the build status to the source provider.
...
* `context` - (Optional) Context of the build status CodeBuild sends to the source provider.
* `target_url` - (Optional) Target url of the build status CodeBuild sends to the source provider.
