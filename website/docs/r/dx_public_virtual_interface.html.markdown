...
* `bgp_asn_long` - (Optional) BGP autonomous system number as an asplain decimal string between `1` and `4294967294`. This argument also accepts values in the `bgp_asn` range. Exactly one of `bgp_asn_long` or `bgp_asn` must be specified.
