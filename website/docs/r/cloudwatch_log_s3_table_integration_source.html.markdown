...
The `id` in the import ID is a UUID generated by AWS per source association. To find it for an existing association created outside Terraform, use the CloudWatch Logs API:

```console
% aws logs list-sources-for-s3-table-integration \
    --integration-arn arn:aws:observabilityadmin:REGION:ACCOUNT_ID:s3tableintegration/YOUR-INTEGRATION-ID
```

Each entry in the response contains an `Identifier` field - that is the `id` value to use in the import.
