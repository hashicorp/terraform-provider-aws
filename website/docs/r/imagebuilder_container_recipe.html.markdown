...
* `platform_override` - (Optional) The platform override is used when referencing a ECR image instead of an Image Builder image. Valid values are `Linux` and `Windows`.
