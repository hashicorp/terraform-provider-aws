...
```hcl
resource "aws_efs_file_system" "foo_with_backup_policy" {
  creation_token = "my-product"
...
* `backup_policy` - (Optional) A file system [backup policy](https://docs.aws.amazon.com/efs/latest/ug/API_BackupPolicy.html) object (documented below).
...
For **back_policy** the following attributes are supported:

* `status` - (Optional, Default: `DISABLED`) Indicates whether automatic daily backup is enabled. Valid values: `ENABLED` or `DISABLED`.
