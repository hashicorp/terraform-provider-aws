...
* `encrypted` - (Optional) Whether the file system will be encrypted at rest. Defaults to `true`. AWS encrypts all new file systems regardless of this value. Encryption at rest [cannot be changed after the file system is created](https://docs.aws.amazon.com/efs/latest/ug/encryption-at-rest.html). Changing an existing unencrypted file system from `false` to `true` forces replacement; changing `true` to `false` is ignored.
* `kms_key_id` - (Optional) The ARN for the KMS encryption key. If `encrypted` is explicitly set to `false`, `kms_key_id` cannot be specified.
