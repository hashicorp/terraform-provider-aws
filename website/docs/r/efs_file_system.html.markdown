...
* `encrypted` - (Optional) Whether the file system will be encrypted at rest. Defaults to `true`. Encryption at rest [cannot be changed after the file system is created](https://docs.aws.amazon.com/efs/latest/ug/encryption-at-rest.html), so changing this argument forces replacement of the file system.
