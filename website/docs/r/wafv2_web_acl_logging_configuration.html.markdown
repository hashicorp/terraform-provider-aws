...
-> **Note:** To start logging from a WAFv2 Web ACL, an [Amazon Kinesis Data Firehose](http://localhost:4567/docs/providers/aws/r/kinesis_firehose_delivery_stream.html) must also be created with a PUT source (not a stream) and in the region that you are operating.
...
* `log_destination_configs` - (Required) The Amazon Kinesis Data Firehose Amazon Resource Name (ARNs) that you want to associate with the web ACL. Up to 100 ARNs are supported. 
