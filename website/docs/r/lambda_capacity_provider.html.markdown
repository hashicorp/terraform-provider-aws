...
* `capacity_provider_operator_role_arn` - (Required) ARN of the IAM role that allows Lambda to manage the Capacity Provider.
...
* `predefined_metric_type` - (Required) Predefined metric type for the scaling policy. Valid values are `"LambdaCapacityProviderAverageCPUUtilization"`.
* `target_value` - (Required) Target value for the scaling policy.
