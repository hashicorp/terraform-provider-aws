...
* `retention_actual` - Effective retention policy applied, including the default (`SIX_MONTHS`) when no `retention` block is configured. Contains a single object with attribute `retention_period`.
