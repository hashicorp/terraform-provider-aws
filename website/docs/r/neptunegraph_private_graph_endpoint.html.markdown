...
* `vpc_id` - (Required) The VPC in which the private graph endpoint needs to be created.
...
* `id` - Uniquely identifies the resource. Same as `private_graph_endpoint_identifier`.
* `private_graph_endpoint_identifier` - PrivateGraphEndpoint resource identifier generated by concatenating the associated GraphIdentifier and VpcId with an underscore separator.
