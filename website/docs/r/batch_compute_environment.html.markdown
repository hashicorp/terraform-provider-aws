...
* `instance_type` - (Optional) A list of instance types that may be launched. This parameter isn't applicable to jobs running on Fargate resources, and shouldn't be specified. When `allocation_strategy` is `BEST_FIT_PROGRESSIVE_ORDERED` or `SPOT_CAPACITY_OPTIMIZED_PRIORITIZED`, the order of this list sets instance type priority and is significant. Under the other allocation strategies the order is ignored, and re-ordering the list alone produces no diff.
