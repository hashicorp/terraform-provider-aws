...
* `track_latest` - (Optional) Whether should track latest `ACTIVE` task definition on AWS or the one created with the resource stored in state. Default is `false`. Useful to prevent drift in event the task definition is modified outside of the resource.
