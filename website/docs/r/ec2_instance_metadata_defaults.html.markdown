...
* `http_tokens_enforced` - (Optional) Whether to enforce the requirement of IMDSv2 on an instance at the time of launch. Can be `"enabled"`, `"disabled"`, or `"no-preference"`. Default: `"no-preference"`.
