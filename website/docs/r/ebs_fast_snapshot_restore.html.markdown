...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import EC2 EBS Fast Snapshot Restore using the `availability_zone` and `snapshot_id` separated by `,`. For example:
...
Using `terraform import`, import EC2 EBS Fast Snapshot Restore using the `availability_zone` and `snapshot_id` separated by `,`. For example:
