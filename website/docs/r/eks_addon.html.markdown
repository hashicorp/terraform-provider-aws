...
* `incremental_version_update` (Optional) – When set to `true`, validates that the `addon_version` is upgraded incrementally, one minor version at a time. This aligns with [AWS best practices for managing EKS addons](https://docs.aws.amazon.com/eks/latest/userguide/workloads-add-ons-available-eks.html), which recommend upgrading addons such as `vpc-cni` or `eks-pod-identity-agent` one version at a time to ensure cluster stability. If the target version is more than one minor version ahead of the current version, the plan will fail with an error. Default is `false`.
