...
* `notification_arn` - (Optional) SNS topic ARNs to which to publish stack-related events 
...
* `provisioning_preferences` - (Optional) An object that contains information about the provisioning preferences for a stack set.
...
* `outputs` - Information about the product created as the result of a request. For example, the output for a CloudFormation-backed product that creates an S3 bucket may include the S3 bucket URL. This is a list of objects containing `Description`, `OutputKey`, and `OutputValue`. 
