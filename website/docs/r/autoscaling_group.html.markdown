...
- `preferences` - (Optional) Override default parameters for Instance Refresh.
    ...
    - `auto_rollback` - (Optional) Automatically rollback if instance refresh fails. Defaults to `false`. Must be using launch_template to enable this option. This is not supported for launch_configuration.
