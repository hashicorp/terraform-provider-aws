...
S3 bucket request payment configuration can be imported using the `bucket` or using the `bucket` and `expected_bucket_owner` separated by a comma (`,`). For example:
