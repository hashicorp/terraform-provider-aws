...
* `data_source` - (Required) Geospatial data provider for the new place index.
...
* `description` - (Optional) Optional description for the place index resource.
...
* `intended_use` - (Optional) How the results of an operation will be stored by the caller. Valid values: `SingleUse`, `Storage`. Default: `SingleUse`.
