...
* `reset_state_on_crud_update_failure` – (Optional) Whether to reset the state to its pre-apply version when an update operation with CRUD lifecycle scope fails. If set to `false` (the default), the state is updated with the new configuration even if the update fails. If set to `true`, the state is not updated and remains as it was before the apply operation.
