...
- `client_id_list` - (Optional) List of client IDs (audiences) that identify the application registered with the OpenID Connect provider. This is the value sent as the `client_id` parameter in OAuth requests. The usage of this resource conflicts with the `aws_iam_openid_connect_provider_client_id` resource.
