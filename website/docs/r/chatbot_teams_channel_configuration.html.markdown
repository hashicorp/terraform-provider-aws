...
import {
  ...
  id = "arn:aws:chatbot::123456789012:chat-configuration/microsoft-teams-channel/example-configuration"
}
```
...
```console
% terraform import aws_chatbot_teams_channel_configuration.example arn:aws:chatbot::123456789012:chat-configuration/microsoft-teams-channel/example-configuration
```

~> **NOTE:** Importing by `team_id` is still supported for backward compatibility, but only works when there is a single channel configuration for the team. Using `chat_configuration_arn` is recommended.
