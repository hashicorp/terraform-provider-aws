...
* `role_arn` - (Required) A service role ARN that grants AWS CodePipeline permission to make calls to AWS services on your behalf.
...
* `encryption_key` - (Optional) The encryption key block AWS CodePipeline uses to encrypt the data in the artifact store, such as a KMS key. If you don't specify a key, AWS CodePipeline uses the default key for S3. An `encryption_key` block is documented below.
