...
* `ignore_default_tags` - (Optional) Skips the application of `provider`-level `default_tags`. Useful when `default_tags` exceed the [10 tag limit](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-tagging.html). Default is `false`. If `true`, only values from `tags` will be applied.
