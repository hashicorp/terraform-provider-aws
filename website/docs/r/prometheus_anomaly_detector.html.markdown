...
* `example_id_arg` - ID argument of the Anomaly Detector.
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import AMP (Managed Prometheus) Anomaly Detector using the `example_id_arg`. For example:

```terraform
import {
  to = aws_prometheus_anomaly_detector.example
  id = "anomaly_detector-id-12345678"
}
```

Using `terraform import`, import AMP (Managed Prometheus) Anomaly Detector using the `example_id_arg`. For example:

```console
% terraform import aws_prometheus_anomaly_detector.example anomaly_detector-id-12345678
```
