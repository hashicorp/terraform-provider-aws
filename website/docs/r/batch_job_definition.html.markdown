...
* `timeout` - (Optional) Specifies the timeout duration for jobs that are submitted with this job definition.
    `timeout` can be spcified only once.  Defined below.
...
* `attempt_duration_seconds` - (Optional) The time duration in seconds (measured from the job attempt's startedAt timestamp) after which AWS Batch terminates the job if it has not finished. The minimum value for the timeout is `60` seconds.
