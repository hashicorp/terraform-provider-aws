...
* `kms_key_id` - (Optional) The ARN of KMS Customer master keys. Amazon Cognito uses the key to encrypt codes and temporary passwords sent to CustomEmailSender and CustomSMSSender.
...
* `lambda_arn` - (Required) The Lambda ARN of the Lambda function that Amazon Cognito triggers to send email notifications to users.
...
* `lambda_arn` - (Required) The Lambda ARN of the Lambda function that Amazon Cognito triggers to send SMS notifications to users.
...
* `lambda_arn` - (Required) The Lambda ARN of the Lambda function that Amazon Cognito triggers to customize access tokens. If you also set an ARN in `pre_token_generation`, its value must be identical to this one.
