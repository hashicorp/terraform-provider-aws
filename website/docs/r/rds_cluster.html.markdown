...
* `seconds_before_timeout` - (Optional) The time, in seconds, that Aurora DB cluster tries to find a scaling point to perform seamless scaling before enforcing the timeout action. Valid values are `10` through `600`. Defaults to `300`.
