...
Use this resource to create and manage message brokers for ActiveMQ and RabbitMQ engines. This resource also manages users for the broker.
...
* `apply_immediately` - (Optional) Whether to apply broker modifications immediately, or during the next maintenance window. Default is `false`.
...
* `data_replication_primary_broker_arn` - (Optional) Amazon Resource Name (ARN) of the primary broker that is used to replicate data from in a data replication pair, and is applied to the replica broker. Must be set when `data_replication_mode` is `CRDR`.
...
* `audit` - (Optional) Whether to enable audit logging. Auditing is only possible for `engine_type` of `ActiveMQ`. User management action made using JMX or the ActiveMQ Web Console is logged. Defaults to `false`.
