...
* `example_id_arg` - ID argument of the Keyword.
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import End User Messaging SMS Keyword using the `example_id_arg`. For example:

```terraform
import {
  to = aws_pinpointsmsvoicev2_keyword.example
  id = "keyword-id-12345678"
}
```

Using `terraform import`, import End User Messaging SMS Keyword using the `example_id_arg`. For example:

```console
% terraform import aws_pinpointsmsvoicev2_keyword.example keyword-id-12345678
```
