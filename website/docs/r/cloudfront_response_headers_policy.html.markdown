...
* `origin_override` - (Required) A Boolean value that determines how CloudFront behaves when the response from the origin contains one of the CORS headers that’s also in the policy.
