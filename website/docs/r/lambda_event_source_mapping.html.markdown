...
* `parallelization_factor`: - (Optional) The number of batches to process from each shard concurrently (maximum of 10). Only available for stream sources (DynamoDB and Kinesis).
* `maximum_retry_attempts`: - (Optional) The maximum number of times to retry when the function returns an error (maximum of 10000). Only available for stream sources (DynamoDB and Kinesis).
* `maximum_record_age_in_seconds`: - (Optional) The maximum age of a record that Lambda sends to a function for processing (minimum of 60 and maximum of 604800). Only available for stream sources (DynamoDB and Kinesis).
