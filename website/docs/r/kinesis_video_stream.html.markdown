...
* `kms_key_id` - (Optional) The ID of the KMS key that you want Kinesis Video Streams to use to encrypt stream data. If no key ID is specified, the default, Kinesis Video-managed key (`aws/kinesisvideo`) is used.
...
* `arn` - The ARN specifying the Stream (same as `id`)
