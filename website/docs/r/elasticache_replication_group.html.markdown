...
* `serverless_cache_snapshot_name` - (Optional) Name of a serverless cache snapshot from which to restore data into the replication group. Available for Valkey and Redis OSS only. Changing `serverless_cache_snapshot_name` forces a new resource. Conflicts with `snapshot_name` and `snapshot_arns`.
