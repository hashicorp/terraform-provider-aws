...
* `auth_token_wo` - (Optional, Write-Only) Password used to access a password protected server, whose value will not be stored in state. Can be specified only if `transit_encryption_enabled = true`. Conflicts with `auth_token`. If set, requires `auth_token_wo_version` to be set.
* `auth_token_wo_version` - (Optional) Required when `auth_token_wo` is set. Changing this value triggers an update to `auth_token_wo`.
...
* `engine_version` - (Optional) Version number of the cache engine to be used for the cache clusters in this replication group.
  ...
  During planning, the provider validates that `engine_version` is available for the configured `engine` in the current Region (using `elasticache:DescribeCacheEngineVersions`) and reports unavailable versions as a plan-time error instead of an apply-time failure. This check is skipped when the API cannot be reached (for example, when planning offline or without the `elasticache:DescribeCacheEngineVersions` permission), in which case the version is validated by AWS at apply time as before.
...
* `security_group_ids` - (Optional) IDs of one or more Amazon VPC security groups associated with this replication group. Use this parameter only when you are creating a replication group in a VPC.
* `security_group_names` - (Optional) Names of one or more Amazon VPC security groups associated with this replication group. Use this parameter only when you are creating a replication group in a VPC.
