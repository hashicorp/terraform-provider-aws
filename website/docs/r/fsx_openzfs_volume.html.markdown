...
* `copy_tags_to_snapshots` - (Optional) Whether tags for the file system should be copied to snapshots. Default is false.
...
* `origin_snapshot` - (Optional) Configuration to use when creating the OpenZFS volume. See [`origin_snapshot` Block](#origin_snapshot-block) below for details.
* `parent_volume_id` - (Required) Volume ID of the volume that will be the parent volume for the volume being created, this could be the root volume created from the `aws_fsx_openzfs_file_system` resource with the `root_volume_id` or the `id` property of another `aws_fsx_openzfs_volume`.
* `read_only` - (Optional) Whether the volume is read-only. Default is false.
* `record_size_kib` - (Optional) Record size of an OpenZFS volume, in kibibytes (KiB). Valid values are `4`, `8`, `16`, `32`, `64`, `128`, `256`, `512`, or `1024` KiB. Default is `128` KiB.
* `region` - (Optional) Region where this resource will be [managed](https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints). Defaults to the Region set in the [provider configuration](https://registry.terraform.io/providers/hashicorp/aws/latest/docs#aws-configuration-reference).
* `storage_capacity_quota_gib`  - (Optional) Maximum amount of storage in gibibytes (GiB) that the volume can use from its parent.
* `storage_capacity_reservation_gib`  - (Optional) Amount of storage in gibibytes (GiB) to reserve from the parent volume.
...
* `volume_type` - (Optional) Volume type. Default is `OPENZFS`.
