...
* `kms_key_identifier` - (Optional) Identifier for a Key Management Service key to encrypt new configuration data versions in the AppConfig hosted configuration store. This attribute is only used for hosted configuration types. Identifier can be a KMS key ID, alias, or the Amazon Resource Name (ARN) of the key ID or alias.
...
The `validator` block supports the following arguments:
