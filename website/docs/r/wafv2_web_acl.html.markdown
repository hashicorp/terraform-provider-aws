...
* `rule_json` (Optional) Raw JSON block to allow more than three rules. Conflicts with `rule` attribute. **NOTE There is no drift detection at this time. If you use this attribute instead of `rule`, you will be foregoing drift detection.
