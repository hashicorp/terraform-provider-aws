...
* `delete_vpc_enis_on_destroy` - (Optional) Whether to delete detached VPC ENIs SageMaker may leave behind when the training job is destroyed. This is provider-side cleanup only and is never sent to the SageMaker API. Default is `false`.
