...
`egress` rules. Neither of these resources works in all scenarios with the [security group rule unique ID](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/security-group-rules.html), so the `aws_vpc_security_group_egress_rule` resource has been added and should be used for all new rules.
