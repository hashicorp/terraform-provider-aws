...
~> **Note:** This resource always call the RDS [`StartActivityStream`][3] API with the `ApplyImmediately` parameter set to `true`. This is because the terraform need the activity stream to be started in order for it to get the associated attributes.
