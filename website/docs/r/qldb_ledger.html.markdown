...
* `deletion_protection` - (Optional) Whether to enable deletion protection for the QLDB Ledger instance. By default it is `true`. To delete this resource via Terraform, this value must be configured to `false` and applied first before attempting deletion.
* `kms_key` - (Optional) Key in KMS to use for encryption of data at rest in the ledger. For more information, see the [AWS documentation](https://docs.aws.amazon.com/qldb/latest/developerguide/encryption-at-rest.html). Valid values are `"AWS_OWNED_KMS_KEY"` to use an AWS KMS key that is owned and managed by AWS on your behalf, or the ARN of a valid symmetric customer managed KMS key.
* `name` - (Optional) Friendly name for the QLDB Ledger instance. By default generated by Terraform.
* `permissions_mode` - (Required) Permissions mode for the QLDB ledger instance. Specify either `ALLOW_ALL` or `STANDARD`.
