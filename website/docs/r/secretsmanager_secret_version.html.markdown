...
* `secret_string_wo_version` - (Required when `secret_string_wo` is set) Version identifier that works together with `secret_string_wo` to trigger an update. Increment this value when an update to `secret_string_wo` is required.
