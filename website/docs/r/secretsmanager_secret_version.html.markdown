...
* `secret_string_wo_version` - (Optional, Required when `secret_string_wo` is set) Version identifier that works together with `secret_string_wo` to trigger an update. Change this value whenever the value of `secret_string_wo` changes, since Terraform cannot detect changes to write-only arguments on its own.
