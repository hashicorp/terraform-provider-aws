...
* `fixed_response` - (Optional) Configuration block for returning a fixed response. See [`fixed_response` Block](#fixed_response-block) below.
* `forward` - (Optional) Route requests to one or more target groups. See [`forward` Block](#forward-block) below.

-> **NOTE:** You must specify exactly one of the following argument blocks: `fixed_response` or `forward`.

### `fixed_response` Block

Fixed response blocks (for `fixed_response`) must include the following argument:
...
* `target_groups` - (Required) One or more target group blocks. See [`target_groups` Block](#target_groups-block) below.

### `target_groups` Block

Target group blocks (for `target_groups`) must include the following arguments:

* `target_group_identifier` - (Optional) ID or ARN of the target group.
...
* `last_updated_at` - Date and time that the listener was last updated, specified in ISO-8601 format.
