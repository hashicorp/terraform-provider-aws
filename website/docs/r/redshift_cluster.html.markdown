...
* `default_iam_role_arn` - (Optional) The ARN for the IAM role that was set as default for the cluster when the cluster was created.
...
* `vpc_security_group_ids` - (Optional) A list of VPC security groups to be associated with the cluster.
* `cluster_subnet_group_name` - (Optional) The name of a cluster subnet group to be associated with this cluster. If this parameter is not provided the resulting cluster will be deployed outside VPC.
