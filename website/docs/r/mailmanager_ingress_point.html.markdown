...
* `example_id_arg` - ID argument of the Ingress Point.
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import SES Mail Manager Ingress Point using the `example_id_arg`. For example:
...
Using `terraform import`, import SES Mail Manager Ingress Point using the `example_id_arg`. For example:
