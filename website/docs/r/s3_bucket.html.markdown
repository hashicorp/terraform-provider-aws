...
* `region` - (Optional) **Deprecated**, use appropriate provider defined in the desired region (you can define [multiple providers](https://www.terraform.io/docs/configuration/providers.html#multiple-provider-instances)). Note that even after removal of this option, it can still be used as an attribute.
