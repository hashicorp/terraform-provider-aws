...
-> Object Lock can be enabled by using the `object_lock_enabled` attribute or by using the [`aws_s3_bucket_object_lock_configuration`](/docs/providers/aws/r/s3_bucket_object_lock_configuration.html) resource.
...
The following arguments marked **Deprecated** will be removed in a future major version.
...
* `cors_rule` - (Optional, **Deprecated**) Rule of [Cross-Origin Resource Sharing](https://docs.aws.amazon.com/AmazonS3/latest/dev/cors.html). See [`cors_rule` Block](#cors_rule-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_cors_configuration`](s3_bucket_cors_configuration.html.markdown) instead.
...
* `grant` - (Optional, **Deprecated**) [ACL policy grant](https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html#sample-acl). See [`grant` Block](#grant-block) below for details. Conflicts with `acl`. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_acl`](s3_bucket_acl.html.markdown) instead.
* `lifecycle_rule` - (Optional, **Deprecated**) Configuration of [object lifecycle management](http://docs.aws.amazon.com/AmazonS3/latest/dev/object-lifecycle-mgmt.html). See [`lifecycle_rule` Block](#lifecycle_rule-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_lifecycle_configuration`](s3_bucket_lifecycle_configuration.html) instead.
* `logging` - (Optional, **Deprecated**) Configuration of [S3 bucket logging](https://docs.aws.amazon.com/AmazonS3/latest/UG/ManagingBucketLogging.html) parameters. See [`logging` Block](#logging-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_logging`](s3_bucket_logging.html.markdown) instead.
* `object_lock_configuration` - (Optional, **Deprecated**) Configuration of [S3 object locking](https://docs.aws.amazon.com/AmazonS3/latest/dev/object-lock.html). See [`object_lock_configuration` Block](#object_lock_configuration-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the `object_lock_enabled` parameter and the resource [`aws_s3_bucket_object_lock_configuration`](s3_bucket_object_lock_configuration.html.markdown) instead.
* `object_lock_enabled` - (Optional) Whether this bucket has an Object Lock configuration enabled. Valid values are `true` or `false`. This argument is not supported in all regions or partitions.
* `policy` - (Optional, **Deprecated**) Valid [bucket policy](https://docs.aws.amazon.com/AmazonS3/latest/dev/example-bucket-policies.html) JSON document. Note that if the policy document is not specific enough (but still valid), Terraform may view the policy as constantly changing in a `terraform plan`. In this case, please make sure you use the verbose/specific version of the policy. For more information about building AWS IAM policy documents with Terraform, see the [AWS IAM Policy Document Guide](https://learn.hashicorp.com/terraform/aws/iam-policy). Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_policy`](s3_bucket_policy.html) instead.
* `region` - (Optional) Region where this resource will be [managed](https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints). Defaults to the Region set in the [provider configuration](https://registry.terraform.io/providers/hashicorp/aws/latest/docs#aws-configuration-reference).
* `replication_configuration` - (Optional, **Deprecated**) Configuration of [replication configuration](http://docs.aws.amazon.com/AmazonS3/latest/dev/crr.html). See [`replication_configuration` Block](#replication_configuration-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_replication_configuration`](s3_bucket_replication_configuration.html) instead.
* `request_payer` - (Optional, **Deprecated**) Who should bear the cost of Amazon S3 data transfer. Can be either `BucketOwner` or `Requester`. By default, the owner of the S3 bucket would incur the costs of any data transfer. See [Requester Pays Buckets](http://docs.aws.amazon.com/AmazonS3/latest/dev/RequesterPaysBuckets.html) developer guide for more information. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_request_payment_configuration`](s3_bucket_request_payment_configuration.html) instead.
* `server_side_encryption_configuration` - (Optional, **Deprecated**) Configuration of [server-side encryption configuration](http://docs.aws.amazon.com/AmazonS3/latest/dev/bucket-encryption.html). See [`server_side_encryption_configuration` Block](#server_side_encryption_configuration-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_server_side_encryption_configuration`](s3_bucket_server_side_encryption_configuration.html) instead.
...
* `versioning` - (Optional, **Deprecated**) Configuration of the [S3 bucket versioning state](https://docs.aws.amazon.com/AmazonS3/latest/dev/Versioning.html). See [`versioning` Block](#versioning-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_versioning`](s3_bucket_versioning.html.markdown) instead.
* `website` - (Optional, **Deprecated**) Configuration of the [S3 bucket website](https://docs.aws.amazon.com/AmazonS3/latest/userguide/WebsiteHosting.html). See [`website` Block](#website-block) below for details. Terraform will only perform drift detection if a configuration value is provided. Use the resource [`aws_s3_bucket_website_configuration`](s3_bucket_website_configuration.html.markdown) instead.
...
* `max_age_seconds` - (Optional) Time in seconds that browser can cache the response for a preflight request.
...
* `abort_incomplete_multipart_upload_days` - (Optional) Number of days after initiating a multipart upload when the multipart upload must be completed.
* `enabled` - (Required) Lifecycle rule status.
* `expiration` - (Optional) Configuration of the object expiration. See [`expiration` Block](#expiration-block) below for details.
* `id` - (Optional) Unique identifier for the rule. Must be less than or equal to 255 characters in length.
* `noncurrent_version_expiration` - (Optional) When noncurrent object versions expire. See [`noncurrent_version_expiration` Block](#noncurrent_version_expiration-block) below for details.
* `noncurrent_version_transition` - (Optional) When noncurrent object versions transition. See [`noncurrent_version_transition` Block](#noncurrent_version_transition-block) below for details.
* `prefix` - (Optional) Object key prefix identifying one or more objects to which the rule applies.
* `tags` - (Optional) Object tags key and value.
* `transition` - (Optional) Configuration of the object transition. See [`transition` Block](#transition-block) below for details.

### `expiration` Block

The `expiration` configuration block supports the following arguments:

* `date` - (Optional) Date after which you want the corresponding action to take effect.
* `days` - (Optional) Number of days after object creation when the specific rule action takes effect.
...
* `object_lock_enabled` - (Optional, **Deprecated**) Whether this bucket has an Object Lock configuration enabled. Valid value is `Enabled`. Use the top-level argument `object_lock_enabled` instead.
* `rule` - (Optional) Object Lock rule in place for this bucket. See [`object_lock_configuration.rule` Block](#object_lock_configurationrule-block) below for details.

#### `object_lock_configuration.rule` Block

The `rule` configuration block supports the following argument:

* `default_retention` - (Required) Default retention period that you want to apply to new objects placed in this bucket. See [`default_retention` Block](#default_retention-block) below for details.
...
* `rules` - (Required) Rules managing the replication. See [`rules` Block](#rules-block) below for details.
...
* `destination` - (Required) Destination for the rule. See [`destination` Block](#destination-block) below for details.
* `filter` - (Optional, Conflicts with `prefix`) Filter that identifies subset of objects to which the replication rule applies. See [`filter` Block](#filter-block) below for details.
* `id` - (Optional) Unique identifier for the rule. Must be less than or equal to 255 characters in length.
...
* `source_selection_criteria` - (Optional) Special object selection criteria. See [`source_selection_criteria` Block](#source_selection_criteria-block) below for details.
...
* `tags` - (Optional) Map of tags that identifies subset of objects to which the rule applies. The rule applies only to objects having all the tags in its tagset.
...
* `access_control_translation` - (Optional) Overrides to use for object owners on replication. See [`access_control_translation` Block](#access_control_translation-block) below for details. Must be used in conjunction with `account_id` owner override configuration.
...
* `metrics` - (Optional) Enables replication metrics (required for S3 RTC). See [`metrics` Block](#metrics-block) below for details.
...
* `replication_time` - (Optional) Enables S3 Replication Time Control (S3 RTC). See [`replication_time` Block](#replication_time-block) below for details.
* `storage_class` - (Optional) [Storage class](https://docs.aws.amazon.com/AmazonS3/latest/API/API_Destination.html#AmazonS3-Type-Destination-StorageClass) used to store the object. By default, Amazon S3 uses the storage class of the source object to create the object replica.

#### `access_control_translation` Block

The `access_control_translation` configuration block supports the following arguments:

* `owner` - (Required) Replica ownership. For default and valid values, see [PUT bucket replication](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutBucketReplication.html) in the Amazon S3 API Reference. The only valid value is `Destination`.
...
* `sse_kms_encrypted_objects` - (Optional) Match SSE-KMS encrypted objects. See [`sse_kms_encrypted_objects` Block](#sse_kms_encrypted_objects-block) below for details. If specified, `replica_kms_key_id` in `destination` must be specified as well.
...
* `rule` - (Required) Single object for server-side encryption by default configuration. See [`server_side_encryption_configuration.rule` Block](#server_side_encryption_configurationrule-block) below for details.

#### `server_side_encryption_configuration.rule` Block

The `rule` configuration block supports the following arguments:

* `apply_server_side_encryption_by_default` - (Required) Single object for setting server-side encryption by default. See [`apply_server_side_encryption_by_default` Block](#apply_server_side_encryption_by_default-block) below for details.
...
* `bucket_regional_domain_name` - Bucket region-specific domain name. The bucket domain name including the region name. Please refer to the [S3 endpoints reference](https://docs.aws.amazon.com/general/latest/gr/s3.html#s3_region) for format. Note: AWS CloudFront allows specifying an S3 region-specific endpoint when creating an S3 origin. This will prevent redirect issues from CloudFront to the S3 Origin URL. For more information, see the [Virtual Hosted-Style Requests for Other Regions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html#deprecated-global-endpoint) section in the AWS S3 User Guide.
