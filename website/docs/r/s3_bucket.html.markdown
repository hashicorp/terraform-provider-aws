...
* `object_lock_enabled` - (Optional, Forces new resource) Whether this bucket has an Object Lock configuration enabled. Valid values are `true` or `false`. This argument is not supported in all regions or partitions.
