...
* `object_ownership` - (Optional) Allows you to disable ACLs and use other policy mechanisms (e.g. IAM, S3 bucket policies) to control access to objects. See [S3 Object Ownership](https://docs.aws.amazon.com/AmazonS3/latest/userguide/about-object-ownership.html). Can be `BucketOwnerPreferred`, `ObjectWriter`, or `BucketOwnerEnforced`.
