...
* `engine_version` - (Optional) The engine version to use. If `auto_minor_version_upgrade` is enabled, you can provide a prefix of the version such as `8.0` (for `8.0.36`). The actual engine version used is returned in the attribute `engine_version_actual`, see [Attribute Reference](#attribute-reference) below. For supported values, see the EngineVersion parameter in [API action CreateDBInstance](https://docs.aws.amazon.com/AmazonRDS/latest/APIReference/API_CreateDBInstance.html). Note that for Amazon Aurora instances the engine version must match the [DB cluster](/docs/providers/aws/r/rds_cluster.html)'s engine version'. When `auto_minor_version_upgrade` is enabled and the configured `engine_version` is not newer than the running version reported in `engine_version_actual` (for example a major-only prefix such as `14` while the instance runs `14.22`, or a version RDS has automatically upgraded past), the difference is suppressed so Terraform does not attempt an engine downgrade. When `auto_minor_version_upgrade` is disabled, `engine_version` is treated as an explicit pin and any difference from the running version is shown. When disabling automatic minor version upgrades after using a version prefix, set `engine_version` to the desired exact running version in the same configuration change to avoid an attempted downgrade.
