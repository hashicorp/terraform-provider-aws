...
* `performance_insights_retention_period` - (Required only if performance_insights_enabled is set to true) The amount of time in days to retain Performance Insights data. Value can range from `7` (for 7 days) to `731` (for 2 years). When specifying `performance_insights_retention_period`, `performance_insights_enabled` must be set to true.
