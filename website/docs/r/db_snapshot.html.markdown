...
* `db_instance_identifier` - (Required) DB instance identifier from which to take the snapshot.
* `db_snapshot_identifier` - (Required) Identifier for the snapshot.
...
* `port` - Port that the DB snapshot listens on.
