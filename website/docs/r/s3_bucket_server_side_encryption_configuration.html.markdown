...
* `rule` - (Required) Set of server-side encryption configuration rules. [See below](#rule-block). Currently, only a single rule is supported.

### `rule` Block

The `rule` configuration block supports the following arguments:

* `apply_server_side_encryption_by_default` - (Optional) Single object for setting server-side encryption by default. [See below](#apply_server_side_encryption_by_default-block).
