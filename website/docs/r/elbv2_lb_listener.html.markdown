...
# Resource: aws_elbv2_lb_listener
...
resource "aws_elbv2_lb_target_group" "front_end" {
  # ...
}

resource "aws_elbv2_lb_listener" "front_end" {
  load_balancer_arn = aws_elbv2_lb.front_end.arn
  port              = "443"
  protocol          = "HTTPS"
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_elbv2_lb_target_group.front_end.arn
  }
}
```
...
resource "aws_elbv2_lb_target_group" "example" {
  name     = "example"
  port     = 6081
  protocol = "GENEVE"
...
resource "aws_elbv2_lb_listener" "example" {
  load_balancer_arn = aws_elbv2_lb.example.id

  default_action {
    target_group_arn = aws_elbv2_lb_target_group.example.id
    type             = "forward"
  }
}
```
...
* `certificate_arn` - (Optional) ARN of the default SSL server certificate. Exactly one certificate is required if the protocol is HTTPS. For adding additional SSL certificates, see the [`aws_elbv2_lb_listener_certificate` resource](/docs/providers/aws/r/lb_listener_certificate.html).
...
```
$ terraform import aws_elbv2_lb_listener.front_end arn:aws:elasticloadbalancing:us-west-2:187416307283:listener/app/front-end-alb/8e4497da625e2d8a/9ab28ade35828f96
```
