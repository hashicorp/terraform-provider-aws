...
* `resource_arn` - (Required) The ARN of the stateful rule group.
...
* `resource_arn` - (Required) The ARN of the stateless rule group.
...
* `id` - The ARN that identifies the firewall policy.

* `arn` - The ARN that identifies the firewall policy.
