...
* `iam_role_arn` - (Required) The ARN of the IAM role that AWS Backup uses to authenticate when restoring and backing up the target resource. This role needs to have `AWSBackupServiceRolePolicyForBackup` & `AWSBackupServiceRolePolicyForRestores` policies attached.
