...
* `kms_key_id` - (Optional) ARN of the AWS KMS customer managed key to use for encryption of data at rest. Must be a symmetric KMS key in the same AWS account and Region as the DB instance. This value is set at creation only; changing it forces a new resource to be created. If omitted, an AWS owned key is used.
