...
* `source_endpoint_arn` - (Required) The ARN string that uniquely identifies the source endpoint.
...
* `target_endpoint_arn` - (Required) The ARN string that uniquely identifies the target endpoint.
...
* `kms_key_id` - (Optional) A KMS key ARN that is used to encrypt the data during DMS Serverless replication. If you don't specify a value for the KmsKeyId parameter, DMS uses your default encryption key.
...
* `vpc_security_group_ids` - (Optional) Specifies the VPC security group to use with the DMS Serverless replication. The VPC security group must work with the VPC containing the replication.

## Attribute Reference

This resource exports the following attributes in addition to the arguments above:

* `arn` - The ARN for the serverless replication config.
