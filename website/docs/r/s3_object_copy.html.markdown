...
* `bucket_key_enabled` - (Optional) Whether to use an S3 Bucket Key for object encryption with server-side encryption using KMS (SSE-KMS).
...
* `kms_encryption_context` - (Optional) AWS KMS Encryption Context to use for object encryption. The value is a base64-encoded UTF-8 string holding JSON with the encryption context key-value pairs.
...
* `source_customer_algorithm` - (Optional) Algorithm to use when decrypting the source object (for example, AES256).
* `source_customer_key` - (Optional) Customer-provided encryption key for Amazon S3 to use to decrypt the source object. The encryption key provided in this header must be one that was used when the source object was created.
* `source_customer_key_md5` - (Optional) 128-bit MD5 digest of the encryption key according to RFC 1321. Amazon S3 uses this header for a message integrity check to ensure that the encryption key was transmitted without error.
* `storage_class` - (Optional) Desired [storage class](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CopyObject.html#AmazonS3-CopyObject-request-header-StorageClass) for the object. Defaults to `STANDARD`.
...
* `default_tags` - (Optional) Configuration block to override the provider [`default_tags` configuration block](https://registry.terraform.io/providers/hashicorp/aws/latest/docs#default_tags-configuration-block). See [`default_tags` Block](#override_providerdefault_tags-block) below.

### `override_provider.default_tags` Block

* `tags` - (Optional) Map of tags to override the provider-level `default_tags`. Set to an empty map (`{}`) to suppress all provider-level `default_tags`.
