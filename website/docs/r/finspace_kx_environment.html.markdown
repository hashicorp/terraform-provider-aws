...
* `cidr_block` - (Required) IPv4 network range to allow or deny, in CIDR notation. The specified CIDR block is modified to its canonical form. For example, `100.68.0.18/18` will be converted to `100.68.0.0/18`.
* `icmp_type_code` - (Optional) ICMP protocol that consists of the ICMP type and code. Defined below.
...
* `rule_action` - (Required) Whether to `allow` or `deny` the traffic that matches the rule.
