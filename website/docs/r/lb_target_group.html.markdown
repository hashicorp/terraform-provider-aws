...
* `protocol` - (Optional) The protocol to use to connect with the target. Defaults to the protocol of the Target Group. Not applicable when `target_type` is `lambda`.
