...
* `disable_healthcheck` - (Optional) A boolean value that stops Route 53 from performing health checks. If true, then Route 53 considers the status of the health check to always be healthy. If you configured DNS failover, Route 53 continues to route traffic to the corresponding resources. If you want to stop routing traffic to a resource, set the value of invert_healthcheck to true.
