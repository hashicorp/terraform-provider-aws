...
* `shard_count_step_scaling` - (Optional) When `true`, shard count changes are applied by iteratively doubling or halving toward the target count, respecting the [UpdateShardCount API limits](https://docs.aws.amazon.com/kinesis/latest/APIReference/API_UpdateShardCount.html). Each step waits for the stream to become `ACTIVE` before proceeding. If a rate limit is hit partway through, the apply fails at that point and the next apply resumes from the current count. Defaults to `false`.
