...
### Strategy with Memory Record Schema

```terraform
resource "aws_bedrockagentcore_memory_strategy" "with_schema" {
  name        = "semantic-with-schema"
  memory_id   = aws_bedrockagentcore_memory.example.id
  type        = "SEMANTIC"
  description = "Semantic strategy with metadata schema"
  namespaces  = ["default"]
    ...
    metadata_schema {
      ...
      extraction_config {
        llm_extraction_config {
          definition                 = "The priority level of the record"
          llm_extraction_instruction = "Infer the priority from the conversation context"
...
* `memory_record_schema` - (Optional) Schema for metadata fields on records generated by this strategy. Valid for all strategy types. See [`memory_record_schema`](#memory_record_schema) below.

### `memory_record_schema`

The `memory_record_schema` block supports the following:

* `metadata_schema` - (Optional) List of metadata field definitions for records generated by this strategy. See [`metadata_schema`](#metadata_schema) below.
...
* `extraction_config` - (Optional) Configuration for extracting this metadata value from conversational content. Applicable only when `extraction_type` is `LLM_INFERRED`. See [`extraction_config`](#extraction_config) below.

### `extraction_config`

The `extraction_config` block supports the following:

* `llm_extraction_config` - (Optional) Model-based extraction configuration. See [`llm_extraction_config`](#llm_extraction_config) below.

### `llm_extraction_config`

The `llm_extraction_config` block supports the following:

* `definition` - (Required) Description of what this metadata field represents.
* `llm_extraction_instruction` - (Optional) Instructions for extraction. Supports built-in operators like `LATEST_VALUE` or custom natural-language instructions.
* `validation` - (Optional) Validation rules to constrain extracted values. See [`validation`](#validation) below.

### `validation`

Each `validation` block specifies exactly one of the following nested blocks:

* `string_validation` - (Optional) Validation for `STRING` fields. See [`string_validation`](#string_validation) below.
* `number_validation` - (Optional) Validation for `NUMBER` fields. See [`number_validation`](#number_validation) below.
* `string_list_validation` - (Optional) Validation for `STRINGLIST` fields. See [`string_list_validation`](#string_list_validation) below.

### `string_validation`

* `allowed_values` - (Required) Allowed values for this `STRING` field.
...
* `allowed_values` - (Optional) Allowed values for items in this `STRINGLIST` field.
* `max_items` - (Optional) Maximum number of items in the string list.
