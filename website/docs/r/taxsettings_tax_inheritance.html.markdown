...
description: |-
  Manages tax information inheritance of the payer account within an AWS Organization.
---

# Resource: aws_taxsettings_tax_inheritance

Manages tax information inheritance of the management account within an AWS Organization.

~> **Note:** This resource requires the AWS Organization management account. The terms "payer account" and "management account" are used interchangeably, as AWS refers to the [managemement account as the payer account](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_getting-started_concepts.html).
...
* `heritage_status` - (Required) Whether to enable (`OptIn`) or disable (`OptOut`) the tax information inheritance of the payer account for all member accounts within your AWS Organization.
