...
* `certificate_rotation_restart` - (Optional) Whether the DB instance is restarted when you rotate your SSL/TLS certificate. By default, the DB instance is restarted. The certificate is not updated until the DB instance is restarted. Do not use this if you are using SSL/TLS to connect to the instance.
