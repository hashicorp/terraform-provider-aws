...
* `certificate_rotation_restart` – (Optional) Whether to restart the DB instance when rotating its SSL/TLS certificate. Defaults to `true`. Restarting the DB instance is required for the new certificate to take effect. If this is set to `false`, the certificate will not be applied until the instance is restarted. Set this to `false` only when the instance is not using SSL/TLS connections.
