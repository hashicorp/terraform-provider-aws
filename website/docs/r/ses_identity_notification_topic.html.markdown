...
* `topic_arn` - (Optional) The ARN of the Amazon SNS topic. Can be set to `""` (an empty string) to disable publishing.
...
* `identity` - (Required) The identity for which the Amazon SNS topic will be set. You can specify an identity by using its name or by using its ARN.
