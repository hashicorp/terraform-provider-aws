...
## Usage with Lambda Function URL
...
resource "aws_lambda_function" "func" {
  filename      = "lambda.zip"
  function_name = "lambda_with_public_url"
  role          = aws_iam_role.iam_for_lambda.arn
  handler       = "exports.handler"
  runtime       = "nodejs12.x"
}
...
* `function_url_authorization_type` - (Optional)  Can only be used when `action` value is `lambda:InvokeFunctionUrl` The type of authentication that the function URL uses. Set to "AWS_IAM" to restrict access to authenticated IAM users only. Set to "NONE" to bypass IAM authentication and create a public endpoint. See the AWS documentation for more details.
