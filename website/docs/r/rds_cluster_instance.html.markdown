...
* `preferred_backup_window` - (Optional) The daily time range during which automated backups are created if automated backups are enabled. Eg: "04:00-09:00". **NOTE:** If the instance is configured to be part of a cluster, the `preferred_backup_window` must be set at the cluster level.
