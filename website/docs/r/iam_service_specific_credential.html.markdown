...
* `id` - The combination of `service_name` and `user_name` as such: `service_name:user_name`.
...
IAM Service Specific Credentials can be imported using the `id`, e.g.

```
$ terraform import aws_iam_service_specific_credential.default `codecommit.amazonaws.com:example`
```