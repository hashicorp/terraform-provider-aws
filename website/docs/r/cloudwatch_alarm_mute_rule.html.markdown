...
* `example_id_arg` - ID argument of the Alarm Mute Rule.
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import CloudWatch Alarm Mute Rule using the `example_id_arg`. For example:

```terraform
import {
  to = aws_cloudwatch_alarm_mute_rule.example
  id = "alarm_mute_rule-id-12345678"
}
```

Using `terraform import`, import CloudWatch Alarm Mute Rule using the `example_id_arg`. For example:

```console
% terraform import aws_cloudwatch_alarm_mute_rule.example alarm_mute_rule-id-12345678
```
