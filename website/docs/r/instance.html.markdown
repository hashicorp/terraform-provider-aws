...
* `ena_queue_count` - (Optional) Number of ENA queues to allocate to the primary network interface created with the instance. Requires `subnet_id` and conflicts with `network_interface` and `primary_network_interface`. The value must be a power of two and is limited by the instance type. Updating this argument stops and restarts the instance. Removing it resets the primary network interface to the default ENA queue count. For more information, see [ENA queues](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ena-queues.html).
