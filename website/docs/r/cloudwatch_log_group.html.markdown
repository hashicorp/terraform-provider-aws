...
* `bearer_token_authentication_enabled` – (Optional) Boolean to indicate whether bearer token authentication is enabled. When enabled, the log group accepts bearer token authentication for log ingestion via the HLC endpoint. Defaults to `false`. Once set, switching to `false` requires explicitly specifying `false` rather than removing this argument.
