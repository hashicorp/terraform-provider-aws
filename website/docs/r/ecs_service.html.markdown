...
### Blue/Green Deployment with SIGINT Cancellation
...
* `sigint_cancellation` - (Optional) Whether to enable graceful termination of Blue/Green deployments using SIGINT signals. When enabled, allows customers to safely cancel an in-progress Blue/Green deployment and automatically trigger a rollback to the previous stable state. Defaults to `false`. Only applicable when using `BLUE_GREEN` deployment strategy.
