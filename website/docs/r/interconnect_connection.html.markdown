...
```terraform
data "aws_interconnect_environments" "available" {}

resource "aws_interconnect_connection" "example" {
  bandwidth      = "10Gbps"
  environment_id = data.aws_interconnect_environments.available.environments[0].environment_id
...
Environments are scoped to a single AWS Region, so use the [`aws_interconnect_environments`](/docs/providers/aws/d/interconnect_environments.html) data source to discover valid values for the Region you are operating in.
...
* `environment_id` - (Required) Identifier of the Environment on which this connection is created. The Environment determines the AWS Region and remote partner location pairing, and is scoped to the AWS Region you are operating in. Use the [`aws_interconnect_environments`](/docs/providers/aws/d/interconnect_environments.html) data source to discover available values. Changing this forces a new resource to be created.
