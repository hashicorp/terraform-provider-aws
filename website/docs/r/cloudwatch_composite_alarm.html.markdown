...
EOF
  ...
  actions_suppressor_alarm = "suppressor-alarm"
...
* `actions_suppressor_alarm` - (Optional) Actions will be suppressed if the suppressor alarm is in the `ALARM` state. `actions_suppressor_alarm` can be an AlarmName or an Amazon Resource Name (ARN) from an existing alarm.
* `actions_suppressor_extension_period` - (Optional) The maximum time in seconds that the composite alarm waits after suppressor alarm goes out of the `ALARM` state. After this time, the composite alarm performs its actions. This is only required `actions_suppressor_alarm` is specified.
* `actions_suppressor_wait_period` - (Optional) The maximum time in seconds that the composite alarm waits for the suppressor alarm to go into the `ALARM` state. After this time, the composite alarm performs its actions. This is only required `actions_suppressor_alarm` is specified.
