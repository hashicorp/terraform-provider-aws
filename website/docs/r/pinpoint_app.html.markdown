...
* `campaign_hook` - (Optional, **Deprecated**) Specifies settings for invoking an AWS Lambda function that customizes a segment for a campaign
* `limits` - (Optional, **Deprecated**) The default campaign limits for the app. These limits apply to each campaign for the app, unless the campaign overrides the default with limits of its own
* `quiet_time` - (Optional, **Deprecated**) The default quiet time for the app. Each campaign for this app sends no messages during this time unless the campaign overrides the default with a quiet time of its own
