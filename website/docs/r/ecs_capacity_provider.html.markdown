...
* `local_storage_configuration` - (Optional) The local storage configuration for Amazon ECS Managed Instances. This defines how Amazon ECS uses instance store volumes available on the container instance. Detailed below.
...
* `use_local_storage` - (Required) Whether to use instance store volumes for data storage when available. When enabled, EBS volumes are not provisioned for data storage. If the container instance has multiple instance store volumes, a single data volume is created.
