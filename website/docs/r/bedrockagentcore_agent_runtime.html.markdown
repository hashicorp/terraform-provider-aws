...
* `platform_version` - (Optional) Platform version that controls how the agent runtime starts your agent. Valid values: `V1`, `V2`. Defaults to `V1`. `V2` restores each instance from a prepared snapshot, which keeps cold starts consistent regardless of image size. On `V2`, creates and updates take several minutes instead of seconds, the total size of `environment_variables` is limited to 2.5 KB for container artifacts and 1.5 KB for code artifacts (4 KB on `V1`), and availability is limited to a subset of Regions. See [Platform versions](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html#runtime-platform-versions) for details.
