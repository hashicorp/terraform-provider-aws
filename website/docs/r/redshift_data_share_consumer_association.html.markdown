...
* `data_share_arn` - (Required) ARN of the datashare that the consumer is to use with the account or the namespace.
...
* `consumer_arn` - (Optional) ARN of the consumer that is associated with the datashare. Conflicts with `associate_entire_account` and `consumer_region`.
