...
func TestNullOutObjectPtrFields(t *testing.T) {
	...
	a.F7 = fwtypes.NewListNestedObjectValueOfValueSliceMust(ctx, []C{
		{
			F1: fwtypes.NewMapValueOfMust[types.String](ctx, map[string]attr.Value{
				"key": types.StringValue("value"),
			}),
			F2: fwtypes.NewSetNestedObjectValueOfValueSliceMust(ctx, []D{
				{
					F1: fwtypes.ARNNull(),
				},
				{
					F1: fwtypes.ARNUnknown(),
				},
			}),
		},
	})
