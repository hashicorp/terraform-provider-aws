...
// Test types for XML wrapper functionality
...
type awsDistributionWithWrappersForFlatten struct {
	StatusCodes *awsStatusCodesForFlatten
	Headers     awsHeadersForFlatten // Direct struct, not pointer
}
...
type awsFunctionAssociationsForFlatten struct {
	Items    []awsFunctionAssociation `json:"Items"`
	Quantity *int32                   `json:"Quantity"`
}

type functionAssociationModelForFlatten struct {
	EventType   types.String `tfsdk:"event_type"`
	FunctionARN types.String `tfsdk:"function_arn"`
}

type tfFunctionAssociationsModelForFlatten struct {
	FunctionAssociations fwtypes.SetNestedObjectValueOf[functionAssociationModelForFlatten] `tfsdk:"function_associations" autoflex:",wrapper=items"`
}
...
type tfDistributionModelForFlatten struct {
	StatusCodes fwtypes.SetValueOf[types.Int64]   `tfsdk:"status_codes" autoflex:",wrapper=items"`
	Headers     fwtypes.ListValueOf[types.String] `tfsdk:"headers" autoflex:",wrapper=items"`
}
...
func TestFlattenXMLWrapper(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"complex type - function associations": {
			...
			WantTarget: &tfFunctionAssociationsModelForFlatten{
				FunctionAssociations: func() fwtypes.SetNestedObjectValueOf[functionAssociationModelForFlatten] {
					elems := []*functionAssociationModelForFlatten{
						{
	...
	for testName, testCase := range testCases {
		t.Run(testName, func(t *testing.T) {
			...
			ctx := context.Background()
			diags := Flatten(ctx, testCase.Source, testCase.Target)
