...
func (e *EmptyResultError) Is(err error) bool {
	_, ok := err.(*EmptyResultError) //nolint:errorlint // Explictly does *not* match down the error tree
	return ok
}
...
func (e *TooManyResultsError) Is(err error) bool {
	_, ok := err.(*TooManyResultsError) //nolint:errorlint // Explictly does *not* match down the error tree
	return ok
}
