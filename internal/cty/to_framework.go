...
// ToFramework populates the struct passed as `target` with the entire cty object passed as `source`.
// The target's fields must be Plugin Framework types.
func ToFramework(ctx context.Context, source cty.Value, target any) error {
	if !source.IsKnown() {
	...
	if kind := vTarget.Kind(); kind != reflect.Ptr { //nolint:govet // wants use to inline constant
