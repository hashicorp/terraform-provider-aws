...
// CIDRBlockKey returns a key matching the equality semantics of CIDRBlocksEqual.
func CIDRBlockKey(cidr string) (string, error) {
	ip, ipnet, err := net.ParseCIDR(cidr)
	if err != nil {
		return "", err
	}

	return ip.String() + "_" + ipnet.String(), nil
}
