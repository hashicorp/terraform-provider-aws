...
const (
	packageName = "arczonalshift"
	awsEnvVar   = "AWS_ENDPOINT_URL_ARC_ZONAL_SHIFT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "arc_zonal_shift"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := arczonalshift.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, arczonalshift.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ARCZonalShiftClient(ctx)

	var result apiCallParams

	input := arczonalshift.ListZonalShiftsInput{}
	_, err := client.ListZonalShifts(ctx, &input,
		func(opts *arczonalshift.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving arczonalshift default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving arczonalshift FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up arczonalshift endpoint %q: %s", hostname, err)
	}
