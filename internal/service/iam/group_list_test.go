...
func TestAccIAMGroup_List_basic(t *testing.T) {
	ctx := acctest.Context(t)

	resourceName1 := "aws_iam_group.test[0]"
	resourceName2 := "aws_iam_group.test[1]"
		...
		Steps: []resource.TestStep{
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					tfquerycheck.ExpectIdentityFunc("aws_iam_group.test", identity1.Checks()),
					querycheck.ExpectResourceDisplayName("aws_iam_group.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), knownvalue.StringExact(rName+"-0")),
					tfquerycheck.ExpectNoResourceObject("aws_iam_group.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks())),

					tfquerycheck.ExpectIdentityFunc("aws_iam_group.test", identity2.Checks()),
					querycheck.ExpectResourceDisplayName("aws_iam_group.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks()), knownvalue.StringExact(rName+"-1")),
					tfquerycheck.ExpectNoResourceObject("aws_iam_group.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks())),
				},
			},
		},
	})
...
func TestAccIAMGroup_List_includeResource(t *testing.T) {
	ctx := acctest.Context(t)
		...
		Steps: []resource.TestStep{
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					tfquerycheck.ExpectIdentityFunc("aws_iam_group.test", identity1.Checks()),
					querycheck.ExpectResourceDisplayName("aws_iam_group.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), knownvalue.StringExact(rName+"-0")),
					querycheck.ExpectResourceKnownValues("aws_iam_group.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), []querycheck.KnownValueCheck{
						tfquerycheck.KnownValueCheck(tfjsonpath.New(names.AttrARN), tfknownvalue.GlobalARNExact("iam", "group/"+rName+"-0")),
...
func TestAccIAMGroup_List_pathPrefix(t *testing.T) {
	ctx := acctest.Context(t)

	resourceNameExpected1 := "aws_iam_group.expected[0]"
	resourceNameExpected2 := "aws_iam_group.expected[1]"
	resourceNameNotExpected1 := "aws_iam_group.not_expected[0]"
	resourceNameNotExpected2 := "aws_iam_group.not_expected[1]"
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/Group/list_path_prefix/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:      config.StringVariable(rName),
					"resource_count":     config.IntegerVariable(2),
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLength("aws_iam_group.test", 2),
					tfquerycheck.ExpectIdentityFunc("aws_iam_group.test", identityExpected1.Checks()),
					tfquerycheck.ExpectIdentityFunc("aws_iam_group.test", identityExpected2.Checks()),
				},
			},
		},
	})
