...
const (
	packageName = "chimesdkvoice"
	awsEnvVar   = "AWS_ENDPOINT_URL_CHIME_SDK_VOICE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "chime_sdk_voice"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := chimesdkvoice.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, chimesdkvoice.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ChimeSDKVoiceClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Chime SDK Voice default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Chime SDK Voice FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Chime SDK Voice endpoint %q: %s", hostname, err)
	}
