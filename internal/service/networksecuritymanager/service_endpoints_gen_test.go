...
const (
	packageName = "networksecuritymanager"
	awsEnvVar   = "AWS_ENDPOINT_URL_NETWORK_SECURITY_MANAGER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "network_security_manager"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := networksecuritymanager.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, networksecuritymanager.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.NetworkSecurityManagerClient(ctx)

	var result apiCallParams

	input := networksecuritymanager.ListScopesInput{}
	_, err := client.ListScopes(ctx, &input,
		func(opts *networksecuritymanager.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Network Security Manager default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Network Security Manager FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Network Security Manager endpoint %q: %s", hostname, err)
	}
