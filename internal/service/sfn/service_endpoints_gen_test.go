...
func defaultEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(sfn_sdkv1.EndpointsID, region)
	if err != nil {
		return err.Error()
	}
...
func defaultFIPSEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(sfn_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SFNConn(ctx)

	req, _ := client.ListActivitiesRequest(&sfn_sdkv1.ListActivitiesInput{})
