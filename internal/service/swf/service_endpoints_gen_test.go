...
const (
	packageName = "swf"
	awsEnvVar   = "AWS_ENDPOINT_URL_SWF"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := swf.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, swf.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SWF (Simple Workflow) default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SWF (Simple Workflow) FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up SWF (Simple Workflow) endpoint %q: %s", hostname, err)
	}
