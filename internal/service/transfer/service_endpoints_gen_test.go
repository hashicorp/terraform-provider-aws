...
const (
	...
	awsEnvVar   = "AWS_ENDPOINT_URL_TRANSFER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "transfer"
)
...
func defaultEndpoint(region string) string {
	r := transfer_sdkv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), transfer_sdkv2.EndpointParameters{
...
func callServiceV2(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.TransferClient(ctx)

	_, err := client.ListConnectors(ctx, &transfer_sdkv2.ListConnectorsInput{},
		func(opts *transfer_sdkv2.Options) {
...
func callServiceV1(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.TransferConn(ctx)

	req, _ := client.ListConnectorsRequest(&transfer_sdkv1.ListConnectorsInput{})
