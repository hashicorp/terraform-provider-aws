...
const (
	packageName = "autoscalingplans"
	awsEnvVar   = "AWS_ENDPOINT_URL_AUTO_SCALING_PLANS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "auto_scaling_plans"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := autoscalingplans.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, autoscalingplans.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.AutoScalingPlansClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Auto Scaling Plans default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Auto Scaling Plans FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Auto Scaling Plans endpoint %q: %s", hostname, err)
	}
