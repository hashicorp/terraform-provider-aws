...
const (
	packageName = "codestarnotifications"
	awsEnvVar   = "AWS_ENDPOINT_URL_CODESTAR_NOTIFICATIONS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "codestar_notifications"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := codestarnotifications.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, codestarnotifications.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CodeStarNotificationsClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CodeStar Notifications default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CodeStar Notifications FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up CodeStar Notifications endpoint %q: %s", hostname, err)
	}
