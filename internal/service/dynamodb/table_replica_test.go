...
func testAccCheckTableReplicaDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("table replica has no ID!")
		}

		tableName, mainRegion, err := tfdynamodb.TableReplicaParseID(rs.Primary.ID)
		if err != nil {
			return create.Error(names.DynamoDB, create.ErrActionReading, "Table Replica", rs.Primary.ID, err)
		}
		...
		if err != nil {
			return fmt.Errorf("checking table replica existence %s still exists: %s", rs.Primary.ID, err)
		}
...
func testAccCheckTableReplicaExists(n string, table *dynamodb.DescribeTableOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if err != nil {
			return fmt.Errorf("describing table replica (%s): %s", rs.Primary.ID, err)
		}
