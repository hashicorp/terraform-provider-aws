...
const (
	packageName = "rdsdata"
	awsEnvVar   = "AWS_ENDPOINT_URL_RDS_DATA"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "rds_data"

	aliasName0 = "rdsdataservice"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := rdsdata.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, rdsdata.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.RDSDataClient(ctx)

	var result apiCallParams

	input := rdsdata.ExecuteStatementInput{
		ResourceArn: aws.String("arn:" + acctest.Partition() + ":rds:" + acctest.Region() + ":" + acctest.Ct12Digit + ":cluster:test"),
	...
	_, err := client.ExecuteStatement(ctx, &input,
		func(opts *rdsdata.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving RDS Data default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving RDS Data FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up RDS Data endpoint %q: %s", hostname, err)
	}
