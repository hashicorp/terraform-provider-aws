		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(dataSourceName, "provisioning_artifact_details.0.guidance", string(types.ProvisioningArtifactGuidanceDefault)),
					resource.TestCheckResourceAttr(dataSourceName, "provisioning_artifact_details.0.name", rName),
					resource.TestCheckResourceAttr(dataSourceName, "provisioning_artifact_details.0.type", string(types.ProductTypeCloudFormationTemplate)),
				),
			},
		},
	})
