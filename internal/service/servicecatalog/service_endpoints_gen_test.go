...
const (
	packageName = "servicecatalog"
	awsEnvVar   = "AWS_ENDPOINT_URL_SERVICE_CATALOG"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "service_catalog"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := servicecatalog.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, servicecatalog.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Service Catalog default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Service Catalog FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Service Catalog endpoint %q: %s", hostname, err)
	}
