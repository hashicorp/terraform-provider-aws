...
func testAccCheckPipelineResourcePolicyDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			if err != nil {
				return fmt.Errorf("error getting OpenSearch Ingestion pipeline Resource Policy (%s) to verify destroy: %s", rs.Primary.ID, err)
			}
		}
...
func testAccPipelineResourcePolicyConfig_basic(rName string) string {
	//fmt.Sprintf(testAccPipelineConfig_basic(rName),
	//aws_osis_pipeline.test.pipeline_arn
	// arn:aws:osis:ap-southeast-2:388303208821:pipeline/tf-acc-test-m3f6ppb6k1
...
func testAccPipelineResourcePolicyConfig_updated(rName string) string {
	return acctest.ConfigCompose(fmt.Sprintf(testAccPipelineConfig_basic(rName), fmt.Sprintf(`
