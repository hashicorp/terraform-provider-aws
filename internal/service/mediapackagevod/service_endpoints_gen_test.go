...
const (
	packageName = "mediapackagevod"
	awsEnvVar   = "AWS_ENDPOINT_URL_MEDIAPACKAGE_VOD"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "mediapackage_vod"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := mediapackagevod.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, mediapackagevod.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MediaPackageVODClient(ctx)

	var result apiCallParams

	input := mediapackagevod.ListPackagingGroupsInput{}
	_, err := client.ListPackagingGroups(ctx, &input,
		func(opts *mediapackagevod.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Elemental MediaPackage VOD default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Elemental MediaPackage VOD FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Elemental MediaPackage VOD endpoint %q: %s", hostname, err)
	}
