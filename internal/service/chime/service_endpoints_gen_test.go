...
func defaultEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(chime_sdkv1.EndpointsID, region)
	if err != nil {
		return err.Error()
	}
...
func defaultFIPSEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(chime_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ChimeConn(ctx)

	req, _ := client.ListAccountsRequest(&chime_sdkv1.ListAccountsInput{})
