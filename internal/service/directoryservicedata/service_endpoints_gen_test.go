...
const (
	packageName = "directoryservicedata"
	awsEnvVar   = "AWS_ENDPOINT_URL_DIRECTORY_SERVICE_DATA"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "directory_service_data"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := directoryservicedata.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, directoryservicedata.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DirectoryServiceDataClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Directory Service Data default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Directory Service Data FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Directory Service Data endpoint %q: %s", hostname, err)
	}
