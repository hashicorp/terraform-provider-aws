...
const (
	packageName = "workspacesweb"
	awsEnvVar   = "AWS_ENDPOINT_URL_WORKSPACES_WEB"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "workspaces_web"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := workspacesweb.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, workspacesweb.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.WorkSpacesWebClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving WorkSpaces Web default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving WorkSpaces Web FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up WorkSpaces Web endpoint %q: %s", hostname, err)
	}
