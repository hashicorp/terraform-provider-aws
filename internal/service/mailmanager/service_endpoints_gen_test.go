...
const (
	...
	awsEnvVar   = "AWS_ENDPOINT_URL_MAILMANAGER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "mailmanager"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := mailmanager.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, mailmanager.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MailManagerClient(ctx)

	var result apiCallParams

	input := mailmanager.ListIngressPointsInput{}
	_, err := client.ListIngressPoints(ctx, &input,
		func(opts *mailmanager.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SES Mail Manager default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SES Mail Manager FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up SES Mail Manager endpoint %q: %s", hostname, err)
	}
