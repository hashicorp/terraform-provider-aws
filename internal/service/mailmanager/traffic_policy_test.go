		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.0.ipv6_expression.0.values.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.0.ipv6_expression.0.values.0", "2001:db8::/32"),
					...
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.1.string_expression.0.operator", "ENDS_WITH"),
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.1.string_expression.0.values.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.1.string_expression.0.values.0", "@example.com"),
					...
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.2.tls_expression.0.operator", "MINIMUM_TLS_VERSION"),
					resource.TestCheckResourceAttr(resourceName, "policy_statement.0.condition.2.tls_expression.0.value", "TLS1_2"),
				),
			},
		},
	})
...
func testAccPreCheckTrafficPolicy(ctx context.Context, t *testing.T) {
	conn := acctest.ProviderMeta(ctx, t).MailManagerClient(ctx)
