  ...
  evaluation_config {
    human {
      ...
      human_workflow_config {
        flow_definition_arn = "arn:${data.aws_partition.current.partition}:sagemaker:${var.region}:${data.aws_caller_identity.current.account_id}:flow-definition/${var.rName}"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ...
        ArnEquals = {
          "aws:SourceArn" = "arn:${data.aws_partition.current.partition}:bedrock:${var.region}:${data.aws_caller_identity.current.account_id}:evaluation-job/*"
        }
      }
    }]
  })
