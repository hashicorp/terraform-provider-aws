...
func (p *servicePackage) NewConn(ctx context.Context) (*lakeformation_sdkv1.LakeFormation, error) {
	sess := p.config["session"].(*session_sdkv1.Session)

	return lakeformation_sdkv1.New(sess.Copy(&aws_sdkv1.Config{Endpoint: aws_sdkv1.String(p.config["endpoint"].(string))})), nil
}
