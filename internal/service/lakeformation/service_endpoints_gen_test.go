...
const (
	packageName = "lakeformation"
	awsEnvVar   = "AWS_ENDPOINT_URL_LAKEFORMATION"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "lakeformation"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := lakeformation.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, lakeformation.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.LakeFormationClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Lake Formation default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Lake Formation FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Lake Formation endpoint %q: %s", hostname, err)
	}
