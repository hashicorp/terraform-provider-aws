...
func defaultEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(ivs_sdkv1.EndpointsID, region)
	if err != nil {
		return err.Error()
	}
...
func defaultFIPSEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(ivs_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.IVSConn(ctx)

	req, _ := client.ListChannelsRequest(&ivs_sdkv1.ListChannelsInput{})
