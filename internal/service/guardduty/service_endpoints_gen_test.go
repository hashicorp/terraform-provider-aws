...
const (
	packageName = "guardduty"
	awsEnvVar   = "AWS_ENDPOINT_URL_GUARDDUTY"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := guardduty.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, guardduty.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving GuardDuty default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving GuardDuty FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up GuardDuty endpoint %q: %s", hostname, err)
	}
