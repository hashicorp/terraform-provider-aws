...
const (
	packageName = "drs"
	awsEnvVar   = "AWS_ENDPOINT_URL_DRS"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := drs.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, drs.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving DRS (Elastic Disaster Recovery) default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving DRS (Elastic Disaster Recovery) FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up DRS (Elastic Disaster Recovery) endpoint %q: %s", hostname, err)
	}
