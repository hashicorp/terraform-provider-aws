...
const (
	packageName = "computeoptimizer"
	awsEnvVar   = "AWS_ENDPOINT_URL_COMPUTE_OPTIMIZER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "compute_optimizer"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := computeoptimizer.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, computeoptimizer.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ComputeOptimizerClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Compute Optimizer default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Compute Optimizer FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Compute Optimizer endpoint %q: %s", hostname, err)
	}
