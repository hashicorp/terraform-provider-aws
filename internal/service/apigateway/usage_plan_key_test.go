...
func TestAccAPIGatewayUsagePlanKey_basic(t *testing.T) {
	...
	apiGatewayApiKeyResourceName := "aws_apigateway_api_key.test"
	apiGatewayUsagePlanResourceName := "aws_apigateway_usage_plan.test"
	resourceName := "aws_apigateway_usage_plan_key.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.0", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.1", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.2", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.3", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.4", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.5", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.6", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.7", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.8", &conf),
					testAccCheckUsagePlanKeyExists("aws_apigateway_usage_plan_key.test.9", &conf),
				),
			},
		},
	})
...
func testAccCheckUsagePlanKeyDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "aws_apigateway_usage_plan_key" {
			continue
		}
...
resource "aws_apigateway_usage_plan_key" "test" {
  key_id        = aws_apigateway_api_key.test.id
  key_type      = "API_KEY"
  usage_plan_id = aws_apigateway_usage_plan.test.id
}
`, rName))
}
...
resource "aws_apigateway_usage_plan_key" "test" {
  ...
  key_id        = aws_apigateway_api_key.test[count.index].id
  key_type      = "API_KEY"
  usage_plan_id = aws_apigateway_usage_plan.test.id
}
`, rName))
