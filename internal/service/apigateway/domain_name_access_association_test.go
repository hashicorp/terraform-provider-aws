		...
		Steps: []resource.TestStep{
			{
				...
				Config: testAccDomainNameAccessAssociationConfig_identityV5(rName, domainName, key, certificate),
			...
			},
			{
				...
				Config: testAccDomainNameAccessAssociationConfig_basic(rName, domainName, key, certificate),
...
func testAccDomainNameAccessAssociationConfig_identityV5(rName, domainName, key, certificate string) string {
	return acctest.ConfigCompose(
		acctest.ConfigAvailableAZsNoOptIn(),
		fmt.Sprintf(`
resource "aws_api_gateway_domain_name_access_association" "test" {
