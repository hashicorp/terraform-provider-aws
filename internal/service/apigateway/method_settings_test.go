...
func TestAccAPIGatewayMethodSettings_basic(t *testing.T) {
	...
	resourceName := "aws_apigateway_method_settings.test"
...
func testAccCheckMethodSettingsDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "aws_apigateway_method_settings" {
			continue
		}
...
resource "aws_apigateway_method_settings" "test" {
  method_path = "${aws_apigateway_resource.test.path_part}/${aws_apigateway_method.test.http_method}"
  rest_api_id = aws_apigateway_rest_api.test.id
  stage_name  = aws_apigateway_deployment.test.stage_name
