...
const (
	packageName = "savingsplans"
	awsEnvVar   = "AWS_ENDPOINT_URL_SAVINGSPLANS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "savingsplans"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := savingsplans.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, savingsplans.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SavingsPlansClient(ctx)

	var result apiCallParams

	input := savingsplans.DescribeSavingsPlansInput{}
	_, err := client.DescribeSavingsPlans(ctx, &input,
		func(opts *savingsplans.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Savings Plans default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Savings Plans FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Savings Plans endpoint %q: %s", hostname, err)
	}
