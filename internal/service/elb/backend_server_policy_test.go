		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPolicyState("aws_elb_lb.test-lb", "aws_elb_lb_policy.test-pubkey-policy0"),
					testAccCheckPolicyState("aws_elb_lb.test-lb", "aws_elb_lb_policy.test-backend-auth-policy0"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPolicyState("aws_elb_lb.test-lb", "aws_elb_lb_policy.test-pubkey-policy0"),
					testAccCheckPolicyState("aws_elb_lb.test-lb", "aws_elb_lb_policy.test-pubkey-policy1"),
					testAccCheckPolicyState("aws_elb_lb.test-lb", "aws_elb_lb_policy.test-backend-auth-policy0"),
					testAccCheckBackendServerPolicyState(lbName, "test-backend-auth-policy0", true),
				),
			},
...
resource "aws_elb_lb_policy" "test-pubkey-policy0" {
...
resource "aws_elb_lb_policy" "test-backend-auth-policy0" {
  load_balancer_name = aws_elb_lb.test-lb.name
  ...
  policy_attribute {
    ...
    value = aws_elb_lb_policy.test-pubkey-policy0.policy_name
  }
}

resource "aws_elb_lb_backend_server_policy" "test-backend-auth-policies-443" {
  load_balancer_name = aws_elb_lb.test-lb.name
  instance_port      = 443

  policy_names = [
    aws_elb_lb_policy.test-backend-auth-policy0.policy_name,
  ]
}
...
resource "aws_elb_lb_policy" "test-pubkey-policy1" {
  load_balancer_name = aws_elb_lb.test-lb.name
...
resource "aws_elb_lb_policy" "test-backend-auth-policy0" {
  ...
  policy_attribute {
    name  = "PublicKeyPolicyName"
    value = aws_elb_lb_policy.test-pubkey-policy1.policy_name
  }
}
