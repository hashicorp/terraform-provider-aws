...
func defaultEndpoint(region string) (url.URL, error) {
	...
	ep, err := r.EndpointFor(outposts_sdkv1.EndpointsID, region)
	if err != nil {
		return url.URL{}, err
	}
...
func defaultFIPSEndpoint(region string) (url.URL, error) {
	...
	ep, err := r.EndpointFor(outposts_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.OutpostsConn(ctx)

	req, _ := client.ListSitesRequest(&outposts_sdkv1.ListSitesInput{})
