...
func TestAccDMSInstanceProfile_List_basic(t *testing.T) {
	ctx := acctest.Context(t)

	resourceName1 := "aws_dms_instance_profile.test[0]"
	resourceName2 := "aws_dms_instance_profile.test[1]"
		...
		Steps: []resource.TestStep{
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					tfquerycheck.ExpectNoResourceObject("aws_dms_instance_profile.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks())),

					tfquerycheck.ExpectIdentityFunc("aws_dms_instance_profile.test", identity2.Checks()),
					querycheck.ExpectResourceDisplayName("aws_dms_instance_profile.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks()), knownvalue.StringExact(rName+"-1")),
					tfquerycheck.ExpectNoResourceObject("aws_dms_instance_profile.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks())),
				},
			},
		},
	})
...
func TestAccDMSInstanceProfile_List_includeResource(t *testing.T) {
	ctx := acctest.Context(t)
...
func TestAccDMSInstanceProfile_List_regionOverride(t *testing.T) {
	ctx := acctest.Context(t)
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/InstanceProfile/list_region_override/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
					"resource_count": config.IntegerVariable(1),
				...
				ConfigStateChecks: []statecheck.StateCheck{
					identity1.GetIdentity(resourceName1),
					statecheck.ExpectKnownValue(resourceName1, tfjsonpath.New(names.AttrName), knownvalue.StringExact(rName+"-0")),
					statecheck.ExpectKnownValue(resourceName1, tfjsonpath.New(names.AttrARN), tfknownvalue.RegionalARNAlternateRegionRegexp("dms", regexache.MustCompile(`instance-profile:.+$`))),
				},
			},
