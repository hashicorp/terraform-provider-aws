...
func (v indexFieldAttributeValidator) ValidateBool(ctx context.Context, req validator.BoolRequest, resp *validator.BoolResponse) {
	...
	if slices.Contains(v.invalidFieldTypes, fieldType.ValueString()) {
		resp.Diagnostics.AddAttributeError(
			...
			fmt.Sprintf(
				"The '%s' attribute cannot be set for '%s' field types. "+
					"This attribute is not supported by AWS CloudSearch for this field type. "+
					"Remove this attribute from the configuration.",
				v.attributeName,
