...
const (
	packageName = "cloudsearch"
	awsEnvVar   = "AWS_ENDPOINT_URL_CLOUDSEARCH"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cloudsearch"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := cloudsearch.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, cloudsearch.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving cloudsearch default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving cloudsearch FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up cloudsearch endpoint %q: %s", hostname, err)
	}
