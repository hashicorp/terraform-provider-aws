...
func TestAccDirectoryServiceConditionalForwarder_Condition_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// test create
			{
				Config: testAccConditionalForwarderConfig_basic(domainName, ip1, ip2),
			...
			// test update
			{
				Config: testAccConditionalForwarderConfig_basic(domainName, ip1, ip3),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckConditionalForwarderExists(
						resourceName,
						[]string{ip1, ip3},
...
func testAccConditionalForwarderConfig_basic(domain, ip1, ip2 string) string {
