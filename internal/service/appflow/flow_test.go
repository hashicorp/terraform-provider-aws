		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "trigger_config.#", acctest.CtOne),
					resource.TestCheckResourceAttr(resourceName, "trigger_config.0.trigger_type", "Scheduled"),
					resource.TestCheckResourceAttr(resourceName, "trigger_config.0.trigger_properties.#", acctest.CtOne),
					resource.TestCheckResourceAttr(resourceName, "trigger_config.0.trigger_properties.0.scheduled.#", acctest.CtOne),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFlowExists(ctx, resourceName, &flowOutput),
					resource.TestCheckResourceAttr(resourceName, "task.0.task_properties.%", acctest.CtTwo),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckTypeSetElemNestedAttrs(resourceName, "task.*", map[string]string{
						...
						"source_fields.#":                       acctest.CtOne,
						"source_fields.0":                       "testField",
						"task_properties.%":                     acctest.CtTwo,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFlowExists(ctx, resourceName, &flowOutput),
					resource.TestCheckResourceAttr(resourceName, "task.#", acctest.CtOne),
				),
			},
			{
				Config:   testAccFlowConfig_task_mapAll(rSourceName, rDestinationName, rFlowName),
