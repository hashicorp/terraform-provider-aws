...
const (
	packageName = "lambdacore"
	awsEnvVar   = "AWS_ENDPOINT_URL_LAMBDA_CORE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "lambda_core"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := lambdacore.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, lambdacore.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.LambdaCoreClient(ctx)

	var result apiCallParams

	input := lambdacore.ListNetworkConnectorsInput{}
	_, err := client.ListNetworkConnectors(ctx, &input,
		func(opts *lambdacore.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Lambda Core default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Lambda Core FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Lambda Core endpoint %q: %s", hostname, err)
	}
