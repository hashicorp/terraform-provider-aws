...
const (
	packageName = "route53profiles"
	awsEnvVar   = "AWS_ENDPOINT_URL_ROUTE_53_PROFILES"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "route_53_profiles"
)
...
func defaultEndpoint(region string) string {
	r := route53profiles_sdkv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), route53profiles_sdkv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.Route53ProfilesClient(ctx)

	_, err := client.ListProfiles(ctx, &route53profiles_sdkv2.ListProfilesInput{},
		func(opts *route53profiles_sdkv2.Options) {
