...
const (
	packageName = "timestreaminfluxdb"
	awsEnvVar   = "AWS_ENDPOINT_URL_TIMESTREAM_INFLUXDB"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "timestream_influxdb"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := timestreaminfluxdb.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, timestreaminfluxdb.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Timestream for InfluxDB default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Timestream for InfluxDB FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Timestream for InfluxDB endpoint %q: %s", hostname, err)
	}
