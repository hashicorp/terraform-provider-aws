...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := rds.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, rds.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving RDS (Relational Database) default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving RDS (Relational Database) FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up RDS (Relational Database) endpoint %q: %s", hostname, err)
	}
