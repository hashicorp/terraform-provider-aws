...
func TestAccRDSClusterInstance_AuroraServerlessV2(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping long-running test in short mode")
	}
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccClusterInstanceConfigAuroraServerlessV2(sdkacctest.RandInt()),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckClusterInstanceExists(resourceName, &v),
					testAccCheckClusterInstanceAttributes(&v),
					acctest.MatchResourceAttrRegionalARN(resourceName, "arn", "rds", regexp.MustCompile(`db:.+`)),
					...
					resource.TestCheckResourceAttr(resourceName, "instance_class", "db.serverless"),
				),
			},
			{
				ResourceName:      resourceName,
...
func testAccClusterInstanceConfigAuroraServerlessV2(n int) string {
	return acctest.ConfigCompose(acctest.ConfigAvailableAZsNoOptIn(), fmt.Sprintf(`
data "aws_rds_orderable_db_instance" "test" {
  engine = "aurora-mysql"
...
data "aws_rds_engine_version" "default" {
	...
	version = data.aws_rds_orderable_db_instance.test.engine_version
}
...
resource "aws_rds_cluster_instance" "cluster_instances" {
  ...
  engine_version          = data.aws_rds_engine_version.default.version
  instance_class          = data.aws_rds_orderable_db_instance.test.instance_class
  db_parameter_group_name = "default.${data.aws_rds_engine_version.default.parameter_group_family}"
  promotion_tier          = "3"
}
`, n))
}

func testAccClusterInstanceModifiedConfigAuroraServerlessV2(n int) string {
	return acctest.ConfigCompose(acctest.ConfigAvailableAZsNoOptInExclude("af-south-1", "ap-northeast-3", "me-south-1"), fmt.Sprintf(`
data "aws_rds_orderable_db_instance" "test" {
  engine = "aurora-mysql"
  preferred_instance_classes = ["db.serverless"]
}
