...
func (r *resourceCluster) Create(ctx context.Context, request resource.CreateRequest, response *resource.CreateResponse) {
	...
	if !data.SnapshotIdentifier.IsUnknown() && !data.SnapshotIdentifier.IsNull() {
		input := restoreFromDBSnapshotInput(ctx, identifier, data)
		...
		if err != nil {
			response.Diagnostics.AddError(
				create.ProblemStandardMessage(names.RDS, create.ErrActionCreating, ResNameCluster, identifier, err),
				err.Error(),
			)
			return
		}
		...
		// create DB cluster
		input := createClusterInput(ctx, identifier, data)
...
func (r *resourceCluster) Read(ctx context.Context, request resource.ReadRequest, response *resource.ReadResponse) {
	...
	if err != nil {
		response.Diagnostics.AddError(
			create.ProblemStandardMessage(names.RDS, create.ErrActionReading, ResNameCluster, data.ID.ValueString(), err),
			err.Error(),
		)
		return
	}
...
func (r *resourceCluster) Update(ctx context.Context, request resource.UpdateRequest, response *resource.UpdateResponse) {
	...
	if modifyCluster {
		...
		if err != nil {
			response.Diagnostics.AddError(
				create.ProblemStandardMessage(names.RDS, create.ErrActionUpdating, ResNameCluster, plan.ID.ValueString(), err),
				err.Error(),
			)
			return
		}
	...
	if !plan.TagsAll.Equal(state.TagsAll) {
		if err := UpdateTags(ctx, conn, plan.ARN.ValueString(), state.TagsAll, plan.TagsAll); err != nil {
			response.Diagnostics.AddError(
				create.ProblemStandardMessage(names.RDS, create.ErrActionUpdating, ResNameCluster, plan.ID.String(), err),
				err.Error(),
			)
			return
		}
...
func (r *resourceCluster) Delete(ctx context.Context, request resource.DeleteRequest, response *resource.DeleteResponse) {
	...
	if !data.GlobalClusterIdentifier.IsNull() && data.GlobalClusterIdentifier.ValueString() != "" {
		...
		if err != nil && !tfawserr.ErrCodeEquals(err, rds.ErrCodeGlobalClusterNotFoundFault) && !tfawserr.ErrMessageContains(err, "InvalidParameterValue", "is not found in global cluster") {
			response.Diagnostics.AddError(
				create.ProblemStandardMessage(names.RDS, create.ErrActionDeleting, ResNameCluster, data.ID.String(), err),
				err.Error(),
			)
			return
		}
	...
	if err != nil {
		response.Diagnostics.AddError(
			create.ProblemStandardMessage(names.RDS, create.ErrActionDeleting, ResNameCluster, data.ID.ValueString(), err),
			err.Error(),
		)
		return
	}
	...
	if _, err := waitDBClusterDeleted(ctx, conn, data.ID.ValueString(), deleteTimeout); err != nil {
		response.Diagnostics.AddError(
			create.ProblemStandardMessage(names.RDS, create.ErrActionWaitingForDeletion, ResNameCluster, data.ID.ValueString(), err),
			err.Error(),
		)
		return
	}
	...
	return map[int64]resource.StateUpgrader{
		0: {
			...
			StateUpgrader: func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
				var priorStateData resourceClusterData
				...
				if priorStateData.GlobalClusterIdentifier.ValueString() == "" {
					upgradedStateData.GlobalClusterIdentifier = types.StringNull()
				}
...
func restoreFromDBSnapshotInput(ctx context.Context, identifier string, data resourceClusterData) *rds.RestoreDBClusterFromSnapshotInput {
...
func restoreFromS3ImportInput(ctx context.Context, identifier string, importValues s3Import, data resourceClusterData) *rds.RestoreDBClusterFromS3Input {
...
func createClusterInput(ctx context.Context, identifier string, data resourceClusterData) *rds.CreateDBClusterInput {
...
func statusDBCluster(ctx context.Context, conn *rds.RDS, id string) sdkresource.StateRefreshFunc {
