...
const (
	packageName = "pricingplanmanager"
	awsEnvVar   = "AWS_ENDPOINT_URL_PRICING_PLAN_MANAGER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "pricing_plan_manager"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := pricingplanmanager.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, pricingplanmanager.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Pricing Plan Manager default endpoint: %s", err)
	}
