...
const (
	packageName = "sso"
	awsEnvVar   = "AWS_ENDPOINT_URL_SSO"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := sso.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, sso.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SSO (Single Sign-On) default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SSO (Single Sign-On) FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up SSO (Single Sign-On) endpoint %q: %s", hostname, err)
	}
