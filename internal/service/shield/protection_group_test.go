...
resource "aws_shield_protection_group" "test" {
  ...
  aggregation         = "MAX"
  pattern             = "ARBITRARY"
  members             = ["arn:${data.aws_partition.current.partition}:ec2:${data.aws_region.current.name}:${data.aws_sts_caller_identity.current.account_id}:eip-allocation/${aws_eip.test.id}"]
}
`, rName))
}
