		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccSecretVersionsCheckDataSource(datasourceName, resource1Name, ""),
				),
			},
			{
				Config: testAccSecretVersionsDataSourceConfig_twoVersions(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccSecretVersionsCheckDataSource(datasourceName, resource1Name, resource2Name),
				),
			},
		},
	})
...
func testAccSecretVersionsCheckDataSource(datasourceName, resource1Name, resource2Name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if resource2Name != "" {
			...
			if dataSource.Primary.Attributes["versions.#"] != "2" {
				return fmt.Errorf(
					"versions.# is %s; want 2",
