...
func testAccCheckTagDestroy(ctx context.Context, t *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_secretsmanager_tag" {
				continue
			}
			...
			_, err = tfsecretsmanager.FindSecretTag(ctx, conn, identifier, key)
			...
			return fmt.Errorf("%s resource (%s) tag (%s) still exists", names.SecretsManager, identifier, key)
		}
