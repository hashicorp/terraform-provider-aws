...
const (
	packageName = "kendra"
	awsEnvVar   = "AWS_ENDPOINT_URL_KENDRA"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := kendra.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, kendra.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Kendra default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Kendra FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Kendra endpoint %q: %s", hostname, err)
	}
