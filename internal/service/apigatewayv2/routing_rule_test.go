...
func TestParseRoutingRuleARN(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:                "valid ARN for private domain",
			input:               "arn:aws:apigateway:us-west-2:123456789012:/domainnames/example.com+abc123/routingrules/test", //lintignore:AWSAT003,AWSAT005
			expectDomainName:    "example.com",
			expectDomainNameId:  "abc123",
			expectRoutingRuleID: "test",
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			gotDomainName, gotDomainNameId, gotRoutingRuleID, err := tfapigatewayv2.ParseRoutingRuleARN(testCase.input)
			...
			if got, want := gotDomainNameId, testCase.expectDomainNameId; got != want {
				t.Errorf("ParseRoutingRuleARN(%q) DomainNameId = %v, want = %v", testCase.input, got, want)
			}
...
func TestAccAPIGatewayV2RoutingRule_privateAPI(t *testing.T) {
	ctx := acctest.Context(t)
	var routingrule apigatewayv2.GetRoutingRuleOutput
	rName := acctest.RandomWithPrefix(t, acctest.ResourcePrefix)
	key := acctest.TLSRSAPrivateKeyPEM(t, 2048)
	...
	certificate := acctest.TLSRSAX509SelfSignedCertificatePEM(t, key, domainName)
	resourceName := "aws_apigatewayv2_routing_rule.test"
	domainNameResourceName := "aws_api_gateway_domain_name.test"
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRoutingRuleConfig_privateAPI(rName, certificate, key, 1, 0),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckRoutingRuleExists(ctx, t, resourceName, &routingrule),
					resource.TestCheckResourceAttr(resourceName, names.AttrPriority, "1"),
					resource.TestCheckResourceAttr(resourceName, names.AttrDomainName, domainName),
					resource.TestCheckResourceAttrPair(resourceName, "domain_name_id", domainNameResourceName, "domain_name_id"),
					acctest.MatchResourceAttrRegionalARN(ctx, resourceName, "routing_rule_arn", "apigateway", regexache.MustCompile(`/domainnames/.+/routingrules/.+$`)),
				),
			},
			{
...
func testAccCheckRoutingRuleDestroy(ctx context.Context, t *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			_, err := tfapigatewayv2.FindRoutingRuleByThreePartKey(ctx, conn, rs.Primary.Attributes[names.AttrDomainName], rs.Primary.Attributes["domain_name_id"], rs.Primary.Attributes["routing_rule_id"])
...
func testAccCheckRoutingRuleExists(ctx context.Context, t *testing.T, n string, v *apigatewayv2.GetRoutingRuleOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		output, err := tfapigatewayv2.FindRoutingRuleByThreePartKey(ctx, conn, rs.Primary.Attributes[names.AttrDomainName], rs.Primary.Attributes["domain_name_id"], rs.Primary.Attributes["routing_rule_id"])
...
func testAccRoutingRuleConfig_privateAPI(rName, certificate, key string, count, index int) string {
	return acctest.ConfigCompose(
		testAccDomainNameImportedCertsConfig(rName, certificate, key, count),
		fmt.Sprintf(`
resource "aws_api_gateway_domain_name" "test" {
...
resource "aws_apigatewayv2_routing_rule" "test" {
  depends_on = [aws_api_gateway_stage.test]
