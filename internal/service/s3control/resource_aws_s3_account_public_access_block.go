...
func resourceAccountPublicAccessBlockRead(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(tfs3control.propagationTimeout, func() *resource.RetryError {
		var err error
...
func resourceAccountPublicAccessBlockUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("block_public_acls") {
		if _, err := tfs3control.waitPublicAccessBlockConfigurationBlockPublicACLsUpdated(conn, d.Id(), d.Get("block_public_acls").(bool)); err != nil {
	...
	if d.HasChange("block_public_policy") {
		if _, err := tfs3control.waitPublicAccessBlockConfigurationBlockPublicPolicyUpdated(conn, d.Id(), d.Get("block_public_policy").(bool)); err != nil {
	...
	if d.HasChange("ignore_public_acls") {
		if _, err := tfs3control.waitPublicAccessBlockConfigurationIgnorePublicACLsUpdated(conn, d.Id(), d.Get("ignore_public_acls").(bool)); err != nil {
	...
	if d.HasChange("restrict_public_buckets") {
		if _, err := tfs3control.waitPublicAccessBlockConfigurationRestrictPublicBucketsUpdated(conn, d.Id(), d.Get("restrict_public_buckets").(bool)); err != nil {
