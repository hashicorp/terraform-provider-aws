...
const (
	packageName = "resiliencehubv2"
	awsEnvVar   = "AWS_ENDPOINT_URL_RESILIENCEHUBV2"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "resiliencehubv2"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := resiliencehubv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, resiliencehubv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ResilienceHubV2Client(ctx)

	var result apiCallParams

	input := resiliencehubv2.ListPoliciesInput{}
	_, err := client.ListPolicies(ctx, &input,
		func(opts *resiliencehubv2.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Resilience Hub V2 default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Resilience Hub V2 FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Resilience Hub V2 endpoint %q: %s", hostname, err)
	}
