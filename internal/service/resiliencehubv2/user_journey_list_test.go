...
func TestAccResilienceHubV2UserJourney_List_basic(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("testdata/UserJourney/list_basic/"),
			...
			},
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					tfquerycheck.ExpectNoResourceObject("aws_resiliencehubv2_user_journey.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks())),

					tfquerycheck.ExpectIdentityFunc("aws_resiliencehubv2_user_journey.test", identity2.Checks()),
					querycheck.ExpectResourceDisplayName("aws_resiliencehubv2_user_journey.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks()), knownvalue.StringExact(rName+"-1")),
					tfquerycheck.ExpectNoResourceObject("aws_resiliencehubv2_user_journey.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks())),
				},
			},
		},
	})
...
func TestAccResilienceHubV2UserJourney_List_includeResource(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("testdata/UserJourney/list_include_resource/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
...
func TestAccResilienceHubV2UserJourney_List_regionOverride(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("testdata/UserJourney/list_region_override/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
					"resource_count": config.IntegerVariable(2),
