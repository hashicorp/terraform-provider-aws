...
func TestAccResilienceHubV2ServiceFunction_basic(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccServiceFunctionConfig_basic(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckServiceFunctionExists(ctx, t, resourceName, &sf),
					resource.TestCheckResourceAttr(resourceName, names.AttrName, rName),
					resource.TestCheckResourceAttr(resourceName, "criticality", "PRIMARY"),
					resource.TestCheckResourceAttrSet(resourceName, "service_function_id"),
				),
			},
		},
	})
...
func testAccServiceFunctionConfig_basic(rName string) string {
	return fmt.Sprintf(`
