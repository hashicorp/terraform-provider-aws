...
const (
	packageName = "events"
	awsEnvVar   = "AWS_ENDPOINT_URL_EVENTBRIDGE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "eventbridge"

	aliasName0 = "eventbridge"
	aliasName1 = "cloudwatchevents"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := eventbridge.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, eventbridge.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving EventBridge default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving EventBridge FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up EventBridge endpoint %q: %s", hostname, err)
	}
