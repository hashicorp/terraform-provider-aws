...
const (
	packageName = "ssmincidents"
	awsEnvVar   = "AWS_ENDPOINT_URL_SSM_INCIDENTS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "ssm_incidents"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := ssmincidents.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, ssmincidents.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SSMIncidentsClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SSM Incident Manager Incidents default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving SSM Incident Manager Incidents FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up SSM Incident Manager Incidents endpoint %q: %s", hostname, err)
	}
