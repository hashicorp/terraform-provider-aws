		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					acctest.MatchResourceAttrRegionalARN(ctx, resourceName, names.AttrARN, "redshift-serverless", regexache.MustCompile(`snapshotcopyconfiguration/+.`)),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "namespace_name", rName),
					resource.TestCheckResourceAttr(resourceName, "destination_region", acctest.AlternateRegion()),
					acctest.MatchResourceAttrRegionalARNRegion(ctx, resourceName, "destination_kms_key_id", "kms", acctest.AlternateRegion(), regexache.MustCompile(`key/+.`)),
				),
			},
		},
	})
...
func testAccCheckSnapshotCopyConfigurationDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			_, errNamespace := tfredshiftserverless.FindNamespaceByName(ctx, conn, rs.Primary.Attributes["namespace_name"])
			...
			_, err := tfredshiftserverless.FindSnapshotCopyConfigurationByID(ctx, conn, rs.Primary.Attributes["namespace_name"], rs.Primary.Attributes[names.AttrID])
...
func testAccCheckSnapshotCopyConfigurationExists(ctx context.Context, name string, snapshotCopyConfiguration *types.SnapshotCopyConfiguration) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*conns.AWSClient).RedshiftServerlessClient(ctx)
		output, err := tfredshiftserverless.FindSnapshotCopyConfigurationByID(ctx, conn, rs.Primary.Attributes["namespace_name"], rs.Primary.Attributes[names.AttrID])
