...
func sweepEndpointAccess(region string) error {
	client, err := sweep.SharedRegionalSweepClient(region)
	if err != nil {
		return fmt.Errorf("error getting client: %w", err)
	}
	...
	err = conn.ListEndpointAccessPages(input, func(page *redshiftserverless.ListEndpointAccessOutput, lastPage bool) bool {
		if len(page.Endpoints) == 0 {
			log.Print("[DEBUG] No Redshift Serverless Endpoint Access to sweep")
			return !lastPage
		}

		for _, endpoint := range page.Endpoints {
			r := ResourceEndpointAccess()
			d := r.Data(nil)
	...
	if err != nil {
		errs = multierror.Append(errs, fmt.Errorf("error describing Redshift Serverless Endpoint Access: %w", err))
	}

	if err = sweep.SweepOrchestrator(sweepResources); err != nil {
		errs = multierror.Append(errs, fmt.Errorf("error sweeping Redshift Serverless Endpoint Access for %s: %w", region, err))
	}

	if sweep.SkipSweepError(errs.ErrorOrNil()) {
		log.Printf("[WARN] Skipping Redshift Serverless Endpoint Access sweep for %s: %s", region, errs)
		return nil
	}
