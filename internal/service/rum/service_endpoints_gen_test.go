...
const (
	packageName = "rum"
	awsEnvVar   = "AWS_ENDPOINT_URL_RUM"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "rum"

	aliasName0 = "cloudwatchrum"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := rum.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, rum.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CloudWatch RUM default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CloudWatch RUM FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up CloudWatch RUM endpoint %q: %s", hostname, err)
	}
