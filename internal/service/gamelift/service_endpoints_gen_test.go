...
func defaultEndpoint(region string) (url.URL, error) {
	...
	ep, err := r.EndpointFor(gamelift_sdkv1.EndpointsID, region)
	if err != nil {
		return url.URL{}, err
	}
...
func defaultFIPSEndpoint(region string) (url.URL, error) {
	...
	ep, err := r.EndpointFor(gamelift_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.GameLiftConn(ctx)

	req, _ := client.ListGameServerGroupsRequest(&gamelift_sdkv1.ListGameServerGroupsInput{})
