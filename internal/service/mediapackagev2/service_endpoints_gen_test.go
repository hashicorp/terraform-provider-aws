...
const (
	packageName = "mediapackagev2"
	awsEnvVar   = "AWS_ENDPOINT_URL_MEDIAPACKAGEV2"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "mediapackagev2"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := mediapackagev2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, mediapackagev2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MediaPackageV2Client(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Elemental MediaPackage Version 2 default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Elemental MediaPackage Version 2 FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Elemental MediaPackage Version 2 endpoint %q: %s", hostname, err)
	}
