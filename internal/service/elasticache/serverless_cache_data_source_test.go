...
func TestAccElastiCacheServerlessCacheDataSource_Redis_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccServerlessCacheDataSourceConfig_Redis_basic(rName),
...
func TestAccElastiCacheServerlessCacheDataSource_Valkey_basic(t *testing.T) {
	ctx := acctest.Context(t)
	if testing.Short() {
		t.Skip("skipping long-running test in short mode")
	}
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccServerlessCacheDataSourceConfig_Valkey_basic(rName),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrPair(dataSourceName, names.AttrARN, resourceName, names.AttrARN),
					resource.TestCheckResourceAttrPair(dataSourceName, names.AttrCreateTime, resourceName, names.AttrCreateTime),
					resource.TestCheckResourceAttrPair(dataSourceName, "daily_snapshot_time", resourceName, "daily_snapshot_time"),
...
func testAccServerlessCacheDataSourceConfig_Redis_basic(rName string) string {
	return fmt.Sprintf(`
...
func testAccServerlessCacheDataSourceConfig_Valkey_basic(rName string) string {
	return fmt.Sprintf(`
resource "aws_elasticache_serverless_cache" "test" {
  engine = "valkey"
  name   = %[1]q
