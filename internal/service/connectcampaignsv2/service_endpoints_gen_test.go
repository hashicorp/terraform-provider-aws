...
const (
	packageName = "connectcampaignsv2"
	awsEnvVar   = "AWS_ENDPOINT_URL_CONNECTCAMPAIGNSV2"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "connectcampaignsv2"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := connectcampaignsv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, connectcampaignsv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ConnectCampaignsV2Client(ctx)

	var result apiCallParams

	input := connectcampaignsv2.ListCampaignsInput{}
	_, err := client.ListCampaigns(ctx, &input,
		func(opts *connectcampaignsv2.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Connect Campaigns V2 default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Connect Campaigns V2 FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Connect Campaigns V2 endpoint %q: %s", hostname, err)
	}
