...
func (p *servicePackage) NewConn(ctx context.Context) (*route53_sdkv1.Route53, error) {
	...
	case endpoints_sdkv1.AwsCnPartitionID:
		...
		if aws.StringValue(config.Endpoint) == "" {
			config.Endpoint = aws.String("https://api.route53.cn")
		}
		config.Region = aws.String(endpoints_sdkv1.CnNorthwest1RegionID)
	case endpoints_sdkv1.AwsUsGovPartitionID:
		config.Region = aws.String(endpoints_sdkv1.UsGovWest1RegionID)
	}
