...
func TestAccRoute53Record_List_TrailingDot_basic(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/Record/list_trailingdot/"),
				ConfigVariables: config.Variables{
					"subdomainName":  config.StringVariable(subdomainName),
					"zoneName":       config.StringVariable(zoneName),
					"resource_count": config.IntegerVariable(2),
...
func TestAccRoute53Record_List_TrailingDot_includeResource(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/Record/list_trailingdot_include_resource/"),
				ConfigVariables: config.Variables{
					"subdomainName":  config.StringVariable(subdomainName),
					"zoneName":       config.StringVariable(zoneName),
					"resource_count": config.IntegerVariable(1),
