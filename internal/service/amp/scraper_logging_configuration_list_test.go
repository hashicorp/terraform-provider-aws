...
func TestAccAMPScraperLoggingConfiguration_List_basic(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/ScraperLoggingConfiguration/list_basic/"),
				...
				ConfigStateChecks: []statecheck.StateCheck{
					...
					statecheck.ExpectKnownValue(resourceName1, tfjsonpath.New("scraper_id"), knownvalue.NotNull()),

					identity2.GetIdentity(resourceName2),
					statecheck.ExpectKnownValue(resourceName2, tfjsonpath.New("scraper_id"), knownvalue.NotNull()),
				},
			},
...
func TestAccAMPScraperLoggingConfiguration_List_includeResource(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/ScraperLoggingConfiguration/list_include_resource/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					querycheck.ExpectResourceKnownValues("aws_amp_scraper_logging_configuration.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), []querycheck.KnownValueCheck{
						...
						tfquerycheck.KnownValueCheck(tfjsonpath.New("scraper_components"), tfknownvalue.SetNotEmpty()),
						tfquerycheck.KnownValueCheck(tfjsonpath.New("scraper_id"), knownvalue.NotNull()),
					}),
				},
			},
		},
...
func TestAccAMPScraperLoggingConfiguration_List_regionOverride(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/ScraperLoggingConfiguration/list_region_override/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
					"resource_count": config.IntegerVariable(2),
