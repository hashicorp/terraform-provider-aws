...
func TestAccCodeConnectionsConnectionDataSource_basic(t *testing.T) {
	ctx := acctest.Context(t)
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	dataSourceName := "data.aws_codeconnections_connection.test_arn"
	dataSourceName2 := "data.aws_codeconnections_connection.test_name"
...
func TestAccCodeConnectionsConnectionDataSource_tags(t *testing.T) {
	ctx := acctest.Context(t)
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	dataSourceName := "data.aws_codeconnections_connection.test"
	resourceName := "aws_codeconnections_connection.test"
...
data "aws_codeconnections_connection" "test_arn" {
  arn = aws_codeconnections_connection.test.arn
}

data "aws_codeconnections_connection" "test_name" {
  name = aws_codeconnections_connection.test.name
}
`, rName)
}
...
data "aws_codeconnections_connection" "test" {
  arn = aws_codeconnections_connection.test.arn
}
`, rName)
}
