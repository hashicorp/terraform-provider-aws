		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrID, "code-connections", regexache.MustCompile("connection/.+")),
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrARN, "code-connections", regexache.MustCompile("connection/.+")),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrID, "code-connections", regexache.MustCompile("connection/.+")),
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrARN, "code-connections", regexache.MustCompile("connection/.+")),
					acctest.MatchResourceAttrRegionalARN(resourceName, "host_arn", "code-connections", regexache.MustCompile("host/.+")),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckConnectionExists(ctx, resourceName, &v),
					acctest.CheckResourceDisappears(ctx, acctest.Provider, tfcodeconnections.ResourceConnection(), resourceName),
				),
...
func testAccCheckConnectionDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			return fmt.Errorf("CodeConnections Connection %s still exists", rs.Primary.ID)
		}
