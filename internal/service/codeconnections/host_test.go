...
func TestAccCodeConnectionsHost_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			...
			acctest.PreCheckPartitionHasService(t, names.CodeConnectionsEndpointID)
		},
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrID, "code-connections", regexache.MustCompile("host/.+")),
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrARN, "code-connections", regexache.MustCompile("host/.+")),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckHostExists(ctx, resourceName, &v),
					acctest.CheckResourceDisappears(ctx, acctest.Provider, tfcodeconnections.ResourceHost(), resourceName),
				),
...
func testAccCheckHostDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			return fmt.Errorf("CodeConnections Host %s still exists", rs.Primary.ID)
		}
