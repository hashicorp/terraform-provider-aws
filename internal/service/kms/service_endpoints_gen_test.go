...
const (
	packageName = "kms"
	awsEnvVar   = "AWS_ENDPOINT_URL_KMS"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := kms.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, kms.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving KMS (Key Management) default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving KMS (Key Management) FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up KMS (Key Management) endpoint %q: %s", hostname, err)
	}
