...
const (
	packageName = "bcmdashboards"
	awsEnvVar   = "AWS_ENDPOINT_URL_BCM_DASHBOARDS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "bcm_dashboards"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := bcmdashboards.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, bcmdashboards.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving BCM Dashboards default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving BCM Dashboards FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up BCM Dashboards endpoint %q: %s", hostname, err)
	}
