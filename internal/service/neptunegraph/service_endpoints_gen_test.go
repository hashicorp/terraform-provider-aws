...
const (
	packageName = "neptunegraph"
	awsEnvVar   = "AWS_ENDPOINT_URL_NEPTUNE_GRAPH"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "neptune_graph"
)
...
func defaultEndpoint(region string) string {
	r := neptunegraph_sdkv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), neptunegraph_sdkv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.NeptuneGraphClient(ctx)

	_, err := client.ListGraphs(ctx, &neptunegraph_sdkv2.ListGraphsInput{},
		func(opts *neptunegraph_sdkv2.Options) {
