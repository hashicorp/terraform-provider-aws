...
func (p *servicePackage) NewConn(ctx context.Context) (*detective_sdkv1.Detective, error) {
	sess := p.config["session"].(*session_sdkv1.Session)

	return detective_sdkv1.New(sess.Copy(&aws_sdkv1.Config{Endpoint: aws_sdkv1.String(p.config["endpoint"].(string))})), nil
}
