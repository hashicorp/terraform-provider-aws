...
func defaultEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(detective_sdkv1.EndpointsID, region)
	if err != nil {
		return err.Error()
	}
...
func defaultFIPSEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(detective_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.DetectiveConn(ctx)

	req, _ := client.ListGraphsRequest(&detective_sdkv1.ListGraphsInput{})
