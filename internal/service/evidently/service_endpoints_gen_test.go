...
const (
	packageName = "evidently"
	awsEnvVar   = "AWS_ENDPOINT_URL_EVIDENTLY"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "evidently"

	aliasName0 = "cloudwatchevidently"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := evidently.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, evidently.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.EvidentlyClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CloudWatch Evidently default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CloudWatch Evidently FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up CloudWatch Evidently endpoint %q: %s", hostname, err)
	}
