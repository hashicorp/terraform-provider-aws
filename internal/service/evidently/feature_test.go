		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFeatureExists(ctx, resourceName, &feature),
					resource.TestCheckResourceAttr(resourceName, "entity_overrides.%", acctest.CtOne),
...
func TestAccEvidentlyFeature_tags(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccFeatureConfig_tags2(rName, rName2, acctest.CtKey1, "value1updated", acctest.CtKey2, acctest.CtValue2),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFeatureExists(ctx, resourceName, &feature),
