...
const (
	packageName = "s3files"
	awsEnvVar   = "AWS_ENDPOINT_URL_S3FILES"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := s3files.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, s3files.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.S3FilesClient(ctx)

	var result apiCallParams

	input := s3files.ListFileSystemsInput{}
	_, err := client.ListFileSystems(ctx, &input,
		func(opts *s3files.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving S3 Files default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving S3 Files FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up S3 Files endpoint %q: %s", hostname, err)
	}
