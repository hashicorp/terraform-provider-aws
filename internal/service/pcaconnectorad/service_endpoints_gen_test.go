...
const (
	packageName = "pcaconnectorad"
	awsEnvVar   = "AWS_ENDPOINT_URL_PCA_CONNECTOR_AD"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "pca_connector_ad"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := pcaconnectorad.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, pcaconnectorad.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.PCAConnectorADClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Private CA Connector for Active Directory default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Private CA Connector for Active Directory FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Private CA Connector for Active Directory endpoint %q: %s", hostname, err)
	}
