...
variable "third_rname" {
  type    = string
  default = "tf-neptune-test-tertiary"
}
...
resource "aws_subnet" "third" {
  ...
  availability_zone = data.aws_availability_zones.third.names[count.index]
  cidr_block        = "10.0.${count.index}.0/24"
...
resource "aws_neptune_subnet_group" "third" {
  provider = awsthird

  name       = var.third_rname
  subnet_ids = aws_subnet.third[*].id
}

resource "aws_neptune_cluster" "tertiary" {
  provider = awsthird

  apply_immediately         = true
  cluster_identifier        = var.third_rname
  engine                    = aws_neptune_cluster_instance.primary.engine
  engine_version            = aws_neptune_cluster_instance.primary.engine_version
  global_cluster_identifier = aws_neptune_cluster.secondary.global_cluster_identifier
  neptune_subnet_group_name = aws_neptune_subnet_group.third.name
  skip_final_snapshot       = true
...
resource "aws_neptune_cluster_instance" "tertiary" {
  provider = awsthird

  apply_immediately  = true
  cluster_identifier = aws_neptune_cluster.tertiary.cluster_identifier
  engine_version     = aws_neptune_global_cluster.test.engine_version
  identifier         = var.third_rname
  instance_class     = "db.r6g.large"
}
