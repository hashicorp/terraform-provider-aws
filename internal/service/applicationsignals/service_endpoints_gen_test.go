...
const (
	packageName = "applicationsignals"
	awsEnvVar   = "AWS_ENDPOINT_URL_APPLICATION_SIGNALS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "application_signals"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := applicationsignals.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, applicationsignals.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ApplicationSignalsClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Application Signals default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Application Signals FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Application Signals endpoint %q: %s", hostname, err)
	}
