...
func resourceWebhookRead(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && tfawserr.ErrCodeEquals(err, codebuild.ErrCodeResourceNotFoundException) {
		create.LogNotFoundRemoveState(names.CodeBuild, create.ErrActionReading, ResWebhook, d.Id())
		d.SetId("")
		return nil
	}

	if err != nil {
		return create.Error(names.CodeBuild, create.ErrActionReading, ResWebhook, d.Id(), err)
	}

	if d.IsNewResource() && len(resp.Projects) == 0 {
		return create.Error(names.CodeBuild, create.ErrActionReading, ResWebhook, d.Id(), errors.New("no project found after create"))
	}
	...
	if d.IsNewResource() && project.Webhook == nil {
		return create.Error(names.CodeBuild, create.ErrActionReading, ResWebhook, d.Id(), errors.New("no webhook after creation"))
	}
