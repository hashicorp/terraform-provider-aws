...
func (p *servicePackage) NewConn(ctx context.Context) (*codebuild_sdkv1.CodeBuild, error) {
	sess := p.config["session"].(*session_sdkv1.Session)

	return codebuild_sdkv1.New(sess.Copy(&aws_sdkv1.Config{Endpoint: aws_sdkv1.String(p.config["endpoint"].(string))})), nil
}
