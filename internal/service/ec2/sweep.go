...
func sweepIPAMPoolCIDRAllocations(region string) error {
		...
		for _, v := range page.IpamPools {
			...
			err := conn.GetIpamPoolAllocationsPages(input, func(page *ec2.GetIpamPoolAllocationsOutput, lastPage bool) bool {
				...
				for _, v := range page.IpamPoolAllocations {
					poolAllocationID := aws.StringValue(v.IpamPoolAllocationId)

					r := ResourceVPCIpamPoolCidrAllocation()
					d := r.Data(nil)
					d.SetId(encodeIpamPoolCidrAllocationID(poolAllocationID, poolID))
					d.Set("cidr", v.Cidr)
					d.Set("ipam_pool_allocation_id", poolAllocationID)
			...
			if err != nil {
				sweeperErrs = multierror.Append(sweeperErrs, fmt.Errorf("error listing IPAM Pool CIDR Allocations (%s): %w", region, err))
			}
		}
	...
	if sweep.SkipSweepError(err) {
		log.Printf("[WARN] Skipping IPAM Pool CIDR Allocations sweep for %s: %s", region, err)
		return sweeperErrs.ErrorOrNil()
	}
	...
	if err != nil {
		sweeperErrs = multierror.Append(sweeperErrs, fmt.Errorf("error sweeping IPAM Pool CIDR Allocations (%s): %w", region, err))
	}
