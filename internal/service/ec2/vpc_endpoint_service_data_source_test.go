...
func TestAccVPCEndpointServiceDataSource_ServiceType_gateway(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVPCEndpointServiceDataSourceConfig_type(names.AttrS3, "Gateway"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckResourceAttrRegionalReverseDNSService(datasourceName, names.AttrServiceName, names.AttrS3),
					resource.TestCheckResourceAttr(datasourceName, "service_type", "Gateway"),
				),
			},
		},
