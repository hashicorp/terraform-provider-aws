...
const (
	customerGatewayAvailableTimeout = 10 * time.Minute
	customerGatewayDeletedTimeout   = 5 * time.Minute
)

func waitCustomerGatewayAvailable(conn *ec2.EC2, id string) (*ec2.CustomerGateway, error) {
	stateConf := &resource.StateChangeConf{
		Pending:    []string{CustomerGatewayStatePending},
		Target:     []string{CustomerGatewayStateAvailable},
		Timeout:    customerGatewayAvailableTimeout,
		Delay:      10 * time.Second,
		MinTimeout: 3 * time.Second,
...
func waitCustomerGatewayDeleted(conn *ec2.EC2, id string) (*ec2.CustomerGateway, error) {
	stateConf := &resource.StateChangeConf{
		Pending: []string{CustomerGatewayStatePending, CustomerGatewayStateAvailable, CustomerGatewayStateDeleting},
		Target:  []string{},
