...
func resourceRouteTableCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err := tfec2.WaitRouteTableReady(conn, d.Id(), d.Timeout(schema.TimeoutCreate)); err != nil {
...
func resourceRouteTableRead(d *schema.ResourceData, meta interface{}) error {
	...
	routeTable, err := tfec2.FindRouteTableByID(conn, d.Id())
...
func resourceRouteTableDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err := tfec2.WaitRouteTableDeleted(conn, d.Id(), d.Timeout(schema.TimeoutDelete)); err != nil {
...
func ec2RouteTableAddRoute(conn *ec2.EC2, routeTableID string, tfMap map[string]interface{}, timeout time.Duration) error {
	...
	var routeFinder tfec2.RouteFinder

	switch destinationAttributeKey {
	case "cidr_block":
		routeFinder = tfec2.FindRouteByIPv4Destination
	case "ipv6_cidr_block":
		routeFinder = tfec2.FindRouteByIPv6Destination
	case "destination_prefix_list_id":
		routeFinder = tfec2.FindRouteByPrefixListIDDestination
	default:
	...
	_, err = tfec2.WaitRouteReady(conn, routeFinder, routeTableID, destination, timeout)
...
func ec2RouteTableDeleteRoute(conn *ec2.EC2, routeTableID string, tfMap map[string]interface{}, timeout time.Duration) error {
	...
	_, err = tfec2.WaitRouteDeleted(conn, routeFinder, routeTableID, destination, timeout)
