...
func testAccCheckEBSVolumeCopyDestroy(ctx context.Context, _ *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		return testAccCheckEBSVolumeCopyDestroyWithProvider(ctx)(s, acctest.Provider)
	}
}
...
resource "aws_ebs_volume_copy" "test" {
  encrypted        = false
  kms_key_id       = "arn:${data.aws_partition.current.partition}:kms:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:key/12345678-1234-1234-1234-123456789012"
