...
func dataSourceSubnetIDsCategorizedRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Println("[INFO] aws_subnet_ids_categorized")
...
func getCategorizedSubnets(conn *ec2.EC2, vpcId string) (public []string, private []string, err error) {
	...
	log.Println("[INFO] IGW retrieved")
	...
	log.Printf("[INFO] Route tables retrieved: %d\n", len(routeTables))

	publicSubnetIds, privateSubnetIds := categorizeSubnetIds(igw, routeTables, allSubnetIds)

	log.Printf("[INFO] Public subnets: %+q\n", publicSubnetIds)
	log.Printf("[INFO] Private subnets: %+q\n", privateSubnetIds)
...
// Returns: Two arrays - one of public subnet IDs and one of private subnet IDs.
func categorizeSubnetIds(igw *ec2.InternetGateway, routeTables []*ec2.RouteTable, allSubnetIds []string) (public []string, private []string) {
	...
	for _, t := range routeTables {
		for _, r := range t.Routes {
			if r.GatewayId != nil && *r.GatewayId == *igw.InternetGatewayId {
				publicRouteTable = t
				break
			}
		}
