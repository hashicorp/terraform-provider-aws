		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "dns_entry.#", acctest.CtZero),
					resource.TestCheckResourceAttr(resourceName, "dns_options.#", acctest.CtZero),
					resource.TestCheckResourceAttr(resourceName, names.AttrIPAddressType, ""),
					resource.TestCheckResourceAttr(resourceName, "network_interface_ids.#", acctest.CtZero),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckVPCEndpointExists(ctx, resourceName, &endpoint),
					acctest.MatchResourceAttrRegionalARN(resourceName, names.AttrARN, "ec2", regexache.MustCompile(`vpc-endpoint/vpce-.+`)),
					resource.TestCheckResourceAttr(resourceName, "cidr_blocks.#", acctest.CtZero),
					resource.TestCheckResourceAttr(resourceName, "dns_entry.#", acctest.CtZero),
...
func TestAccVPCEndpoint_tags(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccVPCEndpointConfig_tags2(rName, acctest.CtKey1, "value1updated", acctest.CtKey2, acctest.CtValue2),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVPCEndpointExists(ctx, resourceName, &endpoint),
