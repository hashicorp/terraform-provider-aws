...
const (
	packageName = "cognitoidp"
	awsEnvVar   = "AWS_ENDPOINT_URL_COGNITO_IDENTITY_PROVIDER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cognito_identity_provider"

	aliasName0 = "cognitoidentityprovider"
)
...
func defaultEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(cognitoidentityprovider_sdkv1.EndpointsID, region)
	if err != nil {
		return err.Error()
	}
...
func defaultFIPSEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(cognitoidentityprovider_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.CognitoIDPConn(ctx)

	req, _ := client.ListUserPoolsRequest(&cognitoidentityprovider_sdkv1.ListUserPoolsInput{})
