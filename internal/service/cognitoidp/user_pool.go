...
func resourceUserPoolRead(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && tfawserr.ErrCodeEquals(err, cognitoidentityprovider.ErrCodeResourceNotFoundException) {
		create.LogNotFoundRemoveState(names.CognitoIDP, create.ErrActionReading, ResUserPool, d.Id())
		d.SetId("")
		return nil
	}

	if err != nil {
		return create.Error(names.CognitoIDP, create.ErrActionReading, ResUserPool, d.Id(), err)
	}
