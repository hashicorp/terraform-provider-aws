...
func resourceResourceServerRead(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && tfawserr.ErrCodeEquals(err, cognitoidentityprovider.ErrCodeResourceNotFoundException) {
		create.LogNotFoundRemoveState(names.CognitoIDP, create.ErrActionReading, ResResourceServer, d.Id())
		d.SetId("")
		return nil
	}

	if err != nil {
		return create.Error(names.CognitoIDP, create.ErrActionReading, ResResourceServer, d.Id(), err)
	}
	...
	if d.IsNewResource() && (resp == nil || resp.ResourceServer == nil) {
		return create.Error(names.CognitoIDP, create.ErrActionReading, ResResourceServer, d.Id(), errors.New("not found after creation"))
	}
