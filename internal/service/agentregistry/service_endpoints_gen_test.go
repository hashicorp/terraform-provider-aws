...
const (
	packageName = "agentregistry"
	awsEnvVar   = "AWS_ENDPOINT_URL_AGENT_REGISTRY_CONTROL"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "agent_registry_control"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := agentregistrycontrol.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, agentregistrycontrol.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Agent Registry default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Agent Registry FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Agent Registry endpoint %q: %s", hostname, err)
	}
