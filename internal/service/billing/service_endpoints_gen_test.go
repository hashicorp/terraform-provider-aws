...
const (
	packageName = "billing"
	awsEnvVar   = "AWS_ENDPOINT_URL_BILLING"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := billing.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, billing.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.BillingClient(ctx)

	var result apiCallParams

	input := billing.ListBillingViewsInput{}
	_, err := client.ListBillingViews(ctx, &input,
		func(opts *billing.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving billing default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving billing FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up billing endpoint %q: %s", hostname, err)
	}
