...
const (
	packageName = "cloudfrontkeyvaluestore"
	awsEnvVar   = "AWS_ENDPOINT_URL_CLOUDFRONT_KEYVALUESTORE"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cloudfront_keyvaluestore"
)
...
func defaultEndpoint(region string) string {
	r := cloudfrontkeyvaluestore_sdkv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), cloudfrontkeyvaluestore_sdkv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.CloudFrontKeyValueStoreClient(ctx)

	_, err := client.ListKeys(ctx, &cloudfrontkeyvaluestore_sdkv2.ListKeysInput{},
		func(opts *cloudfrontkeyvaluestore_sdkv2.Options) {
