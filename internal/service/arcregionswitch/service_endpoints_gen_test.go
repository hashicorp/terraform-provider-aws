...
const (
	packageName = "arcregionswitch"
	awsEnvVar   = "AWS_ENDPOINT_URL_ARC_REGION_SWITCH"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "arc_region_switch"
)
...
func TestEndpointConfiguration(t *testing.T) { //nolint:paralleltest // uses t.Setenv
	...
	testcases := map[string]endpointTestCase{
		// Skip: ARC Region Switch service uses non-standard endpoint format
		// Expected: arc-region-switch.region.api.aws
		// Actual: arc-region-switch-control-plane.region.api.aws
		// "no config": {
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := arcregionswitch.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, arcregionswitch.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ARCRegionSwitchClient(ctx)

	var result apiCallParams

	input := arcregionswitch.ListPlansInput{}
	_, err := client.ListPlans(ctx, &input,
		func(opts *arcregionswitch.Options) {
