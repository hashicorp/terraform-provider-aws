...
const (
	packageName = "interconnect"
	awsEnvVar   = "AWS_ENDPOINT_URL_INTERCONNECT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "interconnect"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := interconnect.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, interconnect.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Interconnect default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Interconnect FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Interconnect endpoint %q: %s", hostname, err)
	}
