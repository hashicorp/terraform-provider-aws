			...
			if filter != nil {
				if filter.And != nil {
					...
					if len(filter.And.Tags) > 0 {
						rule["tags"] = tftags.S3KeyValueTags(filter.And.Tags).IgnoreAws().Map()
					}
				} else {
					// Prefix
					...
					// Tag
					if filter.Tag != nil {
						rule["tags"] = tftags.S3KeyValueTags([]*s3.Tag{filter.Tag}).IgnoreAws().Map()
					}
				}
			} else {
...
func resourceAwsS3BucketReplicationConfigurationUpdate(conn *s3.S3, d *schema.ResourceData) error {
	...
	for _, v := range rcRules {
		...
		if f, ok := rr["filter"].([]interface{}); ok && len(f) > 0 && f[0] != nil {
			...
			tags := tftags.New(filter["tags"]).IgnoreAws().S3Tags()
			if len(tags) > 0 {
...
func resourceAwsS3BucketLifecycleUpdate(conn *s3.S3, d *schema.ResourceData) error {
	...
	for i, lifecycleRule := range lifecycleRules {
		...
		// Filter
		tags := tftags.New(r["tags"]).IgnoreAws().S3Tags()
...
func flattenAwsS3BucketReplicationConfiguration(r *s3.ReplicationConfiguration) []map[string]interface{} {
	...
	for _, v := range r.Rules {
		...
		if f := v.Filter; f != nil {
			...
			if t := f.Tag; t != nil {
				m["tags"] = tftags.S3KeyValueTags([]*s3.Tag{t}).IgnoreAws().Map()
			}
			if a := f.And; a != nil {
				m["prefix"] = aws.StringValue(a.Prefix)
				m["tags"] = tftags.S3KeyValueTags(a.Tags).IgnoreAws().Map()
			}
