...
func resourceBucketUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("replication_configuration") {
		if err := resourceAwsS3BucketInternalReplicationConfigurationUpdate(conn, d); err != nil {
			return err
		}
	}
...
func resourceAwsS3BucketInternalReplicationConfigurationUpdate(conn *s3.S3, d *schema.ResourceData) error {
	bucket := d.Get("bucket").(string)
