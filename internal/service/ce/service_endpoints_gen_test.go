...
const (
	packageName = "ce"
	awsEnvVar   = "AWS_ENDPOINT_URL_COST_EXPLORER"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cost_explorer"

	aliasName0 = "costexplorer"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := costexplorer.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, costexplorer.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CE (Cost Explorer) default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CE (Cost Explorer) FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up CE (Cost Explorer) endpoint %q: %s", hostname, err)
	}
