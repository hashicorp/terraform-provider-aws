...
const (
	packageName = "mwaa"
	awsEnvVar   = "AWS_ENDPOINT_URL_MWAA"
...
func defaultEndpoint(region string) string {
	r := mwaa_sdkv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), mwaa_sdkv2.EndpointParameters{
...
func callServiceV2(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.MWAAClient(ctx)

	_, err := client.ListEnvironments(ctx, &mwaa_sdkv2.ListEnvironmentsInput{},
		func(opts *mwaa_sdkv2.Options) {
...
func callServiceV1(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.MWAAConn(ctx)

	req, _ := client.ListEnvironmentsRequest(&mwaa_sdkv1.ListEnvironmentsInput{})
