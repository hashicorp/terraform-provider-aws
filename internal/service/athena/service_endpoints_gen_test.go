...
const (
	packageName = "athena"
	awsEnvVar   = "AWS_ENDPOINT_URL_ATHENA"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := athena.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, athena.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Athena default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Athena FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Athena endpoint %q: %s", hostname, err)
	}
