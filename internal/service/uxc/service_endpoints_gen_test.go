...
const (
	packageName = "uxc"
	awsEnvVar   = "AWS_ENDPOINT_URL_UXC"
...
func TestEndpointConfiguration(t *testing.T) { //nolint:paralleltest // uses t.Setenv
	...
	// User Experience Customization uses a regional endpoint but is only available in one region or a limited number of regions.
	// The provider overrides the region for User Experience Customization, but the AWS SDK's endpoint resolution returns one for the current region.
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := uxc.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, uxc.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving User Experience Customization default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving User Experience Customization FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up User Experience Customization endpoint %q: %s", hostname, err)
	}
