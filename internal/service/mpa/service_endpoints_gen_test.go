...
const (
	packageName = "mpa"
	awsEnvVar   = "AWS_ENDPOINT_URL_MPA"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := mpa.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, mpa.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MPAClient(ctx)

	var result apiCallParams

	input := mpa.ListApprovalTeamsInput{}
	_, err := client.ListApprovalTeams(ctx, &input,
		func(opts *mpa.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Multi-party Approval default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Multi-party Approval FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Multi-party Approval endpoint %q: %s", hostname, err)
	}
