...
func TestAccLambdaMicroVMsMicroVM_List_basic(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/MicroVM/list_basic/"),
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					tfquerycheck.ExpectIdentityFunc("aws_lambdamicrovms_microvm.test", identity2.Checks()),
					querycheck.ExpectResourceDisplayName("aws_lambdamicrovms_microvm.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks()), knownvalue.NotNull()),
					tfquerycheck.ExpectNoResourceObject("aws_lambdamicrovms_microvm.test", tfqueryfilter.ByResourceIdentityFunc(identity2.Checks())),
				},
			},
		},
	})
...
func TestAccLambdaMicroVMsMicroVM_List_includeResource(t *testing.T) {
	ctx := acctest.Context(t)
	resourceName1 := "aws_lambdamicrovms_microvm.test[0]"
	rName := acctest.RandomWithPrefix(t, acctest.ResourcePrefix)
	identity1 := tfstatecheck.Identity()
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/MicroVM/list_include_resource/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
...
func TestAccLambdaMicroVMsMicroVM_List_regionOverride(t *testing.T) {
	ctx := acctest.Context(t)
	resourceName1 := "aws_lambdamicrovms_microvm.test[0]"
	resourceName2 := "aws_lambdamicrovms_microvm.test[1]"
	rName := acctest.RandomWithPrefix(t, acctest.ResourcePrefix)
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/MicroVM/list_region_override/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
					"resource_count": config.IntegerVariable(2),
