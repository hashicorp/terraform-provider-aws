...
const (
	packageName = "lambdamicrovms"
	awsEnvVar   = "AWS_ENDPOINT_URL_LAMBDA_MICROVMS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "lambda_microvms"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := lambdamicrovms.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, lambdamicrovms.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	_, err := client.ListMicrovms(ctx, &input,
		func(opts *lambdamicrovms.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Lambda MicroVMs default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Lambda MicroVMs FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Lambda MicroVMs endpoint %q: %s", hostname, err)
	}
