...
func TestAccECSDaemon_List_basic(t *testing.T) {
	...
	resourceName2 := "aws_ecs_daemon.test[1]"
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("testdata/Daemon/list_basic/"),
			...
			},
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					tfquerycheck.ExpectIdentityFunc("aws_ecs_daemon.test", identity2.Checks()),
				},
			},
		},
	})
...
func TestAccECSDaemon_List_regionOverride(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("testdata/Daemon/list_region_override/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
					"resource_count": config.IntegerVariable(2),
...
func TestAccECSDaemon_List_includeResource(t *testing.T) {
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				ConfigDirectory: config.StaticDirectory("testdata/Daemon/list_include_resource/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
			...
			},
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					querycheck.ExpectResourceKnownValues("aws_ecs_daemon.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), []querycheck.KnownValueCheck{
						...
						tfquerycheck.KnownValueCheck(tfjsonpath.New("capacity_provider_arns"), knownvalue.SetSizeExact(1)),
						...
						tfquerycheck.KnownValueCheck(tfjsonpath.New("daemon_task_definition_arn"), tfknownvalue.RegionalARNRegexp("ecs", regexache.MustCompile(`daemon-task-definition/`+rName+`:\d+$`))),
