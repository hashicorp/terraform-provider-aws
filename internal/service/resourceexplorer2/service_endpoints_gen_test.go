...
const (
	packageName = "resourceexplorer2"
	awsEnvVar   = "AWS_ENDPOINT_URL_RESOURCE_EXPLORER_2"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "resource_explorer_2"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := resourceexplorer2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, resourceexplorer2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ResourceExplorer2Client(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Resource Explorer default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Resource Explorer FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Resource Explorer endpoint %q: %s", hostname, err)
	}
