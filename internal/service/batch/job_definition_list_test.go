...
func TestAccBatchJobDefinition_List_includeResource(t *testing.T) {
	ctx := acctest.Context(t)
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/JobDefinition/list_include_resource/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					tfquerycheck.ExpectIdentityFunc("aws_batch_job_definition.test", identity1.Checks()),
					querycheck.ExpectResourceDisplayName("aws_batch_job_definition.test", tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), knownvalue.StringExact(rName+"-0")),
...
func TestAccBatchJobDefinition_List_regionOverride(t *testing.T) {
	ctx := acctest.Context(t)
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Setup
			{
				ConfigDirectory: config.StaticDirectory("testdata/JobDefinition/list_region_override/"),
				ConfigVariables: config.Variables{
					acctest.CtRName:  config.StringVariable(rName),
				...
				ConfigStateChecks: []statecheck.StateCheck{
					identity1.GetIdentity(resourceName1),
					statecheck.ExpectKnownValue(resourceName1, tfjsonpath.New(names.AttrARN), tfknownvalue.RegionalARNAlternateRegionExact("batch", "job-definition/"+rName+"-0:1")),

					identity2.GetIdentity(resourceName2),
					statecheck.ExpectKnownValue(resourceName2, tfjsonpath.New(names.AttrARN), tfknownvalue.RegionalARNAlternateRegionExact("batch", "job-definition/"+rName+"-1:1")),
				},
			},
			...
			// Step 2: Query
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					tfquerycheck.ExpectIdentityFunc("aws_batch_job_definition.test", identity1.Checks()),
					tfquerycheck.ExpectIdentityFunc("aws_batch_job_definition.test", identity2.Checks()),
				},
			},
		},
	})
