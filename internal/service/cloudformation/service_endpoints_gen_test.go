...
const (
	packageName = "cloudformation"
	awsEnvVar   = "AWS_ENDPOINT_URL_CLOUDFORMATION"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "cloudformation"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := cloudformation.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, cloudformation.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CloudFormation default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving CloudFormation FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up CloudFormation endpoint %q: %s", hostname, err)
	}
