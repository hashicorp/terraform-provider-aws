...
const (
	packageName = "securityagent"
	awsEnvVar   = "AWS_ENDPOINT_URL_SECURITYAGENT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "securityagent"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := securityagent.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, securityagent.EndpointParameters{
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Security Agent default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Security Agent FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Security Agent endpoint %q: %s", hostname, err)
	}
