...
const (
	packageName = "paymentcryptography"
	awsEnvVar   = "AWS_ENDPOINT_URL_PAYMENTCRYPTOGRAPHY"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "paymentcryptography"
)
...
func defaultEndpoint(region string) string {
	r := paymentcryptography_sdkv2.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(context.Background(), paymentcryptography_sdkv2.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.PaymentCryptographyClient(ctx)

	_, err := client.ListKeys(ctx, &paymentcryptography_sdkv2.ListKeysInput{},
		func(opts *paymentcryptography_sdkv2.Options) {
