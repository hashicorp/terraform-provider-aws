...
const (
	packageName = "servicediscovery"
	awsEnvVar   = "AWS_ENDPOINT_URL_SERVICEDISCOVERY"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "servicediscovery"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := servicediscovery.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, servicediscovery.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.ServiceDiscoveryClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving servicediscovery default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving servicediscovery FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up servicediscovery endpoint %q: %s", hostname, err)
	}
