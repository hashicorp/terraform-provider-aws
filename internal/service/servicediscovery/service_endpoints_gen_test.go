...
func defaultEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(servicediscovery_sdkv1.EndpointsID, region)
	if err != nil {
		return err.Error()
	}
...
func defaultFIPSEndpoint(region string) string {
	...
	ep, err := r.EndpointFor(servicediscovery_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) string {
	...
	client := meta.ServiceDiscoveryConn(ctx)

	req, _ := client.ListNamespacesRequest(&servicediscovery_sdkv1.ListNamespacesInput{})
