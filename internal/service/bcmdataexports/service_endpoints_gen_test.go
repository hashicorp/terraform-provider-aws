...
const (
	packageName = "bcmdataexports"
	awsEnvVar   = "AWS_ENDPOINT_URL_BCM_DATA_EXPORTS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "bcm_data_exports"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := bcmdataexports.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, bcmdataexports.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.BCMDataExportsClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving BCM Data Exports default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving BCM Data Exports FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up BCM Data Exports endpoint %q: %s", hostname, err)
	}
