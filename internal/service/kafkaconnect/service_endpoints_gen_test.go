...
const (
	packageName = "kafkaconnect"
	awsEnvVar   = "AWS_ENDPOINT_URL_KAFKACONNECT"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "kafkaconnect"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := kafkaconnect.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, kafkaconnect.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.KafkaConnectClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Managed Streaming for Kafka Connect default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Managed Streaming for Kafka Connect FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Managed Streaming for Kafka Connect endpoint %q: %s", hostname, err)
	}
