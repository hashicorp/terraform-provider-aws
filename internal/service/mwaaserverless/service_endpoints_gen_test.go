...
const (
	packageName = "mwaaserverless"
	awsEnvVar   = "AWS_ENDPOINT_URL_MWAA_SERVERLESS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "mwaa_serverless"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := mwaaserverless.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, mwaaserverless.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.MWAAServerlessClient(ctx)

	var result apiCallParams

	input := mwaaserverless.ListWorkflowsInput{}
	_, err := client.ListWorkflows(ctx, &input,
		func(opts *mwaaserverless.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving MWAA (Managed Workflows for Apache Airflow) Serverless default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving MWAA (Managed Workflows for Apache Airflow) Serverless FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up MWAA (Managed Workflows for Apache Airflow) Serverless endpoint %q: %s", hostname, err)
	}
