...
func TestAccBedrockAgentCoreHarness_model_geminiAdditionalParams(t *testing.T) {
	ctx := acctest.Context(t)
	var harness awstypes.Harness
	rName := testAccRandomHarnessName(t)
	credentialProviderName := acctest.RandomWithPrefix(t, acctest.ResourcePrefix)
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccHarnessConfig_geminiAdditionalParams(rName, credentialProviderName),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckHarnessExists(ctx, t, resourceName, &harness),
					resource.TestCheckResourceAttr(resourceName, "model.0.gemini_model_config.0.additional_params", `{"thinking_budget":1024}`),
				),
			},
			{
				ImportStateIdFunc:                    acctest.AttrImportStateIdFunc(resourceName, "harness_id"),
...
func testAccHarnessConfig_geminiAdditionalParams(rName, credentialProviderName string) string {
	return acctest.ConfigCompose(
		testAccHarnessConfig_iamRole(rName),
		testAccAPIKeyCredentialProviderConfig_basic(credentialProviderName, "test-api-key"),
		fmt.Sprintf(`
