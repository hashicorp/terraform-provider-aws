...
func TestAccBedrockAgentCoreHarness_authorizer_privateEndpointOverrides(t *testing.T) {
	ctx := acctest.Context(t)
	var harness awstypes.Harness
	rName := testAccRandomHarnessName(t)
	resourceName := "aws_bedrockagentcore_harness.test"
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// The API omits private_endpoint_overrides on read; the shared preserve fix must keep
				// the configured block so the implicit post-apply empty-plan check passes.
				Config: testAccHarnessConfig_authorizerPrivateEndpointOverrides(rName),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckHarnessExists(ctx, t, resourceName, &harness),
...
func testAccHarnessConfig_authorizerPrivateEndpointOverrides(rName string) string {
	return acctest.ConfigCompose(testAccHarnessConfig_iamRole(rName), fmt.Sprintf(`
