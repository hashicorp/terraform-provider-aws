...
var memoryConfig = func(rName string) string {
	return testAccMemoryConfig(rName, names.AttrDescription, 30, "aws_iam_role.test.arn")
}
...
func testAccCheckMemoryStrategyDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			_, err := tfbedrockagentcore.FindMemoryStrategyByID(ctx, conn, rs.Primary.Attributes["memory_id"], rs.Primary.ID)
			if tfresource.NotFound(err) {
				return nil
			}
			if err != nil {
				return create.Error(names.BedrockAgentCore, create.ErrActionCheckingDestroyed, tfbedrockagentcore.ResNameMemoryStrategy, rs.Primary.ID, err)
			}

			return create.Error(names.BedrockAgentCore, create.ErrActionCheckingDestroyed, tfbedrockagentcore.ResNameMemoryStrategy, rs.Primary.ID, errors.New("not destroyed"))
		}
...
func testAccCheckMemoryStrategyExists(ctx context.Context, name string, memorystrategy *awstypes.MemoryStrategy) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		resp, err := tfbedrockagentcore.FindMemoryStrategyByID(ctx, conn, rs.Primary.Attributes["memory_id"], rs.Primary.ID)
		if err != nil {
			return create.Error(names.BedrockAgentCore, create.ErrActionCheckingExistence, tfbedrockagentcore.ResNameMemoryStrategy, rs.Primary.ID, err)
		}
...
func testAccMemoryStrategyImportStateIdFunc(resourceName string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		return fmt.Sprintf("%s,%s", rs.Primary.Attributes["memory_id"], rs.Primary.ID), nil
	}
}
