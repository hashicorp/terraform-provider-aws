...
const (
	packageName = "kinesisanalytics"
	awsEnvVar   = "AWS_ENDPOINT_URL_KINESIS_ANALYTICS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "kinesis_analytics"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := kinesisanalytics.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, kinesisanalytics.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.KinesisAnalyticsClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Kinesis Analytics default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Kinesis Analytics FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Kinesis Analytics endpoint %q: %s", hostname, err)
	}
