...
const (
	packageName = "route53domains"
	awsEnvVar   = "AWS_ENDPOINT_URL_ROUTE_53_DOMAINS"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "route_53_domains"
)
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := route53domains.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, route53domains.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.Route53DomainsClient(ctx)
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Route 53 Domains default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Route 53 Domains FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errs.As[*net.DNSError](err); ok && dnsErr.IsNotFound {
		...
		t.Fatalf("looking up Route 53 Domains endpoint %q: %s", hostname, err)
	}
