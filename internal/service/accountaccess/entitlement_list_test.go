			...
			},
			{
				...
				QueryResultChecks: []querycheck.QueryResultCheck{
					...
					querycheck.ExpectResourceKnownValues(resourceName, tfqueryfilter.ByResourceIdentityFunc(identity1.Checks()), []querycheck.KnownValueCheck{
						...
						tfquerycheck.KnownValueCheck(tfjsonpath.New("entitlement").AtSliceIndex(0).AtMapKey("principal_role").AtSliceIndex(0).AtMapKey(names.AttrAccountID), knownvalue.NotNull()),
						tfquerycheck.KnownValueCheck(tfjsonpath.New("entitlement").AtSliceIndex(0).AtMapKey("principal_role").AtSliceIndex(0).AtMapKey("account_name"), knownvalue.NotNull()),
						tfquerycheck.KnownValueCheck(tfjsonpath.New("entitlement").AtSliceIndex(0).AtMapKey("principal_role").AtSliceIndex(0).AtMapKey(names.AttrPrincipal).AtSliceIndex(0).AtMapKey("identity_center").AtSliceIndex(0).AtMapKey("user_id"), knownvalue.NotNull()),
						tfquerycheck.KnownValueCheck(tfjsonpath.New(names.AttrRegion), knownvalue.StringExact(acctest.Region())),
						tfquerycheck.KnownValueCheck(tfjsonpath.New("entitlement").AtSliceIndex(0).AtMapKey("principal_role").AtSliceIndex(0).AtMapKey(names.AttrRoleARN), knownvalue.NotNull()),
					}),
				},
			},
		},
