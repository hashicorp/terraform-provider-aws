...
func testAccMesh_tags(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMeshConfigWithTags(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMeshExists(resourceName, &mesh),
			...
			{
				Config: testAccMeshConfigWithUpdateTags(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMeshExists(resourceName, &mesh),
			...
			{
				Config: testAccMeshConfigWithRemoveTags(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMeshExists(resourceName, &mesh),
...
func testAccMeshConfigWithTags(rName string) string {
	return fmt.Sprintf(`
resource "aws_appmesh_mesh" "test" {
  name = %[1]q
...
func testAccMeshConfigWithUpdateTags(rName string) string {
	return fmt.Sprintf(`
resource "aws_appmesh_mesh" "test" {
  name = %[1]q
...
func testAccMeshConfigWithRemoveTags(rName string) string {
	return fmt.Sprintf(`
resource "aws_appmesh_mesh" "test" {
  name = %[1]q
