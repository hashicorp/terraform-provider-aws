...
func (c *Config) clientConns(client *AWSClient, sess *session.Session) {
	...
	client.ComprehendConn = comprehend.New(sess.Copy(&aws.Config{Endpoint: aws.String(c.Endpoints[names.Comprehend])}))
	...
	client.MediaLiveConn = medialive.New(sess.Copy(&aws.Config{Endpoint: aws.String(c.Endpoints[names.MediaLive])}))
